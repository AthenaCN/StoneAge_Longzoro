/*
  ランキング機能付きデータベースの表  (带排行榜功能的数据库表)

  1999 Aug 14 Created by ringo

  データベースは1 のリンクの形で保存される  (数据库以1的链表形式保存)
...
/* データベースのリンクの要素1個をあらわす  (表示数据库链表的一个元素) */
struct dbentry {
	int use;
	//    unsigned int keyhash;       /* 検索キーのハッシュコード  (搜索键的哈希码) */
	int ivalue; /* スコアがトップ、NODE は  -1 で  (分数最高、NODE为-1)
				すべてのスコアは 0 以上でないといけない  (所有分数必须为0以上)*/
				//    int nextind;                /* -1 だったら    終わりを意味する  (-1则表示结束) */
	// Spock 2000/10/12
	int prev; // 前一个dbentry, -1表示此项为head
	int next; // 下一个dbentry, -1表示此项为tail
	char key[KEY_MAX];
	char charvalue[CHARVALUE_MAX];
	//    char key[64];               /* 検索キーとなる  値   (作为搜索键的值) */
	//    int charvalue_index;        /*   値  バッファをさすindex  (指向值缓冲区的索引) */
	// Spock end
};
...
/* 1個のデータベースをあらわす  (表示一个数据库) */
struct table {
	int use;	   // 0:未使用 1:已使用
	DBTYPE type;   /* DB及潘   */
	char name[32]; /* データベースの  名前  (数据库的名字) */
	int num;	   /* エントリの数  (条目的数量) */
	int toplinkindex;
	// Spock 2000/10/12
...
struct dbentry *master_buf; /* エントリ記憶用  (条目存储用) */
...
/*
	値  バッファを1個わりあてる  (分配一个值缓冲区)
  たりなくなったらreallocする  (不足时重新分配)
...
/*
  charvalue から/に値をゲット/セットする  (从/向charvalue取值/设值)
...
/*
  DBの大きさが足らなくなったらわりあてなおす。今のサイズの2倍にする  (DB大小不足时重新分配，设为当前大小的2倍)
  0だったら1にする  (若为0则设为1)
 */
static int
reallocDB(void) {
...
dbAllocNode() {
	...
	for (i = 0; i < dbsize; i++) {
		...
		if (master_buf[dbent_finder].use == 0) {
			...
			/* Spock deleted 2000/10/12
			// int でも付加木の為に stringbuffer を使いにする kawata  (即使int也为附加树将stringbuffer投入使用 kawata)
			if( type == DB_STRING || type == DB_INT_SORTED){
				if( ( master_buf[dbent_finder].charvalue_index =
					  dbAllocCharValue() ) < 0 ){
					//   値  バッファがたりないぞ  (值缓冲区不够了)
					return -1;
				}
			}
			*/
...
// Spock end
/*
  リンクのトップを与えられたらキーを  手がかりにノードを検索する  (给定链表顶部则按键线索搜索节点)
  みつからない場合はエラーではないので0  (找不到时并非错误，故返回0)
...
dbExtractNodeByKey( int topind , char *k  )
{
	...
	// リンクが尽きてもみつからないだけなので0を返す  (即使链表走完也未找到，所以返回0)
	...
	for(;;){
		...
		if( master_buf[cur].keyhash == h
			...
			// prev の値 が cur の値 になるようにする  (使prev的值变为cur的值)
			if( prev == -1 ){
				// 先  だったのでリンクはいじらない  (因为是先头所以不改动链表)
			} else {
				master_buf[prev].nextind = master_buf[cur].nextind;
			}
			// それで自分がリストから外れるので解放  する  (如此自己从列表脱离，故释放)
			dbReleaseNode( cur );
...
/*
  リンクのトップを与えられたら値を  手がかりにノードを検索して  (给定链表顶部则以值为线索搜索节点)
  適切なところに Insert する。小さいから大きいにならんでいる  (插入到合适位置，按从小到大排列)
  移烂
...
/*
  データベースの  名前を  取る。数が少ないので  適当にstrcmpしてよい  (获取数据库名，数量少所以可随意strcmp)
  DBTYPE : 種  (类型)

  同じ名前のものは  総数と  値  の  に存在することができる  (同名者可与总数和值并存)
	...
	for (i = 0; i < MAXTABLE; i++) {
		if (dbt[i].use == 0) {
			...
			*/
			...
			/* トップのノードを初期化するなり  (把顶部节点初始化即可)
			   値  の場合も総数の場合も同じでよい  (值的情况与总数的情况可相同)
			 0x7fffffff という値は  値  の場合さほど意味を持たないのだ  (0x7fffffff在数值情况下没什么意义)*/
...
/*
  ...
  とりあえず strtol でできる値しかサポートしないよ  (暂时只支持strtol能处理的值)

  検索して見つけたノードに対して  (对搜索找到的节点)
  リンクから  辿ってから今度はソートするのに使う値を手がかりに  (从链表遍历后，以排序所需值为线索)
  検索していい場所を見つけたらそこに差しこむ  (搜索到合适位置就插入)
...
/*
  エラーの場合は  0だったら返す  (错误时若为0则返回)

  int *rank_out : ランクの出  (排名输出)
  int *count_out : そこから何個か  の出  (从其中取出几个的输出)

  int データベース使用  ね  (数据库使用)
...
int dbGetEntryRank(char *table, char *key, int *rank_out, int *count_out) {
	...
	int cur;
	int now_score = 0x7fffffff; /*int でいちばんでかい値  (int中最大的值) */
	int r = -1, i = 0;
...
#define MAXHITS 1024 /* 適当やなあ。でもこれで十分らしいぞ ringo  (真随意啊，不过这样似乎够了 ringo) */
	struct hitent {	 /* この構造にヒットしたやつをたどっていく  (顺着此结构追踪命中的对象) */
		int entind;
		int rank;
	};
...
/* 指定した位置から指定した個数取りだす  (从指定位置取出指定个数)
  失敗したら  外  したら0。外  しても位置の出  の場合があるぞ  (失败则返回外/0；即使外，有时也有位置输出的情况)
   numが0の場合とか  該当するエントリがないとき  (num为0或没有对应条目时)

 int データベース使用  だぞ  (数据库使用中)
