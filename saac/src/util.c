...
char *makeEscapeString1(char *src, char *dest, int sizeofdest) {
	...
	for (i = 0; i < srclen; i++) {
		...
		char escapechar = '\0';
		if (destindex + 1 >= sizeofdest)
			/*  '\0'分が割りないのでここで終り  ('\0'部分不够所以在此结束) */
			break;

		/*	// マルチバイトかどうかをチェック  (检查是否多字节)
			if( IS_2BYTEWORD( src[i] ) ){
				// マルチバイトだ。その場合はバイト余分に進ませる  (是多字节则多推进一字节)
				// ただしバイトしかない場合はそうしない  (但只有一字节时不这样)
				// そのままバイト転送  (原样传输字节)
...
/* 繋ぎstrcatsafe。バッファがあふれる場合は何もしない  (连接strcatsafe。缓冲区溢出时不操作) */
