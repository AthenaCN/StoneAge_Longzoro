...
215 :      ACCharSaveに対する返事  (对ACCharSave的回复)
216 :      <br><br>
217 :      <dl>
218 :        <dt> string result
219 :        <dd>"successful" , "failed" のいずれか  (两者之一)
220 :        <dt> string data
221 :        <dd>"failed"の意味は 失敗を示す時間が判り分かる 値 である  (是表示failed含义、可分辨失败时间的值)
222 :        <dt> int id
223 :        <dd> 番号のidをそのままコピーして返す  (原样复制编号id并返回)
224 :      </dl>
225 :      <br>
226 :      <br>
227 :
...
257 :      ACCharDeleteに対する返事  (对ACCharDelete的回复)
258 :      <br><br>
259 :      <dl>
260 :        <dt> string result
261 :        <dd>"successful" , "failed" のいずれか  (两者之一)
262 :        <dt> string data
263 :        <dd>"failed"の意味は 失敗を示す時間が判り分かる 値 である  (是表示failed含义、可分辨失败时间的值)
264 :        <dt> int id
265 :        <dd> 番号のidのそのままのコピー  (编号id的原样复制)
266 :
267 :      </dl>
268 :      <br>
269 :      <br>
...
291 :      ACLockに対する返事  (对ACLock的回复)
292 :      <br><br>
293 :      <dl>
294 :        <dt>string result
295 :        <dd>"successful" , "failed" のいずれか  (两者之一)
296 :        <dt>string data
297 :        <dd>"failed"の意味は 失敗を示す時間が判り分かる 値 である  (是表示failed含义、可分辨失败时间的值)
298 :        <dt>int id
299 :        <dd>番号のidのそのままコピー  (编号id的原样复制)
300 :      </dl>
301 :      <br>
302 :      <br>
303 :
...
319 : 	 アカウントサーバーからゲームサーバーに対してあるユーザが現在ログイン  (从帐号服务器向游戏服务器确认某用户当前登录)
320 : 	 しているかどうかをチェックする  (检查是否…)
321 : 	 <BR><BR>
322 : 	 <dl>
323 : 	   <dt>string mem_id
324 : 	   <dd>調べる対象のユーザの gamersDream member id  (要检查的目标用户的gamersDream成员id)
325 : 	 </dl>
326 : 	 <BR><BR>
327 :
328 :
...
345 : 	 そのコマンドの結果が失敗しても復帰不可なので無視してよい  (即使该命令结果失败也无法恢复，所以可忽略)
346 : 	 <dl>
...
440 : 	   <dt>string table
441 : 	   <dd> テーブル をそのまま返す  (原样返回表)
442 : 	   <dt>string key
443 : 	   <dd>キーをそのまま返す  (原样返回键)
444 : 	   <dt>int msgid, int msgid2
445 : 	   <dd>msgid をそのまま返す  (原样返回msgid)
446 : 	 </dl>
447 : 	 <BR><BR>
448 :
*/
...
469 : 	   <dd>赐匏［1动晓及袄毛潸月［
470 : 	   <dt>int count
471 : 	   <dd>そのいくつ取り出すか。元のエントリは0から始まる  (取出几个。原始条目从0开始)
472 : 	   <dt>string table
473 : 	   <dd> テーブル をそのまま返す  (原样返回表)
474 : 	   <dt>string key
475 : 	   <dd>キーをそのまま返す  (原样返回键)
476 : 	   <dt>int msgid, int msgid2
477 : 	   <dd>msgid をそのまま返す  (原样返回msgid)
478 : 	 </dl>
479 : 	 <BR><BR>
480 :
481 :
...
499 : 	   <dt>string table
500 : 	   <dd> テーブル をそのまま返す  (原样返回表)
501 : 	   <dt>string key
502 : 	   <dd>キーをそのまま返す  (原样返回键)
503 : 	   <dt>int msgid, int msgid2
504 : 	   <dd>msgid をそのまま返す  (原样返回msgid)
505 : 	 </dl>
506 : 	 <BR><BR>
507 :
*/
...
526 : 	   <dd>値そのもの  (值本身)
527 : 	   <dt>string table
528 : 	   <dd> テーブル をそのまま返す  (原样返回表)
529 : 	   <dt>string key
530 : 	   <dd>キーをそのまま返す  (原样返回键)
531 : 	   <dt>int msgid, int msgid2
532 : 	   <dd>msgid をそのまま返す  (原样返回msgid)
533 : 	 </dl>
534 :
535 :
536 :
...
559 : 		   RANKは順位である。KEYはその順位になっているスコアを持っている  (RANK是排名，KEY是排在该名次的分值)
560 : 		   キー、SCOREは総数値でスコアである  (KEY、SCORE为数值型分值)
561 : 		     ［<br>
562 : 		   "1,RINGO,123465,data1|2,HOGE,4567,data2|2,FUCK,4567,data3"<BR>
563 : 		   この 場合、2位は2人いることになる  (此时第2名会有2人)
564 : 	   <dt>string table
565 : 	   <dd>  □皮伙
...
588 : 	   <dt>string list
589 : 	   <dd>リスト。1エントリあたりの個の値 "INDEX,RANK,KEY,SCORE,  (列表。每个条目的几个值)INFO" 及4蜊及树
590 : 		   の縦であり、それが個数分でつながる。個数が0の場合は  (纵向排列，按个数连接。个数为0时)
591 : 		   坞  侬  ［
592 : 	   <dt>int num
593 : 	   <dd>いくつ取り出すか。1なら1個、2なら2個  (取出几个。1则1个，2则2个)
...
612 : 	 Broadcastのアカウントサーバーからゲームサーバーへの送信用  (Broadcast从帐号服务器到游戏服务器的发送用)
613 : 	 <dl>
614 : 	   <dt>string id
615 : 	   <dd>user ID
...
619 : 	   <dd>メッセージ  (消息)
620 : 	   <dt>int flag
621 : 	   <dd>1にすると 自分には 送られない  (若为1则不发送给自己)
622 : 	 </dl>
623 :
*/
...
643 : 	 アカウントサーバーからゲームサーバーにメッセージを送信する  (从帐号服务器向游戏服务器发送消息)
644 : 	 <dl>
...
654 : 	   <dd>メッセージの  (消息的)
655 : 	   <dt>int option
656 : 	   <dd>オプション(その関数を参照)  (选项(参照相应函数))
657 : 	   <dt>int mesgid
658 : 	   <dd>メッセージID アカウントサーバーの保存用  (消息ID 帐号服务器保存用)
659 : 	 </dl>
660 :
*/
...
void saacproto_ACRecalPlayer_send(int fd, char *uid, int userfdid, int GmCliId, char *id, int char_num, int date, char *char_data, int backup_flag) {
	if (backup_flag == 1)
		saacproto_CreateHeader(saacproto.work, "ACRecalBackupDate"); // 回传备份日期
	else if (backup_flag == 2)
		saacproto_CreateHeader(saacproto.work, "ACRecalPlayer"); // 回传线上人物资料
	else if (backup_flag == 3)
		saacproto_CreateHeader(saacproto.work, "ACRecalBackupPlayer"); // 回传备份人物资料
	else if (backup_flag == 4)
		saacproto_CreateHeader(saacproto.work, "ACRecalAllBackupOK"); // 回传回溯ok讯息
