...
/*
   ゲームサーバーがアカウントサーバーからログアウトする  (游戏服务器从帐号服务器登出)
   int fd : 対象socket  (目标socket)
...
/*
   ...
   char *id : ID
   char *pas : パスワード  (密码)
   char *charname : キャラ  (角色)
   char *opt : オプション値のときにはロックして返す値  (作为选项值时锁定并返回的值)
   char *charinfo : ファイルに保存するキャラクタのステータス値  (保存到文件中的角色状态值)
   int id : メッセージID  (消息ID)
*/
#ifdef _NewSave
...
void saacproto_ACRecalPlayer_recv(int fd, char *uid, int userfdid, int GmCliId,
	...
	if (char_num < 0 || char_num > 1) {
		log("\n char_num(%d) err !!", char_num);
		return;
	}
	if (date < 20000000) {
		log("\n date(%d) format err!!", date);
		return;
	}
...
void saacproto_ACRecalBackup_recv(int fd, char *uid, int userfdid, int GmCliId, char *id,
	...
	if (backup_flag == 1) { // 回溯人物
		...
		for (i = 0; i < 5; i++) {
			...
			item_pet_chang.equit_item[i] = atoi(temp);
			count++;
		}
		for (i = 0; i < 15; i++) {
			memset(temp, 0, sizeof(temp));
			easyGetTokenFromString(char_data, count, temp, sizeof(temp));
			item_pet_chang.have_item[i] = atoi(temp);
			count++;
		}
		for (i = 0; i < 30; i++) {
			memset(temp, 0, sizeof(temp));
			easyGetTokenFromString(char_data, count, temp, sizeof(temp));
			item_pet_chang.pool_item[i] = atoi(temp);
			count++;
		}
		for (i = 0; i < 5; i++) {
			memset(temp, 0, sizeof(temp));
			easyGetTokenFromString(char_data, count, temp, sizeof(temp));
			item_pet_chang.have_pet[i] = atoi(temp);
			count++;
		}
		for (i = 0; i < 20; i++) {
			memset(temp, 0, sizeof(temp));
			easyGetTokenFromString(char_data, count, temp, sizeof(temp));
			item_pet_chang.pool_pet[i] = atoi(temp);
			count++;
		}
...
void analysis_char(char *char_data) {
	...
	for (i = 0; i < MAX_HAVE_ITEM; i++) {
		...
		if ((strcmp(player_data.item[i], "")) == 0)
			continue;
...
int get_delim(char *msg, char *temp, char *delime1, char *delime2) {
	...
	if ((out1 = strstr(msg, delime1)) == NULL)
		return 0;
	if ((out2 = strstr(out1 + strlen(delime1), delime2)) == NULL)
		return 0;
	if ((buffsize = strlen(out1) - strlen(out2) - strlen(delime1)) <= 0)
		return 0;
...
int rgetFileName(char *dirname, STRING64 *string, int size) {
	int index = 0;
	if (rrd(dirname, string, size, &index) == -1)
		return -1;
	else
		return index;
}
...
extern void delMissionTableOnedata(int index);
	...
	if (racetype == 0) {
		...
		for (i = 0; i < DEFMAXRANK; i++) {
			...
			if (catchcnt > ranktable[i].catchcnt) { // 只有大於才能把人家挤掉
				...
				if (strcmp(id, ranktable[i].id) == 0 && strcmp(name, ranktable[i].name) == 0)
					continue;
			}

			if (ranktable[i].catchcnt == 0)
				break;
