...
/*
		...
		このファイルで定義されている関数はぜんぶ数をつかう  (本文件中定义的函数全部使用数字)


				char *c0 : ID
				char *c1 : Passwd  もう済んでるから構わない  (已经处理完了所以没关系)
				char *c2 : Charname
				char *c3 : opt
...
void charLoadCallback(int ti, int auth, char *c0, char *c1,
	...
	if (charindex < 0) {
		/* キャラが存在しないのでエラーなのだ  (因为角色不存在所以是错误) */
#ifdef _NewSave
...
static void getCharOptionFromString(char *str, char *out) {
	...
	for (c = 0;; c++) {
		...
		if (IS_2BYTEWORD(str[c])) {
			c++;
			if (str[c] == '\0')
				break; // その値がすでにNULLなら終わり  (若该值已是NULL则结束)
			continue;
		}
		...
		if (str[c] == SPACE) {
			...
			for (;; c++) {
				...
				if (str[c] == '\0') { // 終わったら NULL にされて終わる  (结束后被置为NULL并结束)
					out[outc] = '\0';
					break;
				}

				// ここも2バイト値の1バイトならもう1バイトコピーする  (这里也是，若2字节值的1字节则再复制1字节)
				//----   For 2Byte Code ----
				if (IS_2BYTEWORD(str[c])) {
					out[outc++] = str[c++]; // 1バイトコピー  (复制1字节)
					out[outc++] = str[c];	// 2バイトコピー  (复制2字节)
					if (str[c] == '\0')
						break; // その値がすでにNULLなら終わり  (若该值已是NULL则结束)
					continue;
				}

				// 異常値は1バイトコピー  (异常值复制1字节)
				out[outc] = str[c]; // 普通にコピー  (普通复制)
				if (str[c] == '\0')
					break; // 終わったら終わる  (结束后结束)
...
static int makeSaveCharString(char *out, int outlen,
...
#endif
	//    }
	/* つなげる。途中がエラー検出のために一括ワクに入れる  (连接。途中因错误检测而放入统一框中) */
	...
	/* NULL値を入れるから0を入れんといかんのだ  (因放入NULL值所以必须放0) */
...
/*
  ...
  キャラの空いているスロットを検索する  (搜索角色的空闲槽位)

		  なら空いていない。0以上なら空きスロットのindex.  (若没有空闲；0以上则为空槽位的index)


				キーにヒットできなかったらそれを返すだけ  (若未命中键则只返回它)
				もっとちゃんとできるやろけども  (虽然可以做得更好)
