...
static void checkMissionTimelimit(void);
#endif
...
int mainsockfd;				  /* accept の一覧に入れる  (放入accept列表中) */
struct sockaddr_in localaddr; /* bind するアドレス  (bind的地址) */

struct connection *con; /* コネクション  (连接) */

static int mb_finder = 0; /* mbの位置を検索するための  (用于搜索mb的位置)
					検索トップの位置 記憶用  (记忆搜索起点位置用) */
// WON FIX
char tmpbuf[65536];
...
#define OK 0					  /* 正常  (正常) */
#define TCPSTRUCT_ENOMEM -1		  /* malloc 失敗  (malloc失败) */
#define TCPSTRUCT_ESOCK -2		  /* socket 失敗  (socket失败) */
#define TCPSTRUCT_EBIND -3		  /* bind 失敗  (bind失败) */
#define TCPSTRUCT_ELISTEN -4	  /* listen 失敗  (listen失败) */
#define TCPSTRUCT_EBUG -6		  /* バグである  (是bug) */
#define TCPSTRUCT_EINVCIND -7	  /* conへのindexがおかしいよ  (对con的index不对) */
#define TCPSTRUCT_EREADFIN -8	  /* read するデータがなくて closed by remote  (没有可read的数据，被远端关闭) */
#define TCPSTRUCT_EHOST -9		  /* gethostbyname 失敗  (gethostbyname失败) */
#define TCPSTRUCT_ECONNECT -10	  /* connect 失敗  (connect失败) */
#define TCPSTRUCT_ECFULL -11	  /* con がいっぱい  (con已满) */
#define TCPSTRUCT_ETOOLONG -12	  /* バッファが長すぎる  (缓冲区太长) */
#define TCPSTRUCT_EMBFULL -13	  /* mb がいっぱい  (mb已满) */
#define TCPSTRUCT_ECLOSEAGAIN -14 /* close が2回された  (close被调用两次) */
#endif

int port; /* ゲームサーバーが待機してくるポート  (游戏服务器等待接入的端口) */
int Total_Charlist;
int Expired_mail;
...
#ifndef _FIX_WORKS
typedef enum {
	WKSTAT_IDLE = 0,   /* 何もしてない状態  (什么都没做的状态) */
	WKSTAT_WAITING = 2 /* ワカからの返事を待ってる状態  (等待Waka回复的状态) */
} WKSTAT;
...
int main(int argc, char **argv) {
...
#ifdef _DEATH_CONTEND
	if (PKLIST_InitPkTeamList(2000) == -1)
		exit(1);
	...
	if (PKLIST_LoadPkTeamListfromFile("pklist", "pk_list_team.txt") == -1)
		exit(1);
	...
	/* たまってるパケットを 見こむ  (收进积压的数据包) */
	if (readMail(maildir) < 0) {
		log("不能初始化邮件\n");
		exit(1);
	}

	/* TCPSTRUCT を初期化  (初始化TCPSTRUCT) */
	{
		int tcpr;
	...
	while (1) {
		...
		/* ワカのタイムアウトを調べる  (检查Waka超时)
		   タイムアウトしたらデバッグを出してワカを殺してしまう  (超时则输出调试信息并杀掉Waka)
		   で、ゲームサーバーに終了を通知する  (然后通知游戏服务器结束)           */
		{
			int i;
...
/*
  デバッグのヒットカウントをクリアする  (清除调试命中计数)
  同じ名前のものはすべてのデバッグファイルが同じになるようにする  (让同名内容的所有调试文件保持一致)

  この仕様は数値の桁が大きくなるが、インターバルを更新しても  (此规格数值位数会变大，但即使更新间隔)
	暹屺分［
...
int tcpstruct_init(char *addr, int p, int timeout_ms, int mem_use, int db) {
	...
	{
		int opt = 1;
		setsockopt(mainsockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
	}
	...
	{
		int opt = 1;
		setsockopt(worksockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
	}
	bzero(&localaddr, sizeof(localaddr));
	localaddr.sin_family = AF_INET;
	...
	if (bind(worksockfd, (struct sockaddr *)&localaddr,
			 sizeof(localaddr)) < 0)
		return TCPSTRUCT_WORKEBIND;
	if (listen(worksockfd, BACKLOGNUM) < 0)
		return TCPSTRUCT_ELISTEN;
#endif
	return OK;
}
...
/*
	読み込むものがもうなくてしかも remoteclose だったら -1 を返す  (没有可读内容且已remoteclose时返回-1)
...
/*
  バッファを読みこむ  (读入缓冲区)
  int kend : 1ならバッファの \n を殺す  (若为1则删除缓冲区的换行符)
  int kend_r : 1ならバッファの \r も殺す(あれば)  (若为1则也删除缓冲区的回车(若有))

	読み込むものがもうなくてかつ remote closed だったら-1を返す  (没有可读内容且已remote closed时返回-1)
  // Nuke
	Read 1 line
...
static int consumeMemBufList(int top, char *out, int len,
	...
	if (consumeflag) {
		/* 長さが0になってるなら解放でも、元のやつは解放しないぞ  (若长度已为0则即使释放，原来的那个也不释放) */
...
/*
  ...
  return:
	はエラー  (是错误)
  0以上の場合は read してもよい量  (0以上时为可read的量)


  mbsize から mbuse を引いてサイズを計算するだけ  (只是用mbsize减去mbuse来计算大小)
  こまかい余りの分は無視する。そんな細かい値にこだわらなくても可  (细小的余数部分忽略，不必在意那么细的值)
...
/*
  ...
  membuf の位置を探しだす  (找出membuf的位置)

  return : みつからなかったら >=0 で index.  (找不到时返回>=0的index)
  みつからなかったら  (若找不到)

  検索のときには mb_finder を使う  (搜索时使用mb_finder)
  これで検索して 先頭にregする  (这样搜索并注册到开头)
 */
...
/*
  すべてのゲームサーバーに送る  (发送给所有游戏服务器)

  int flag : 1だったら送信先には送らない  (若为1则不发送给发送目标)
