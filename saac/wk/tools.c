...
void stockStore(Stock_Struct *s, char *buf, int limit) {
	...
	if (ptr && s->tail &&
		...
		strcat((char *)realloc(s->tail->data,
							   strlen(s->tail->data) + strlen(ptr) + 1),
			   ptr);
		if (Ytoken == ';') {
	...
	while (ptr) {
		stockAdd(s, ptr);
		if (Xtoken == '&')
			s->tail->flag |= STOCK_START; // The start
...
void rankAdd(Stock_Struct *ss, int rank, Rank_Struct *rs) {
	...
	if (!ss) {
		error("rankAdd(): Null Stock?\n");
		return;
	}
	if (!rs) {
		error("rankAdd(): Null Storage?\n");
		return;
	}
	if (rank <= 0) {
		error("rankAdd(): Invalid rank\n");
		return;
	}
	for (i = 0; i < 10; i++) {
		rs->entry[i].name[0] = 0;
	...
	for (i = 0; (i < 10) && (ss->head); i++) {
		...
		if (ss->head)
			rs->entry[i].score = atoi(stockOut(ss));
		else
			rs->entry[i].score = 0;
	}
}
