 ...
 *      argv      引数の配列 (参数的数组)
 * 戻り値 (返回值)
 *      TRUE(1)     正常なコマンドライン引数だったら (命令行参数正常时)
 *      FALSE(0)    異常なコマンドライン引数だったら (命令行参数异常时)
 */
BOOL parseCommandLine(int argc, char **argv)
{
	int c;				 /* getopt を使う (使用 getopt) */
	extern char *optarg; /* getopt を使う (使用 getopt) */
...
/* lsgen用のワーキングバッファの大きさ (lsgen 用工作缓冲区大小) */
 ...
 * 初期化ルーチン (初始化例程)
 * 引数 (参数)
 *      argc    argvの数 (argv 的个数)
 *      argv    コマンドライン引数 (命令行参数)
 *      env     環境変数 (环境变量)
 * 戻り値 (返回值)
 *      TRUE(1) 成功 (成功)
 *      FALSE(1) 失敗 (失败)
...
BOOL init(int argc, char **argv, char **env)
{
	...
	*/
	...
	print("初始化 NPC 客户端 ... ");
	/*  rpc(client)の初期化 (rpc(client) 的初始化) */
