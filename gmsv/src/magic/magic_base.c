...
typedef struct tagMagic_MagicFunctionTable {
	char *functionname;	 /*	各修正ファイルに書く関数の  名前  (各修正文件中写入的函数名) */
	MAGIC_CALLFUNC func; /*    実際に呼び出される関数  (实际调用的函数) */
	int hash;			 /*  hash */
} MAGIC_MagicFunctionTable;

/* 各関数をここに登録する仕様  (将各函数在此登记的规格) */
...
/*------------------------------------------------------------------------
 * Magicの修整ファイル  読み  込み  (读取Magic修正文件)
 *-----------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------
 * MAGIC_IDから配列を知る関数  (从MAGIC_ID获取数组下标的函数)
 * 返り値  (返回值)
 * 戻り : 配列  (返回:数组下标)
 * エラー : -1  (出错:-1)
 *-----------------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * 各の関数  からポインタ□を返す  (从各函数返回指针)
 * 娄醒
 *  name        char*       各の  名前  (name: 各函数名)
 * 返り値  (返回值)
 *  関数へのポインタが無い場合にはNULL  (没有函数指针时返回NULL)
 ------------------------------------------------------------*/
...
// Nuke start (08/23)
/*
  Nuke  さんのチェック  (Nuke的检查)
	法の効果  区をチェックする  (检查法术效果区)
