...
{
	BOOL acceptmore;				/*  1回目なら、accept したあと
										すぐに close する */
	unsigned int fdid;				/*  fd の残り数 */
	unsigned int closeallsocketnum; /*   closeallsocket   の残りの
										 数*/
	int shutdown;					/*  サーバをshutdownするモード
									 *	0:通常 それ以外:シャットダウンモード
									 * モードになってからの時間が  たっている。
									 */
	int dsptime;					/* shutdown モードのワーク数*/
	int limittime;					/* これも */
} ServerState;
...
#define MTIO_FIXED_BUFSIZE (65536)
	...
	int wbuse;
	pthread_mutex_t mutex; /* Connectをいじるときに使うロック */
#else
	char *rb;
	int rbuse;
	char *wb;
	...
	struct sockaddr_in sin; /* ソケット先のアドレス */
	...
	int charaindex;				   /* char    へのインデックス。
									* ログイン時に保存される。 -1がデフォルトで、
									*     にない。
									*/
	char CAbuf[2048];			   /*  CA() を蓄えるためのバッファ */
	int CAbufsiz;				   /*  CAbuf のサイズ  */
	struct timeval lastCAsendtime; /*    最後にCAを送った間 */

	char CDbuf[2048];			   /*  CD() を蓄えるためのバッファ */
	int CDbufsiz;				   /*  CDbuf のサイズ  */
	struct timeval lastCDsendtime; /*    最後にCDを送った間 */

	struct timeval lastCharSaveTime; /*     最後にキャラデータをセーブした時間 */

	struct timeval lastprocesstime; /*     最後にプロトコルを処理した時間*/
	struct timeval lastreadtime;	/*     最後にreadした時間。 とは違う*/
...
#define BEO_RESTORE 100
	...
	int appendwb_overflow_flag; /* 1度でもappendWbが失敗したら1にする */
...
#define CONNECT_LOCK_ARG2(i, j)                                                             \
	fprintf(stderr, "LO T:%d(%d:%d) %s %d", (int)pthread_self(), i, j, __FILE__, __LINE__); \
	pthread_mutex_lock(&Connect[i].mutex);                                                  \
	fprintf(stderr, "CK T:%d(%d:%d)\n", (int)pthread_self(), i, j);
#define CONNECT_UNLOCK_ARG2(i, j)                                                             \
	fprintf(stderr, "UNLO T:%d(%d:%d) %s %d", (int)pthread_self(), i, j, __FILE__, __LINE__); \
	pthread_mutex_unlock(&Connect[i].mutex);                                                  \
	fprintf(stderr, "CK T:%d(%d:%d)\n", (int)pthread_self(), i, j);
#define CONNECT_LOCK(i)                                                               \
	fprintf(stderr, "LO T:%d(%d) %s %d", (int)pthread_self(), i, __FILE__, __LINE__); \
	pthread_mutex_lock(&Connect[i].mutex);                                            \
	fprintf(stderr, "CK T:%d(%d)\n", (int)pthread_self(), i);
#define CONNECT_UNLOCK(i)                                                               \
	fprintf(stderr, "UNLO T:%d(%d) %s %d", (int)pthread_self(), i, __FILE__, __LINE__); \
	pthread_mutex_unlock(&Connect[i].mutex);                                            \
	fprintf(stderr, "CK T:%d(%d)\n", (int)pthread_self(), i);
/* 下記のログはものすごくディスクを食うのでスリープを入れたり */
...
static int appendWB(int fd, char *buf, int size)
{
	...
	if (fd != acfd) {
		...
		if (Connect[fd].wbuse + size > AC_WBSIZE) {
			...
			if ((fp = fopen("appendWBerr.log", "a+")) == NULL)
				return -1;
...
static int appendRB(int fd, char *buf, int size)
{
	if (fd != acfd) {
		if (Connect[fd].rbuse + size > RBSIZE) {
			if (fd == mfd)
				print("appendRB:MSERVER err buffer over \n");
			else
...
/*------------------------------------------------------------
 * CAcheck などに使われる関数。 実際に送る。 (被CAcheck等使用的函数。实际发送。)
 * 引数
 *  fd      int     ファイルディスクリプタ
 * 戻り値
 *  なし
...
/*------------------------------------------------------------
 * CDcheck などに使われる関数。 実際に送る。 (被CAcheck等使用的函数。实际发送。)
 * 引数
 *  fd      int     ファイルディスクリプタ
 * 戻り値
 *  なし
...
/*------------------------------------------------------------
 * fd が valid なものかどうかを調べる (检查fd是否有效)
 * 引数
 *  fd          int         fd
 * 戻り値
 *  valid   TRUE(1)
...
/*------------------------------------------------------------
 * cdkey から fd を得る。 (从cdkey获得fd)
 * 引数
 *  cd      char*       cdkey
 * 戻り値
 *  ファイルディスクリプタ  よかったら、 -1 の場合はエラー
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * charaindex から fd を得る。 (从charaindex获得fd)
 *   高速のチェックを なくした。 (去掉了高速检查)
 * 引数
...
/*------------------------------------------------------------
 * charaindex から cdkey を得る。 (从charaindex获得cdkey)
 * 引数
 *  charaindex  int     キャラのインデックス
 * 戻り値
 *  0なら真  、  なら偽
...
/*------------------------------------------------------------
 * fdid からキャラのindex をもとめる。 (从fdid求得角色索引)
 * 引数
 *  fdid    int     fdのid
 * 戻り値
 *  -1 の場合はログインのキャラはみつからなかった。0以上なら
 * ログインのキャラのキャラの配列への index
 ------------------------------------------------------------*/
ANYTHREAD int getCharindexFromFdid(int fdid)
{
	...
	for (i = 0; i < ConnectLen; i++) {
		CONNECT_LOCK(i);
		if (Connect[i].use == TRUE
			&& Connect[i].fdid == fdid && Connect[i].charaindex >= 0) {
...
/*------------------------------------------------------------
 * キャラindex から fdid をもとめる。 (从角色索引求得fdid)
 * 引数
 *  charind  int     ファイルディスクリプタ
 * かえりち fdid    だったらキャラindがおかしい
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * fdに割り当てられたソケットがクライアントだと、BOOLを返す (若分配给fd的套接字是客户端，则返回BOOL)
 * 軽いエラーチェックはしない。 (不做轻量错误检查)
 * 引数
 *  fd  int     ファイルディスクリプタ
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * fdに割り当てられたソケットがアカウントサーバだと、BOOLを返す (若分配给fd的套接字是账号服务器，则返回BOOL)
 * 軽いエラーチェックはしない。
 * 引数
 *  fd  int     ファイルディスクリプタ
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * fdに割り当てられたソケットがログイン状態であるかどうか (分配给fd的套接字是否处于登录状态)
 * を返す
 * 引数
 *  fd  int     ファイルディスクリプタ
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * 全部のソケットを切断して、アカウントサーバにデータを保存しようとする。 (切断全部套接字，向账号服务器保存数据)
 * 引数・戻り値
 *  なし
 ------------------------------------------------------------*/
...
#ifdef _ITEM_ADDEXP // vincent 经验提升
					if (CHAR_getWorkInt(Connect[i].charaindex, CHAR_WORKITEM_ADDEXP) > 0 && CHAR_getInt(Connect[i].charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
#if 1
	...
	if (ret > 0 && FD_ISSET(bindedfd, &fdset)) {
		...
		if (sockfd == -1 && errno == EINTR) {
			...
			if ((cono == 0) || (acceptmore <= 0) || isThereThisIP(sinip)) {
				...
				// char mess[1024]="E伺服器忙线中，请稍候再试。";
				// WSL修复: 客户端(127.0.0.1)与saac(127.0.0.1)来源IP相同导致from_acsv误判，
				// 原代码if(!from_acsv)会跳过握手发送，客户端收不到握手就不发登录请求，30秒后被超时踢掉。
				// saac不会主动connect到gmsv监听端口(gmsv主动连saac)，故无条件发送握手不影响服务器间通信。
				// Tokyo-sa补丁: 9.0服务端握手字符是'L'(Longzoro/龙zoro)，不是'F'。客户端收到'F'会立即断开报版本不同。
				mess[0] = 'L';
...
#define CREDIT_SPOOL 3
			...
			default: {
				...
				// Syu ADD 定时读取Announce
				if (j_counter > 60 * 10) { // 6000 约 600秒=10分钟
					j_counter = 0;
					print("\n定时读取公告");
					LoadAnnounce();
...
#else
						ret = write(fdremember, Connect[fdremember].wb,
									(Connect[fdremember].wbuse < 16384) ? Connect[fdremember].wbuse : 16384);
#endif
					}
					// Nuke end
...
/*
  1行とりだす。とりだした長さを返す (取出1行。返回取出的长度)
  高速化のため、 outlen にはつねに十分なサイズがはいっていると仮定するぞ (为提速，假定outlen总有足够空间)
 */
...
MTIO_loop(void)
{
	while (1) {
		...
		/* マルチスレッドなので遅くてもよいのだ。 50msまで待ってみる*/
		tmv.tv_sec = 0;
		tmv.tv_usec = 50 * 1000;
		...
		for (i = 0; i < ConnectLen; i++) {
			...
			if (Connect[i].use && Connect[i].wbuse > 0)
				FD_SET(i, &fds);
...
/*
  I/Oでないスレッドからのみ呼ばれる。 (仅从非I/O线程调用)
  closeはI/Oスレッドでは一切呼ばない (I/O线程中绝不调用close)
 */
MUTLITHREAD static void closeCheck(void)
{
	int i;
	for (i = 0; i < ConnectLen; i++) {
		if (CONNECT_getUse_debug(i, 2628) && CONNECT_getClosed(i)) {
			print("CLOSING %d\n", i);
			close(i);
			CONNECT_endOne_debug(i);
		}
...
/* netloop はメインスレッドで呼び出される。 定義しなければならない */
...
MUTLITHREAD BOOL netloop_faster(void)
{
	...
	for (i = 0; i < ConnectLen; i++) {
		...
		int ll;
		if (CONNECT_getUse_debug(i, 2527) == FALSE)
			continue;
		if (CONNECT_getState(i) == WHILECLOSEALLSOCKETSSAVE)
			continue;
...
BOOL MSBUF_CHECKbuflen(int size, float defp)
{
	if (mfd == -1)
		return FALSE;
	if (Connect[mfd].wbuse + size >= WBSIZE * defp)
		return FALSE;
