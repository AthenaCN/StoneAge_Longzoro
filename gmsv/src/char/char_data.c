...
/*------------------------------------------------------------
  * 顔イメージ番号のチェック  (检查形象编号)
 *
  * 顔イメージ番号とスプライトが一致しているか  (形象编号与精灵是否匹配)
  * number = キャラクター  (角色)
...
int CHAR_eqimagetbl[][5] = {
	// 怒り         エモーション   笑い         泣き       ノーマル  (愤怒/表情/笑/哭/普通)
...
/*------------------------------------------------------------
  * 番号とイメージ番号から装備中のイメージ番号を返す  (由番号与形象号返回装备形象)
 * 引数
  *  basebaseimagenumber     int             見ているイメージ番号  (查看的形象编号)
  *  category                ITEM_CATEGORY   アクションの種類  (动作种类)
 * 戻り値
  *  装備中のイメージ番号。見つからない時は -1  (装备中的形象编号，未找到时为-1)
 ------------------------------------------------------------*/
int CHAR_getNewImagenumberFromEquip(int basebaseimagenumber,
									ITEM_CATEGORY category) {
	int i;
	/*  ITEM_CATEGORY チェック  (类别检查)  */
...
/*------------------------------------------------------------
  *   値からイメージ番号を返す  (由值返回形象编号)
 * 引数
  *  string      char*         値  (值)
 * 戻り値
  *  見つからない -1  (未找到返回-1)
  *  それ以外はハッシュテーブルで検索  (否则用哈希表查找)
 ------------------------------------------------------------*/
...
BOOL CHAR_isInvincibleArea(int floor, int x, int y) {
	int i;
	for (i = 0; i < CHAR_invareanum; i++)
		if (CHAR_invarea[i].kind == CHAR_INVAREA && CHAR_invarea[i].floor == floor && CoordinateInRect(&CHAR_invarea[i].area, x, y))
			return TRUE;
	return FALSE;
}
...
BOOL CHAR_isCannotMagicArea(int floor, int x, int y, int magicnum) {
	int i;
	for (i = 0; i < CHAR_invareanum; i++) {
		if (CHAR_invarea[i].kind == CHAR_CANNOTMAGICAREA && CHAR_invarea[i].floor == floor && CoordinateInRect(&CHAR_invarea[i].area, x, y)) {
			int j;
			for (j = 0;
...
BOOL CHAR_isCannotDropArea(int floor, int x, int y) {
	int i;
	for (i = 0; i < CHAR_invareanum; i++)
		if (CHAR_invarea[i].kind == CHAR_CANNOTDROPAREA && CHAR_invarea[i].floor == floor && CoordinateInRect(&CHAR_invarea[i].area, x, y))
			return TRUE;
	return FALSE;
}
...
/*------------------------------------------------------------
  * マスターの位置を返す  (返回导师位置)
 * 引数
  *  elderindex      int     マスターインターフェース  (导师索引)
  *  fl              int*    フロアを受け取るポインタ  (接收楼层的指针)
  *  x               int*    xを受け取るポインタ  (接收x的指针)
  *  y               int*    yを受け取るポインタ  (接收y的指针)
 * 戻り値
 *  成功    TRUE(1)
 *  失敗    FALSE(0)
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
  * マスターNPCの初期化の時にマスターの場所を設定する  (初始化导师NPC时设定导师位置)
  * そのためのインターフェースデータ  (其用的接口数据)
 * 引数
  *  index   int 見つからなかった時FALSE を返す  (未找到时返回FALSE)
  *  fl      int 場所の番号  (位置编号)
 *  x       int
 *  y       int
  * NPCの座標と変わった場合  (NPC坐标变化时)
 ------------------------------------------------------------*/
...
int CHAR_PetLevelUp(int petindex) {
	...
	// ランダムにパラメータを加算して調整する  (随机加成调整参数)
	for (i = 0; i < 10; i++) {
		Param[RAND(0, 3)] += 1.0;
	}

	// ランクによる成長率の範囲  (按阶级的成长率范围)
...
#endif
#ifdef _FMVER21
					if (CHAR_getInt(ownerindex, CHAR_FMLEADERFLAG) > 0 && CHAR_getInt(ownerindex, CHAR_FMLEADERFLAG) != FMMEMBER_APPLY) {
...
void CHAR_CheckWaterStatus(int charaindex) {
	...
	if (CHAR_getWorkInt(charaindex, CHAR_WORKMAPFLOORTYPE) == 1 &&
		...
		if (myhp <= 0) {
			myhp = 1;
			if (CHAR_getInt(charaindex, CHAR_FLOOR) == 151 || CHAR_getInt(charaindex, CHAR_FLOOR) == 160 || CHAR_getInt(charaindex, CHAR_FLOOR) == 161)
