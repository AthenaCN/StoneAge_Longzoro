...
int PKLIST_GetPkTeamListArrayFromNum(int teamnum) {
	int i;
	for (i = 0; i < maxteam; i++) {
		if (PkTeamList[i].use != 1)
			continue;
		if (PkTeamList[i].teamnum == teamnum)
			return i;
	}
	return -1;
}
...
BOOL PKLIST_CHECKPkTeamSame(int teamnum, int charaindex, char *cdkey, int toteamnum) {
	...
	for (k = 0; k < MAXBATTLENUM; k++) {
		...
		if (PkTeamList[array].BHistory[k].use == 0)
			continue;
		if (PkTeamList[array].BHistory[k].teamnum == toteamnum)
			return FALSE;
	}
	return TRUE;
}
...
int PKLIST_CHECKPkTeamNew(int teamnum, int charaindex, char *cdkey) {
	...
	for (k = 0; k < MAXBATTLENUM; k++) {
		if (PkTeamList[array].BHistory[k].use == 1)
			continue;
		break;
	}
	if (k >= DEFMAXBATTLENUM) {
...
BOOL PKLIST_LoadPkTeamListDataSub(int ti, char *data) {
	int oldinside = -1;
	char buf[512];
	if (getStringFromIndexWithDelim(data, "|", 1, buf, sizeof(buf)) == FALSE)
		return FALSE;
	PkTeamList[ti].teamnum = atoi(buf);
	if (getStringFromIndexWithDelim(data, "|", 2, buf, sizeof(buf)) == FALSE)
		return FALSE;
	sprintf(PkTeamList[ti].pathdir, "%s", buf);
	if (getStringFromIndexWithDelim(data, "|", 3, buf, sizeof(buf)) == FALSE)
		return FALSE;
	sprintf(PkTeamList[ti].teamname, "%s", buf);
	if (getStringFromIndexWithDelim(data, "|", 4, buf, sizeof(buf)) == FALSE)
		return FALSE;
	sprintf(PkTeamList[ti].leadercdkey, "%s", buf);
	if (getStringFromIndexWithDelim(data, "|", 5, buf, sizeof(buf)) == FALSE)
		return FALSE;
	PkTeamList[ti].win = atoi(buf);
	if (getStringFromIndexWithDelim(data, "|", 6, buf, sizeof(buf)) == FALSE)
		return FALSE;
	PkTeamList[ti].lost = atoi(buf);
	if (getStringFromIndexWithDelim(data, "|", 7, buf, sizeof(buf)) == FALSE)
		return FALSE;
	PkTeamList[ti].battleplay = atoi(buf);
	if (getStringFromIndexWithDelim(data, "|", 8, buf, sizeof(buf)) == FALSE)
		return FALSE;
	PkTeamList[ti].score = atoi(buf);

	oldinside = PkTeamList[ti].inside;
	if (getStringFromIndexWithDelim(data, "|", 9, buf, sizeof(buf)) == FALSE)
		return FALSE;
...
void PKLIST_LoadInitPkTeamListData(void) {
	...
	while (1) {
		...
		if (fgets(szReadData, 1024, fp) == NULL)
			break;
		...
		for (i = 0; i < MAXTEAMMANNUM; i++) {
			if (getStringFromIndexWithDelim(szReadData, "|", 3 + i * 2, buf, sizeof(buf)) == FALSE) {
				if (i == 0)
					print("err team has no leader data:%s\n", szReadData);
				break;
			}
			if (getStringFromIndexWithDelim(szReadData, "|", 4 + i * 2, buf1, sizeof(buf1)) == FALSE)
				break;
...
BOOL PKLIST_LoadPkTeamListData(void) // 改成读档
{
	...
	while (1) {
		memset(szReadData, 0, sizeof(szReadData));
		memset(buf, 0, sizeof(buf));
		if (fgets(szReadData, 10240, fp) == NULL)
			break;
...
void PKLIST_InsertTeamNum(int charaindex) {
	...
	for (i = 0; i < maxteam; i++) {
		if (PkTeamList[i].use == 0)
			continue;
		for (k = 0; k < MAXTEAMMANNUM; k++) {
			if (PkTeamList[i].MyTeamMans[k].use == 0)
				continue;
			if (strcmp(PkTeamList[i].MyTeamMans[k].cdkey, cdkey))
				continue;
...
void PKLIST_ShowPkListTeamData(void) {
	int i;
	{
		...
		for (i = 0; i < maxteam; i++) {
			...
			print("\n");
			for (k = 0; k < MAXBATTLENUM; k++) {
				if (PkTeamList[i].BHistory[k].use == 0)
					continue;
				if (k != 0 && k % 10 == 0)
					print("\n");
...
void NPC_PKLIST_PlayerLogout_Exit(int charaindex) {
...
#ifdef _DEATH_CONTENDAB
	{
		...
		if ((menum = CHAR_getInt(charaindex, CHAR_PKLISTTEAMNUM)) < 0)
			return;
		...
		if ((battlearray = PKLIST_GetPKProcedureArray(menum)) < 0)
			return;

		for (j = 0; j < 2; j++) {
			if (PKProcedure[battlearray].Team[j].use == 0)
				continue;
			if (PKProcedure[battlearray].Team[j].teamnum != menum)
				continue;

			if (PKProcedure[battlearray].type != PKTYPE_STANDBY)
				return;
			if (PKProcedure[battlearray].time + 1 >= (int)time(NULL))
				return;
			side = (j + 1) % 2;

			if (PKProcedure[battlearray].Team[side].use == 0)
				return;
			if ((tonum = PKProcedure[battlearray].Team[side].teamnum) < 0)
				return;
		...
		if ((aB1 = ABATTLE_getInBattle(menum)) == NULL)
			return;
		if ((aBo = aB1->top) == NULL)
			return;
		if ((aB1 = aBo->next[0]) == NULL || (aB2 = aBo->next[1]) == NULL)
			return;
		...
		{ // 广播胜负
			...
			int playernum = CHAR_getPlayerMaxNum();
			for (i = 0; i < playernum; i++) {
				if (!CHAR_getCharUse(i) || CHAR_getInt(i, CHAR_WHICHTYPE) != CHAR_TYPEPLAYER)
					continue;
				CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
...
int NPC_PKLIST_Finish_Exit(int menum, int tonum, int winside, int battlemap) {
...
#ifdef _DEATH_CONTENDAB
	{
		...
		if ((aB1 = ABATTLE_getInBattle(menum)) == NULL)
			return 0;
		if ((aB2 = ABATTLE_getInBattle(tonum)) == NULL)
			return 0;
...
BOOL PKLIST_GetMyPKListTeamData(int teamnum, char *data, int sizes) {
	int ti, k;
	char buf[256];
	if ((ti = PKLIST_GetPkTeamListArrayFromNum(teamnum)) == -1)
		return FALSE;
	if (PkTeamList[ti].use == 0)
		return FALSE;
	if (PkTeamList[ti].MyTeamMans[0].use == 0)
		return FALSE;
	...
	for (k = 1; k < MAXTEAMMANNUM; k++) {
		if (PkTeamList[ti].MyTeamMans[k].use == 0)
			continue;
...
BOOL PKLIST_CheckPKReapetTeam(int menum, int tonum) {
	...
	for (k = 0; k < MAXBATTLENUM; k++) {
		if (PkTeamList[meti].BHistory[k].use == 0)
			continue;
	...
	for (k = 0; k < MAXBATTLENUM; k++) {
		if (PkTeamList[toti].BHistory[k].use == 0)
			continue;
...
int PKLIST_GetPKProcedureArray(int menum) {
	...
	for (i = 0; i < MAXJOINTEAM; i++) {
		for (j = 0; j < 2; j++) {
			if (PKProcedure[i].Team[j].use == 0)
				continue;
			if (PKProcedure[i].Team[j].teamnum == menum)
				return i;
		}
	}
	return -1;
...
void ABATTLE_InitABattle(int maxnums) {
	int i, j, k;
	if (maxnums < 1 || maxnums >= MAXBAFLOOR)
		return;
...
void ABATTLE_ShowBattlefromFl(int ti, int fl) {
	...
	if (fl < 0)
		return;
	if (ti < 0 || ti >= MAXBAHEAD)
		return;
	print("headABattle:%x\n", headABattle);
	ABATTLE_ShowNet(headABattle[ti], fl);
	print("\nNOWBATTLE:\n");
	if (fl < 0 || fl >= MAXBAFLOOR)
		return;
...
BOOL ABATTLE_InsertBattle(ArrangeBattleC *aB) {
	...
	for (i = 0; i < MAXBATTLEPAGE; i++) {
		if (InBattle[i] != NULL)
			continue;
		if ((aBo = aB->next[0]) == NULL || aBo->use == 0 || aBo->type > 0)
			return FALSE;
		if ((aBo = aB->next[1]) == NULL || aBo->use == 0 || aBo->type > 0)
			return FALSE;
		InBattle[i] = aB;
...
int ABATTLE_FindBattlefromFl(int ti, int fl) {
	...
	for (i = 0; i < MAXNOWBATTLE; i++) {
		if (NowABattle[ti][fl][i] == NULL)
			continue;
		aBo = NowABattle[ti][fl][i];

		for (j = 0; j < 2; j++) {
			if ((aB1 = aBo->next[j]) == NULL || aB1->use == 0 || aB1->type > 0)
				continue;
...
int ABATTLE_CheckBattlefromFl_sub(int charindex, int ti, int fl, int battleindex) {
	...
	for (i = 0; i < MAXNOWBATTLE; i++) {
		if ((aBo = NowABattle[ti][fl][i]) == NULL)
			continue;
		aB1 = aBo->next[0];
		aB2 = aBo->next[1];
...
BOOL ABATTLE_CheckInABattle(int ti) {
	...
	if (ti < 0 || ti > MAXBATTLEPAGE)
		return FALSE;
	if ((aB = InBattle[ti]) == NULL)
		return FALSE;
...
void ABATTLE_MakeInABattleString(void) {
	...
	for (i = 0; i < MAXBATTLEPAGE; i++) {
		if ((aB = InBattle[i]) == NULL)
			continue;
		if ((aB1 = aB->next[0]) == NULL || aB1->use == 0)
			continue;
		if ((aB2 = aB->next[1]) == NULL || aB2->use == 0)
			continue;
...
BOOL PKLIST_GetABattlelistDataString(int ti, int *tindex, int *stimes,
	...
	case 5: {
		...
		if (getStringFromIndexWithDelim(data, "|", 1, buf, sizeof(buf)) == FALSE)
			return FALSE;
		*tindex = atoi(buf);
		if (getStringFromIndexWithDelim(data, "|", 2, buff, sizeof(buff)) == FALSE)
			return FALSE;
		if (getStringFromIndexWithDelim(data, "|", 3, buf, sizeof(buf)) == FALSE)
			return FALSE;
		sprintf(buf1, "［%s］%s ", buff, buf);
		if (getStringFromIndexWithDelim(data, "|", 4, buff, sizeof(buff)) == FALSE)
			return FALSE;
		if (getStringFromIndexWithDelim(data, "|", 5, buf, sizeof(buf)) == FALSE)
			return FALSE;
...
void ABATTLE_RecordBattle(int ti, char *buf1, char *tstr1, char *buf2, char *tstr2) { // ABLevelup
	char strlog[512];
	FILE *fp = NULL;

	if ((fp = fopen("battle.rec", "a+")) == NULL)
		return;
...
void ABATTLE_GetRecordBattle(void) { // ABLevelup
	char data[512];
	FILE *fp = NULL;
	memset(ABLevelup, 0, sizeof(ABLevelup));

	if ((fp = fopen("battle.rec", "r")) == NULL)
		return;
	while (fgets(data, sizeof(data) - 1, fp)) {
		if (strlen(data) <= 0)
			continue;
		remove_r(data);
		if (strlen(ABLevelup) > 0)
			strcat(ABLevelup, " ");
...
int PKLIST_SetOneBHistory(int ti, int hi, int use, int teamnum, int flg) {
	if (ti < 0 || ti >= maxteam)
		return -1;
	if (hi < 0 || hi >= MAXBATTLENUM)
		return -1;
	if (PkTeamList[ti].BHistory[hi].use == use)
		return -1;
