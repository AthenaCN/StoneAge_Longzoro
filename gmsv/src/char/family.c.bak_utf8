...
int CheckLeaderQ(int meindex) {
	if (CHAR_getInt(meindex, CHAR_LV) < FMLEADERLV && CHAR_getInt(meindex, CHAR_TRANSMIGRATION) <= 0)
		return -1;
...
void FAMILY_Add(int fd, int meindex, char *message) {
	...
	if (CHAR_getWorkInt(meindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE)
		return;
	if (CHAR_getInt(meindex, CHAR_FMINDEX) >= 0 && strcmp(CHAR_getChar(meindex, CHAR_FMNAME), "") != 0) {
...
void FAMILY_Join(int fd, int meindex, char *message) {
	...
	if ((CHAR_getWorkInt(meindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE) || (CHAR_getWorkInt(meindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE))
		return;
...
void FAMILY_Leave(int fd, int meindex, char *message) {
	...
	if ((CHAR_getInt(meindex, CHAR_FMINDEX) == -1) || (strcmp(CHAR_getChar(meindex, CHAR_FMNAME), "") == 0)
#ifdef _FMVER21
...
void FAMILY_Detail(int fd, int meindex, char *message) {
	...
	// shan begin
		...
		for (i = 0 + 1; i < FAMILY_MAXHOME + 1; i++) {
			...
			sprintf(fmname, "%s", CHAR_getChar(meindex, CHAR_FMNAME));
			index = CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI);
			if (fmpks[fmpks_pos + 1].flag == FMPKS_FLAG_MANOR_BATTLEBEGIN || fmpks[fmpks_pos + 1].flag == FMPKS_FLAG_MANOR_PREPARE || fmpks[fmpks_pos + 1].flag == FMPKS_FLAG_MANOR_OTHERPLANET) {
				if ((fmpks[fmpks_pos + 1].host_index == index && strcmp(fmname, fmpks[fmpks_pos + 1].host_name) == 0) || (fmpks[fmpks_pos + 1].guest_index == index && strcmp(fmname, fmpks[fmpks_pos + 1].guest_name) == 0)) {
					struct tm tm1;
...
void FAMILY_CheckMember(int fd, int meindex, char *message) {
	...
	if ((index == -1) || (strcmp(fmname, "") == 0)
		...
		|| ((CHAR_getInt(meindex, CHAR_FMLEADERFLAG) != FMMEMBER_LEADER) && (CHAR_getInt(meindex, CHAR_FMLEADERFLAG) != FMMEMBER_ELDER)))
#else
		|| (CHAR_getInt(meindex, CHAR_FMLEADERFLAG) != 1))
#endif
	{
	...
	if ((strcmp(charname, CHAR_getChar(meindex, CHAR_NAME)) == 0) && (charindex == CHAR_getWorkInt(meindex, CHAR_WORKFMINDEXI)))
		return;
...
#endif
	{
		for (i = 0 + 1; i < FAMILY_MAXHOME + 1; i++) {
			...
			if ((fmpks[fmpks_pos].host_index == index && strcmp(fmname, fmpks[fmpks_pos].host_name) == 0) || (fmpks[fmpks_pos].guest_index == index && strcmp(fmname, fmpks[fmpks_pos].guest_name) == 0)) {
				lssproto_WN_send(fd, WINDOW_MESSAGETYPE_MESSAGE,
								 WINDOW_BUTTONTYPE_OK,
								 -1, -1,
...
void FAMILY_Bank(int fd, int meindex, char *message) {
	...
	if (strcmp(token, "G") == 0) {
		...
		if (((cash - toBank) >= 0) && ((cash - toBank) <= MaxGold) && ((bank + toBank) >= 0) && ((bank + toBank) <= CHAR_MAXBANKGOLDHAVE)) {
			// shan add
			...
			LogStone(
				-1,
				CHAR_getChar(meindex, CHAR_NAME),  /* キャラクタ  (角色)   */
				CHAR_getChar(meindex, CHAR_CDKEY), /* アカウントID  (账号ID) */
				-toBank,						   /* マナ量  (石币数量) */
				CHAR_getInt(meindex, CHAR_GOLD),
				"myBank(家族个人银行)",
				CHAR_getInt(meindex, CHAR_FLOOR),
...
void FAMILY_SetPoint(int fd, int meindex, char *message) {
	...
	for (i = 1; i <= MANORNUM; i++) {
		if (((strcmp(fmpks[i * MAX_SCHEDULE + 1].guest_name,
			...
			((fmpks[i * MAX_SCHEDULE + 1].flag = FMPKS_FLAG_MANOR_BATTLEBEGIN) || (fmpks[i * MAX_SCHEDULE + 1].flag == FMPKS_FLAG_MANOR_PREPARE))) {
			char tmpbuf[256];
			sprintf(tmpbuf, "\n你跟%s已经有预约庄园争夺赛了～\n无法再申请庄园了喔！",
	...
	if (strcmp(token, "P") == 0) {
		...
		if (atoi(token2) != -1) {
			...
			if (CHAR_getInt(petindex, CHAR_TRANSMIGRATION) > 2)
				return;
#endif
			leaderimageNo = 100700 + ((CHAR_getInt(meindex, CHAR_BASEBASEIMAGENUMBER) - 100000) / 20) * 10 + CHAR_getInt(meindex, CHAR_FMSPRITE) * 5;
#ifndef _NEW_RIDEPETS
...
#ifndef _NEW_RIDEPETS
				if ((leaderimageNo == ridePetTable[i].charNo) && (CHAR_getInt(petindex, CHAR_BASEBASEIMAGENUMBER) == ridePetTable[i].petNo)
#ifdef _EVERYONE_RIDE
					&& big4fm != 0 && CHAR_getInt(meindex, CHAR_FMLEADERFLAG) != FMMEMBER_NONE && CHAR_getInt(meindex, CHAR_FMLEADERFLAG) != FMMEMBER_APPLY) {
#else
					&& CHAR_getInt(meindex, CHAR_FMLEADERFLAG) == FMMEMBER_LEADER) {
#endif
					rideGraNo = ridePetTable[i].rideNo;
