 ...
 */
/*------------------------------------------------------------------------
 * ペットを１つ割り当てるデバッグ用［  (分配一只宠物(调试用))
 * CHAR作成にのみ反映される［  (仅反映到CHAR创建)
 *   戻り値 反映されたキャラindex  失敗  -1  (返回值/已反映角色索引/失败-1)
 *-----------------------------------------------------------------------*/
int PET_DEBUG_initPetOne(int charaindex) {
	Char ch;
	int havepetindex;
	int index;
	/* ペットを付けるか調べる  (检查是否分配宠物) */
 ...
 *     手放すペットを取る  (取要放生的宠物)
 * 戻り値  (返回值)
 *  itemindex       int         アイテムインデックス  (物品索引)
 *  floor           int         フロアID  (楼层ID)
 *  x               int         x座標  (x坐标)
 *  y               int         y座標  (y坐标)
 *  net             BOOL        ネットワークのことをするかどうか  (是否执行网络操作)
 * 戻り値  (返回值)
 *  成功      objindex  (成功)
 *  失敗      -1  (失败)
 ------------------------------------------------------------*/
int PET_dropPetAbsolute(int petindex, int floor, int x, int y, BOOL net) {
	...
	/* ネットワークフラグが立っているときはする  (网络旗标已置时执行)  by ringo*/
	if (net)
...
int PET_initCharOneArray(Char *ch) {
	...
	if (ch->data[CHAR_FUSIONBEIT] == 1 &&
		...
		ch->functable[CHAR_LOOPFUNCTEMP1] = getFunctionPointerFromName("PET_CheckIncubateLoop");
...
int PET_createPetFromCharaIndex(int charaindex, int enemyindex) {
	...
	if (!CHAR_getDefaultChar(&CharNew, 31010))
		return -1;
	CharNew.data[CHAR_BASEBASEIMAGENUMBER] = CharNew.data[CHAR_BASEIMAGENUMBER] = CHAR_getInt(enemyindex, CHAR_BASEIMAGENUMBER);
...
BOOL PET_SelectBattleEntryPet(int charaindex, int petarray) {
	int pindex;
	/* 戦闘は重いラグでこれに時間がかかる可能性あり  (战斗因高延迟可能耗时)  */
	if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE)
		return FALSE;
	/* -1の場所は -1にしてセットして終わり  (-1位置设为-1后结束)［*/
	if (petarray == -1) {
		CHAR_setInt(charaindex, CHAR_DEFAULTPET, -1);
		return TRUE;
	}
