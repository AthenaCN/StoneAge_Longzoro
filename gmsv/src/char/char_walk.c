...
static void CHAR_sendMapAtWalk(int index, int fl, int ox, int oy,
							   int fx, int fy) {
	/*
	 *  現在の数と前までの数の差を取る  (取当前位置与之前位置的差)
	 *  こうするとワープNPCも簡単だ  (这样传送NPC也简单)
	 */
	int vx, vy;
	...
	if (ABS(vx) >= seesiz / 2 || /*  差が大きい  (差值过大)  */
		...
		int absy = ABS(vy);
		/*
		 * ひとつずつにスタートを取るのと vx , vy から1ひくのがポイント  (逐个取起点，从vx/vy减1是关键)
		 * 前とかさならないように  (避免与之前重叠)
		 * clipRect は w,h 0 をみとっている  (clipRect允许w/h为0)
		 */
...
void CHAR_walkcall(int index) {
	...
	/* 親なら前に場所を記憶しておく  (若是队长则记住之前位置)
	 * 親の歩く前の場所に移動する為  (为了移动到队长移动前的位置)
	 */
