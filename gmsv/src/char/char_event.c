...
static struct ParamShow {
	int paramindex;	  /*  パラメータのインターフェース  (参数索引)    */
	char *offmessage; /*    送信時のメッセージの値  (发送时的消息值)      */
} pShow[] = {
...
static BOOL CHAR_clairvoyance_player(int objindex, char *buf) {
	if (OBJECT_getType(objindex) == OBJTYPE_CHARA && CHAR_getInt(OBJECT_getIndex(objindex), CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
		*buf = 'P';
		return TRUE;
	}
	return FALSE;
...
static BOOL CHAR_clairvoyance_enemy(int objindex, char *buf) {
	if (OBJECT_getType(objindex) == OBJTYPE_CHARA && CHAR_getInt(OBJECT_getIndex(objindex), CHAR_WHICHTYPE) == CHAR_TYPEENEMY) {
		*buf = 'E';
		return TRUE;
	}
	return FALSE;
		...
		for (j = centery - CHAR_CLAIRVOYANCEWIDTH / 2;
			...
			for (object = MAP_getTopObj(fl, i, j); object;
				...
				if (OBJECT_getType(objindex) == OBJTYPE_NOUSE)
					continue;
				if (OBJECT_getType(objindex) == OBJTYPE_CHARA && OBJECT_getIndex(objindex) == charaindex)
					continue;
