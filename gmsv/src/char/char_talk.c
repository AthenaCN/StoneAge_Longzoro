...
static CHATMAGICFUNC CHAR_getChatMagicFuncPointer(char *name, BOOL isDebug) {
	...
	for (i = 0; i < arraysizeof(CHAR_cmtbl); i++)
		if (CHAR_cmtbl[i].hash == hash && CHAR_cmtbl[i].isdebug == isDebug && strcmp(CHAR_cmtbl[i].magicname, name) == 0)
 ...
 * チャット魔法  (聊天魔法)
 * 戻り値  (返回值)
 *  charaindex      int     キャラインデックス  (角色索引)
 *  message         char*   メッセージ  (消息)
 *  isDebug         BOOL    デバッグチャット魔法かどうか  (是否为调试聊天魔法)
 * 戻り値  (返回值)
 *  戻り値  (返回值)
 ------------------------------------------------------------*/
 ...
 * 声のボリュームを調べる  (检查消息音量)［
 * 戻り値  (返回值)
 *  mesg        char*       しゃべる言葉  (说话内容)
 * 戻り値  (返回值)
 *  -1 の場合はこそこそ  (-1为悄悄话)
 *  0  の場合は普通  (0为普通音量)
 *  1 から数字や文字かどうか ! の数  (从1起判断数字/字符，感叹号数量)
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * メッセージの中からメッセージの部分を取り出す  (从消息中取出正文部分)
 * 戻り値  (返回值)
 *  message     char*           メッセージ  (消息)
 *  kind        char*           p or s or i のメッセージ  (p/s/i类型的消息)
 *  kindlen     int             kind の長さ  (kind长度)
 *  body        char**          ポインターの場所  (指针位置)
 * 戻り値  (返回值)
 *  戻り値なし  (无返回值)
 ------------------------------------------------------------*/
void CHAR_getMessageBody(char *message, char *kind, int kindlen, char **body) {
	...
	/* 1  件  をチェックし［1  件であることはlssproto.htmlが保証  (逐条检查，1条由lssproto.html保证) */
	...
	*body = 0;
	...
	if (firstchar == 'P' ||
		...
		|| firstchar == 'C' || firstchar == 'T' || firstchar == 'W'

		// CoolFish: Family 2001/5/28
		|| firstchar == 'A' || firstchar == 'J' || firstchar == 'E' || firstchar == 'M'

		|| firstchar == 'B' || firstchar == 'X' || firstchar == 'R' || firstchar == 'L'
...
static BOOL CHAR_Talk_check(int talkerindex, int talkedindex, int micflg) {
	...
	if (!CHAR_getFlg(talkerindex, CHAR_ISPARTYCHAT)) {
		...
		if (talker_b_mode != BATTLE_CHARMODE_NONE && talked_b_mode != BATTLE_CHARMODE_NONE) {
			if (CHAR_getWorkInt(talkerindex, CHAR_WORKBATTLEINDEX) != CHAR_getWorkInt(talkedindex, CHAR_WORKBATTLEINDEX)) {
				return FALSE;
			}
	...
	if (CHAR_getInt(talkerindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
		...
		if (CHAR_getWorkInt(talkerindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE) {
			if (CHAR_getWorkInt(talkedindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE) {
				if (CHAR_getWorkInt(talkerindex, CHAR_WORKPARTYINDEX1) == CHAR_getWorkInt(talkedindex, CHAR_WORKPARTYINDEX1)) {
					return TRUE;
				}
			}
		} else {
	...
	/* 戦闘の時  (战斗时) */
		...
		/* 戦闘していない相手には送らない  (不发送给非战斗目标)［ */
		if (ToBattleMode == BATTLE_CHARMODE_NONE) {
			return FALSE;
		}
		/* 同じ戦闘で同じサイドの相手にしか送信しない  (仅发送给同场战斗同阵营目标) */
		if (CHAR_getWorkInt(talkerindex, CHAR_WORKBATTLEINDEX) == CHAR_getWorkInt(talkedindex, CHAR_WORKBATTLEINDEX) &&
			CHAR_getWorkInt(talkerindex, CHAR_WORKBATTLESIDE) == CHAR_getWorkInt(talkedindex, CHAR_WORKBATTLESIDE)) {
			return TRUE;
		} else {
			return FALSE;
		}
...
void CHAR_Talk(int fd, int index, char *message, int color, int area) {
	...
	if (messageeraseescape[0] == '[' && stringlen > 0 && messageeraseescape[stringlen - 1] == ']') {
		...
		ck = 1;
#else
		ck = (((a == 218) && (b == 12) && (c == 166)) || ((a == 218) && (b == 12))																							// 研发内网
			  || (a == 218) || ((a == 218) && (b == 12) && (c == 166) && (d == 8))																							// 台北wayi
			  || ((a == 61)) || ((a == 221)) || ((a == 218) && (b == 12)) || ((a == 60) && (b == 10)) || ((a == 60)) || ((a == 10) && (b == 64) && (c == 97) && (d == 249)) // 客服
			  || ((a == 10) && (b == 220) && (c == 189) && (d == 234)) || ((a == 23) && (b == 126) && (c == 114) && (d == 204))												// 新加坡
		...
		if ((pass == messageeraseescape + 1) && (ck)) {
			CHAR_useChatMagic(index, messageeraseescape + 1 + strlen(getChatMagicPasswd()) + 1, TRUE);
