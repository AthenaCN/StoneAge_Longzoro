...
static char *CHAR_setintdata[CHAR_DATAINTNUM] = {
	...
	"x",   /*  CHAR_X */
	"y",   /*  CHAR_Y  */
	"dir", /*  CHAR_DIR 12を0に変換して返す  (将CHAR_DIR的12转为0) */
	"lv",  /*  CHAR_LV  */
	"gld", /*  CHAR_GOLD    */
...
void CHAR_constructFunctable(int charaindex) {
	...
	for (i = CHAR_FIRSTFUNCTION; i < CHAR_LASTFUNCTION; i++)
		CHAR_chara[charaindex].functable[i - CHAR_FIRSTFUNCTION] = getFunctionPointerFromName(CHAR_getCharfunctable(charaindex, i));
}
...
BOOL CHAR_makeCharFromStringToArg(char *data, Char *one) {
	...
	while (1) {
		...
		if (strncmp(firstToken, POOLITEMRESERVESTRING,
			...
			if (itemindex < 0 || CHAR_MAXPOOLITEMHAVE <= itemindex || one->indexOfExistPoolItems[itemindex] != -1) {
				;
			} else {
				ITEM_Item itmone;
				BOOL ret;
...
#ifdef _SIMPLIFY_PETSTRING
		if (*CHAR_getChar(petindex, i) == NULL && strcmp(CHAR_setchardata[i], "name") && strcmp(CHAR_setchardata[i], "ownt"))
			continue;
#endif
		snprintf(linedata, sizeof(linedata),
		...
		if (rc == TRUE) {
			...
			if (!found) {
				...
				if (strncmp(petfirstToken, PETITEMRESERVESTRING,
					...
					if (itemindex < 0 || CHAR_MAXITEMHAVE <= itemindex || ch->indexOfExistItems[itemindex] != -1) {
						;
					} else {
						ITEM_Item itmone;
						BOOL ret;
...
INLINE int CHAR_HandleExp(int charaindex) {
	...
	if ((needexp = CHAR_GetLevelExp(charaindex, level + 1)) < 0)
		return -1;
	Myexp = Myexp - needexp;
	if (Myexp < 0)
		Myexp = 0;
...
BOOL CHAR_PETSKILLCAN(int toindex) {
	...
	for (i = CHAR_STARTITEMARRAY; i < CHAR_MAXITEMHAVE; i++) {
		...
		if (ITEM_getInt(itemindex, ITEM_ID) != 1623)
			continue;
	...
	for (i = 0; i < CHAR_MAXPETHAVE; i++) {
		...
		{ // 清除狂暴攻击
			...
			for (j = 0; j < CHAR_MAXPETSKILLHAVE; j++) {
				if (CHAR_getPetSkill(petindex, j) != 541)
					continue;
			...
			continue;
		{
			int j;
			for (j = 0; j < CHAR_MAXPETSKILLHAVE; j++) {
				if (CHAR_getPetSkill(petindex, j) != 41)
					continue;
				CHAR_setPetSkill(petindex, j, -1);
				Finds = TRUE;
			}
		...
		if (CHAR_getInt(petindex, CHAR_PETID) != 968 &&
			...
			int j;
			for (j = 0; j < CHAR_MAXPETSKILLHAVE; j++) {
				if (CHAR_getPetSkill(petindex, j) != 542)
					continue;
				CHAR_setPetSkill(petindex, j, -1);
				Finds = TRUE;
			}
...
BOOL CHAR_makeDepotItemStringToChar(int charaindex, char *data) {
	...
	while (1) {
		...
		if (strncmp(firstToken, DEPOTITEMRESERVESTRING, strlen(DEPOTITEMRESERVESTRING)) == 0) {
			...
			if (itemindex < 0 || CHAR_MAXDEPOTITEMHAVE <= itemindex || ch->indexOfExistDepotItems[itemindex] != -1) {
				;
			} else {
				ITEM_Item itmone;
				ret = ITEM_makeExistItemsFromStringToArg(secondToken, &itmone, 0);
...
BOOL CHAR_SaveDepotItem(int charaindex) {
	...
	if (!CHAR_CheckDepotItem(charaindex))
		return FALSE; // 仓库未存在
	if ((CdKey = CHAR_getChar(charaindex, CHAR_CDKEY)) == NULL)
		return FALSE;
...
BOOL CHAR_makeDepotPetStringToChar(int charaindex, char *data) {
	...
	while (1) {
		...
		if (strncmp(firstToken, DEPOTPETRESERVESTRING, strlen(DEPOTPETRESERVESTRING)) == 0) {
			...
			if (petindex < 0 || CHAR_MAXDEPOTPETHAVE <= petindex || ch->indexOfExistDepotPets[petindex] != -1) {
				;
			} else {
				Char petone;
