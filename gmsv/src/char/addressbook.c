...
/*------------------------------------------------------------
 * アドレスブックのメッセージを送信する  (发送通讯录消息)
 * MSGプロトコルから使われる  (由 MSG 协议调用)
 *
 * やることは connection と cdkey で検索して、キャラも
 * ヒットしたら MSG_send する。その時に、相手のリストに
 * ないなら何もしない。
 * 引数
 *  cindex  int     キャラクタのindex  (角色索引)
 *  aindex  int     アドレスブックのindex  (通讯录索引)
 *  text    char*   送信する値  (发送内容)
 *  color   int     色  (颜色)
 * 戻り値
 * オンラインキャラにメッセージを送信したらTRUE,
 * オフラインならFALSEを返す
 ------------------------------------------------------------*/
BOOL ADDRESSBOOK_sendMessage(int cindex, int aindex, char *text, int color) {
	...
	/* サーバにいるプレイヤーを探す  (查找在线玩家) */
	for (i = 0; i < playernum; i++) {
		if (CHAR_CHECKINDEX(i) &&
			strcmp(CHAR_getChar(i, CHAR_CDKEY), ae->cdkey) == 0 &&
			strcmp(CHAR_getChar(i, CHAR_NAME), ae->charname) == 0) {
			/*
			 * CDKEY と キャラクタ名を検索して、そのキャラが  (按 CDKEY 和角色名查找)
			 * アドレスブックに登録があれば調べて、  (在通讯录中则查找)
			 * 一致したら MSG する  (匹配则发送 MSG)
			 */
...
/*------------------------------------------------------------
 * アドレスブックのメッセージを送信する  (发送通讯录消息)
 * saac と msg を受け取ってインターフェースにポストする  (接收 saac/msg 并投递)
 *
 * 戻り値
 ------------------------------------------------------------*/
...
BOOL ADDRESSBOOK_addEntry(int meindex) {
	...
	if (cnt == 1) {
		...
		for (i = 0;
			 CONNECT_getTradecardcharaindex(fd, i) != -1 && i < CONNECT_WINDOWBUFSIZE; i++) {
...
/*------------------------------------------------------------
 * ひとつのアドレスブックエントリを値に起こす  (将单个通讯录条目转成字符串)
 * これはキャラクタがいない時にインターフェースに送信するように確認する  (不存在时经接口发送)
 * ない時は空を返す  (不存在时返回空)
 * 引数
 *  a   ADDRESSBOOK_entry*    値を入れる構造体へのポインタ  (存放值的结构体指针)
 * 戻り値
 *  char *
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * アドレスブックエントリを構造に起こす  (将通讯录条目转为结构)
 * この構造のuse部分の結果は確認しない  (不检查此结构的use字段)
 * 引数
 *  in      char*                     値  (值)
 *  a       ADDRESSBOOK_entry*      データを収る所  (存放数据的结构)
 * 戻り値
 * つねにTRUE  (始终返回TRUE)
 ------------------------------------------------------------*/
