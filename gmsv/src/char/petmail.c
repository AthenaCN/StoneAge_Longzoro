...
static int PETMAIL_getIdleTime(int index) {
#define PETMAIL_DIVRANGE 25		  //	分割する範囲  (分割范围)
#define PETMAIL_IDLELEVELRANGE 10 //	分割する区間  (分割区间)
#define PETMAIL_IDLEUNITTIME 3	  //	単位  (单位)
	int d;
	...
	if (d < 0)
		d = 0;
	if (d > PETMAIL_IDLELEVELRANGE)
		d = PETMAIL_IDLELEVELRANGE;
...
static void PETMAIL_sendPetmail(int index, int tocharaindex) {
	...
	// Syu Add 06/16
		...
		if (fd != -1)
			lssproto_MSG_send(fd, index_to_my_info,
