...
/*------------------------------------------------------------------------
 * ENEMY_enemyのデータファイル 読み直し  (重读ENEMY_enemy数据文件)
 *-----------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------
 ENEMY_Enemyの検索を知る  (获取ENEMY_Enemy检索编号)
 *-----------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------
 * ENEMY_ID からENEMY_Enemyの検索を知る  (从ENEMY_ID获取ENEMY_Enemy检索编号)
 *-----------------------------------------------------------------------*/
...
BOOL GROUP_initGroup(char *filename) {
	...
	while (fgets(line, sizeof(line), f)) {
		...
		}
		{
			...
			/* もしも２度目のループに足した数のために初期化しておく  (为第二遍循环做初始化) */
			for (j = 0; j < GROUP_DATAINTNUM; j++) {
...

			{
				...
				for (loop = ENEMY_ID1; loop < CREATEPROB1; loop++) {
					if (GROUP_getInt(group_readlen, loop) != -1) {
						for (i = 0; i < ENEMY_enemynum; i++) {
							if (ENEMY_getInt(i, ENEMY_ID) == GROUP_getInt(group_readlen, loop)) {
								break;
							}
						}
						if (i == ENEMY_enemynum) {
...
/*------------------------------------------------------------------------
 * GROUP_ID からGROUP_Groupの検索を知る  (从GROUP_ID获取GROUP_Group检索编号)
 *-----------------------------------------------------------------------*/
...
static int ENEMY_getExp(int array, int tarray, int level, int rank) {
	...
	alpha = (*(tp + E_T_CRITICAL) + *(tp + E_T_COUNTER) + *(tp + E_T_GET) + *(tp + E_T_POISON) + *(tp + E_T_PARALYSIS) + *(tp + E_T_SLEEP) + *(tp + E_T_STONE) + *(tp + E_T_DRUNK) + *(tp + E_T_CONFUSION)) / 100.0 + *(tp + E_T_RARE);
	/* EXP「基本  EXP≈{  ランク+α  ＊レベル  (EXP=基本+[等级+α]×等级) */
...
int ENEMY_getRank(int array, int tarray) {
	...
	tp = ENEMYTEMP_enemy[tarray].intdata;
	/* ランクを計算する  (计算等级) */
	/* このの仕様は仕様書を参照の事  (此规格详见规格书) */
	paramsum = *(tp + E_T_BASEVITAL) +
			   *(tp + E_T_BASESTR) +
			   *(tp + E_T_BASETGH) +
			   *(tp + E_T_BASEDEX);

	ranknum = 0; // デフォルトは０  (默认为0)
...
static int EnemyGymSkill[] = {
	PETSKILL_GUARDBREAK,		  // ３"ガード崩し  (破防)
	PETSKILL_CONTINUATIONATTACK1, // １０"連撃  (连击)
	PETSKILL_CONTINUATIONATTACK2, // １１"連撃  (连击)
	PETSKILL_CONTINUATIONATTACK3, // １２"連撃  (连击)
	PETSKILL_CHARGE1,			  // ３０"チャージ１  (蓄力1)
	PETSKILL_CHARGE2,			  // ３１"チャージ２  (蓄力2)
	PETSKILL_MIGHTY1,			  // ４０"一撃必殺  (一击必杀)
	PETSKILL_MIGHTY2,			  // ４１"一撃必殺  (一击必杀)
	PETSKILL_POWERBALANCE1,		  // ５０"強化１  (强化1)
	PETSKILL_POWERBALANCE2,		  // ５１"強化２  (强化2)
	PETSKILL_POWERBALANCE3,		  // ５２"強化３  (强化3)
	PETSKILL_POISON_ATTACK1,	  // ６０"毒攻撃  (毒攻击)
	PETSKILL_POISON_ATTACK2,	  // ６１"毒攻撃  (毒攻击)
	PETSKILL_STONE,				  // ８０"石化攻撃  (石化攻击)
	PETSKILL_CONFUSION_ATTACK,	  // ９０"混乱攻撃  (混乱攻击)
	PETSKILL_DRUNK_ATTACK,		  // １００"酔い攻撃  (醉酒攻击)
	PETSKILL_SLEEP_ATTACK,		  // １１０"眠り攻撃  (睡眠攻击)
	PETSKILL_NOGUARD1,			  // １５０"ノーガード１  (无防御1)
	PETSKILL_NOGUARD2,			  // １５１"ノーガード２  (无防御2)
	PETSKILL_NOGUARD3,			  // １５２"ノーガード３  (无防御3)
...
/*------------------------------------------------------------------------
 * ENEMYにランダムなスキルを与える  (给ENEMY随机技能)［
 *-----------------------------------------------------------------------*/
int ENEMY_RandomChange(int enemyindex, int tempno) {
	int work, work2, iRet = 0;

	// ランダムキャラかどうかチェック  (检查是否为随机角色)
	if ((564 <= tempno && tempno <= 580) || (739 <= tempno && tempno <= 750) || (895 <= tempno && tempno <= 906)) {
		//********************************************
		// プレイヤーの場合  (玩家的情况)
		//********************************************
		iRet = 1;
		...
		// ランダムキャラかどうかチェック  (检查是否为随机角色)
		if ((655 <= tempno && tempno <= 720) || (859 <= tempno && tempno <= 894) || (907 <= tempno && tempno <= 940)) {
			//********************************************
			// ペットの場合  (宠物的情况)
			//********************************************
			iRet = 2;
	...
	if (iRet == 1) {
		//********************************************
		//
		// プレイヤーの出した敵なのでスキルもランダム  (玩家出的敌人技能也随机)
		//
		//********************************************
		// スキルもランダム  (技能也随机)
...
/*------------------------------------------------------------------------
 * ENEMY_enemyからキャラクターを生成する  (从ENEMY_enemy生成角色)［
 *-----------------------------------------------------------------------*/
int ENEMY_createEnemy(int array, int baselevel) {
	...
	/*    番号    */
	CharNew.data[CHAR_BASEBASEIMAGENUMBER] = CharNew.data[CHAR_BASEIMAGENUMBER] = *(tp + E_T_IMGNUMBER);
	/*  敵であると設定する  (设为敌人)   */
	...
	/* レベルを決定する［  (决定等级) */
	if (baselevel > 0) {
		level = baselevel; // レベル固定  (等级固定)
	} else {
		level = RAND((*(p + ENEMY_LV_MIN)), (*(p + ENEMY_LV_MAX)));
	}
#define E_PAR(a) (*(p + (a)))
#define ET_PAR(a) (*(tp + (a)))
/* パラメータ指数から 実際の基本パラメータへの計算式  (从参数指数到实际基础参数的计算式) */
#if 1
...
#endif
	/* 先ほどの割り振りポイントに +2をランダムで足す  (先前分配点数随机加2) */
	tp[E_T_BASEVITAL] += RAND(0, 4) - 2;
	tp[E_T_BASESTR] += RAND(0, 4) - 2;
	tp[E_T_BASETGH] += RAND(0, 4) - 2;
	tp[E_T_BASEDEX] += RAND(0, 4) - 2;
	/* この時点で割り振りポイントを保存しておく  (此时保存分配点数) */
	CharNew.data[CHAR_ALLOCPOINT] = (tp[E_T_BASEVITAL] << 24) + (tp[E_T_BASESTR] << 16) + (tp[E_T_BASETGH] << 8) + (tp[E_T_BASEDEX] << 0);
	/* その他 ランダムでポイントを１づつ加算、これを１０回繰り返す  (其余随机逐点累加，重复10次) */
	for (i = 0; i < 10; i++) {
		int work = RAND(0, 3);
		if (work == 0)
			tp[E_T_BASEVITAL]++;
...
/*------------------------------------------------------------
 * 指定    データの識別    番号が 乱数ならランダムに変更する  (若指定数据识别号为随机则随机变更)
 ------------------------------------------------------------*/
int ENEMY_RandomEnemyArray(int e_array, int *pNew) {
	...
	*pNew = -1;
	// この 番号はランダム  (此编号为随机)
	if (
		(RANDOMENEMY_TOP <= e_array && e_array <= RANDOMENEMY_END) || (964 <= e_array && e_array <= 969)) {
		// どのダミーか調べる  (查找是哪个占位)
...
#define PARAM_CAL(l) ((level - 1) * ET_PAR(E_T_LVUPPOINT) + ET_PAR(E_T_INITNUM))
	...
	work[2] += (RAND(0, 4) - 2);
	work[3] += (RAND(0, 4) - 2);
	LevelUpPoint = (work[0] << 24) + (work[1] << 16) + (work[2] << 8) + (work[3] << 0);
