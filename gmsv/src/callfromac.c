...
/* Tokyo-sa: convert saac text charlist to 9.0 win format.
   saac: "name|opt\\zf1... name2|opt2..." (space-separated)
   9.0:  "idx,group,name,hometown,mapname|..." (pipe-separated) */
static void convert_saac_charlist(char *out, int outlen, const char *saac_data)
{
	char work[65500];
	char *saveptr1, *saveptr2;
	char *role, *name, *opt;
	int idx = 0, group = 0, last_ht = -999;

	out[0] = 0;
	if (!saac_data || !saac_data[0]) return;
	strncpy(work, saac_data, sizeof(work)-1);
	work[sizeof(work)-1] = 0;

	for (role = strtok_r(work, " ", &saveptr1); role;
	     role = strtok_r(NULL, " ", &saveptr1)) {
		char *fields[20]; int nf = 0;
		char *tok; int ht = 0; char mapn[64]=""; char rname[64]="";

		name = strtok_r(role, "|", &saveptr2);
		opt = strtok_r(NULL, "|", &saveptr2);
		if (!name || !opt) continue;
		strncpy(rname, name, sizeof(rname)-1);

		for (tok = strtok(opt, "\\"); tok && nf < 20; tok = strtok(NULL, "\\")) {
			if (tok[0]=='z') tok++;
			fields[nf++] = tok;
		}
		if (nf > 0) ht = atoi(fields[0]);
		if (nf > 15) strncpy(mapn, fields[15], sizeof(mapn)-1);
		if (ht != last_ht) { group = idx; last_ht = ht; }

		if (out[0]) strncat(out, "|", outlen-strlen(out)-1);
		snprintf(out+strlen(out), outlen-strlen(out),
			"%d,%d,%s,%d,%s", idx, group, rname, ht, mapn);
		idx++;
	}
}
...
void saacproto_ACCharList_recv(int fd, char *result, char *data, int retfd)
{
	...
	char converted[65500];

	if (CONNECT_checkfd(clifd) == FALSE) {
		return;
	}
	print("[CHARLIST-RECV] result=%s data_len=%d data=%.100s\n",
	      result ? result : "null", data ? (int)strlen(data) : -1, data ? data : "(null)");

	convert_saac_charlist(converted, sizeof(converted), data);
	print("[CHARLIST-CONVERTED] len=%d data=%s\n", (int)strlen(converted), converted);
	print("[CHARLIST-RAW] len=%d data=%.100s\n", data ? (int)strlen(data) : -1, data ? data : "(null)");

	/* Tokyo-sa: 客户端接受 saac 原始文本角色列表，直接转发；
	 * convert_saac_charlist 为 9.0 二进制格式，保留备用 */
	lssproto_CharList_send(clifd, result, data ? data : "");
...
void saacproto_LoadPkTeamListData_recv(int fd, char *result, char *data, int ti)
{
	...
	if (!strcmp(result, "success")) {
		if (PKLIST_LoadPkTeamListData() == FALSE)
			return;
		if (ti != -1)
...
void saacproto_ACCharSave_recv(int fd, char *result, char *data, int retfd)
{
	...
	/*
	 *   際エラーでもないのにここではじかれる事はある (虽然不是异常，但也有在此被拒绝的情况)
	 * それはいきなり強制的に切られた場合だ (那是连接被突然强制切断的情况)
	 */
	if (CONNECT_checkfd(clifd) == FALSE)
		return;
	...
	case WHILECREATE:
		print("[ACCREATE-DEBUG] result=%s data=%.50s\n", result, data ? data : "(null)");
		if (strcmp(result, FAILED) == 0) {
			lssproto_CreateNewChar_send(clifd, FAILED, data);
		} else {
			/* Tokyo-sa: create char response uses same format as char list */
			char cbuf[65500];
			convert_saac_charlist(cbuf, sizeof(cbuf), data);
			print("[CREATE-CONVERTED] len=%d data=%s\n", (int)strlen(cbuf), cbuf);
			lssproto_CreateNewChar_send(clifd, SUCCESSFUL, cbuf);
		}
		CONNECT_setState(clifd, NOTLOGIN);
		break;

	case WHILELOGOUTSAVE:
		/*   強制のログアウトの強制のセーブの返信 (强制登出时的强制存档的回复) */
		if (strcmp(result, SUCCESSFUL) == 0)
			/* 成功の場合 (成功时)  */
	...
	case WHILELOSTCHARSAVE:
		/*  結果がどうであれ、このキャラは削除する (无论结果如何，都删除这个角色)  */
...
/*
 *  キックの返信。これは解析にしか使わない。現在ほぼ使われない。意味はない。 (踢出操作的回复。这仅用于解析。现在基本已不使用。没有意义。)
 */
void saacproto_ACLock_recv(int fd, char *result, char *data, int retfd)
{
	...
	case WHILECANNOTLOGIN:
		/* ログイン失敗した強制のキックの返信 (登录失败时的强制踢出回复)  */
		if (strcmp(result, SUCCESSFUL) == 0) {
			/* 何もしていない状態にする (将状态设为未执行任何操作) */
			CONNECT_setState(clifd, NOTLOGIN);
			...
			if (strcmp(data, mesg) == 0) {
				/*
				 * キックされていないのでもう何もしない (因未被踢出，不再执行任何操作)
				 * したのと同じ扱いにする (按与上述相同的方式处理)
				 */
				CONNECT_setState(clifd, NOTLOGIN);
			} else {
				/*
				 * 強制キック解析しようとする (尝试进行强制踢出解析)
				 * stateの変化なし (state 无变化)
				 */
...
/* ---------------------------------------------
 * データベース更新受けとり処理 数値 (数据库更新接收处理 数值)
 * --------------------------------------------*/
									  ...
									  char *table, int count_start, int msgid, int msgid2)
{
	...
	/* duelpoint database の処理 (决斗点数据库的处理)  */
	if (strcmp(table, DB_DUELPOINT) == 0) {
...
#endif
/* ---------------------------------------------
 * ブロードキャストメッセージ受けとり (广播消息接收处理)
 * --------------------------------------------*/
void saacproto_Broadcast_recv(int fd, char *id, char *charname, char *message)
{
#ifndef _DEATH_CONTEND
	if (strstr(message, "online") == 0 || strstr(message, "offline") == 0 || strstr(message, "param") == 0 || strstr(message, "chardelete") == 0) {
		char buff[512];
		char escapebuf[1024];
...
void saacproto_ACSHOWMEMBERLIST2_recv(int fd, int charaindex, int fm1, char *fm1_name, int fm2, char *fm2_name, int time, int id)
{
	...
	if (fm1_name == NULL || fm2_name == NULL)
		return;
...
void saacproto_ACRaceRecordandSort_recv(int fd, int charaindex, int racetype, char *data)
{
	...
	if (racetype == 0 || racetype == 9)
		return; // 因为登记动作而接收到ac的回传，所以不用处理
