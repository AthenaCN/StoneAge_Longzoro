...
/*------------------------------------------------------------
 * オブジェクトのメンバーを元に引数にして (基于对象成员作为参数)
 * オブジェクトを登録して ID を得る。 (注册对象并取得ID)
 *
 * 娄醒
 *  type    OBJTYPE     オブジェクトのタイプ
 *  index   int         それぞれの での index
 *  x       int         x座標
 *  y       int         y座標
 *  floor   int         floorid
 * 忒曰袄
 *   int  割り当てられた index, 失敗 の場合は -1
 ------------------------------------------------------------*/
...
int detectStoreFile(char *filename)
{
	...
	if (strncmp(buffer, STOREITEMID, sizeof(STOREITEMID)) == 0 || strncmp(buffer, STOREGOLDID, sizeof(STOREGOLDID)) == 0) {
		fclose(f);
		return 2;
	}
