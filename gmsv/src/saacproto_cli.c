...
91 :      ゲームサーバーがアカウントサーバーにログインするときに呼ばれる。 (游戏服务器登录账户服务器时调用)
92 :      ログインを試みたあと、このログインをしないとすべてのサービスをしない。 (尝试登录后，若不登录则无法使用所有服务)
93 :      <br><br>
94 :      <dl>
95 :        <dt>string servername
96 :        <dd>サーバー  ［acserv.cfに保存されている［ (服务器 [保存在acserv.cf中])
97 :        <dt>string serverpas
98 :        <dd>サーバーパスワード［acserv.cfに保存されている［ (服务器密码 [保存在acserv.cf中])
99 :      </dl>
100 :
*/
...
115 :      サーバーがアカウントサーバーからログアウトするときに呼ばれる。 (服务器从账户服务器注销时调用)
116 :      ログインはサーバによって手で切られる。したがってこれに対するアカウントサーバー
117 :      の返信はコネクションを切るだけであろう。 (其响应只是断开连接而已)
118 :      <br>
119 :      <br>
120 :      <br>
121 :
...
609 : 	 ゲームサーバーに対してこのコマンドを送信すると、 (向游戏服务器发送此命令后)
610 : 	 すべてのゲームサーバーにたいして同一メッセージが送信される。 (向所有游戏服务器发送同一消息)
611 : 	 id , charname はよくつかわれるので、個の引数をもつようにした。 (id、charname 因常用，故设有多个参数)
612 : 	 いちいち  侬  パースのルーチンを書かなくてもいいように。 (免去每次都要写解析例程的麻烦)
613 : 	 <dl>
614 : 	   <dt>string id
615 : 	   <dd>user ID
616 : 	   <dt>string charname
617 : 	   <dd>キャラ  (エスケープなし) (角色 (不转义))
618 : 	   <dt>string message
619 : 	   <dd>  送されるメッセージ［ゲームサーバーが受けに呼ばれる［ (发送的消息 [由游戏服务器接收时调用])
620 : 	 </dl>
*/
...
646 : 	   <dd>メッセージ (消息)
647 : 	   <dt>int option
648 : 	   <dd>オプション［チャットの缙とかいろいろ入れる［今の拡張用 (选项 [聊天等的各种设置 [供将来扩展])
649 : 		   これをどう呼ばれるかはゲームサーバーの手で、アカウントサーバーは
650 : 		   一切この    にタッチしない (完全不插手此项)
651 : 	 </dL>
*/
...
int saacproto_ClientDispatchMessage(int fd, char *line)
{
	...
	355 : 	 このコマンドの結果、失敗しても復元できないので無視してよいぞ (此命令的结果，即使失败也无法复原，可忽略)
	356 : 	 <dl>
	357 : 	   <dt>string result
	358 : 	   <dD>successful / failed (成功或失败)
	359 : 	   <dt>string table
	360 : 	   <dD>
	361 : 	   <dt>int msgid
	362 : 	   <dD>
	...
	*/
	...
	446 : 	 DBUpdateEntryの結果［ (DBUpdateEntry的结果)
	447 : 	 <dl>
	448 : 	   <dt>string result
	449 : 	   <dd>succesful/failed
	450 : 	   <dt>string table
	451 : 	   <dd>  テーブル  をそのままかえす (原样返回表)
	452 : 	   <dt>string key
	453 : 	   <dd>キーをそのままかえす (原样返回键)
	454 : 	   <dt>int msgid, int msgid2
	455 : 	   <dd>msgid をそのまま返す［ (原样返回msgid)
	456 : 	 </dl>
	457 : 	 <BR><BR>
	458 :
	*/
	...
	478 : 	   <dt>int rank
	479 : 	   <dd>順位［1番目の値を取る［ (排名 [取第1位的值])
	480 : 	   <dt>int count
	481 : 	   <dd>何番目からいくつ取り出すか［先頭のエントリは0からはじまる［ (从第几个开始取几个 [首个条目从0开始])
	482 : 	   <dt>string table
	483 : 	   <dd>  テーブル  をそのままかえす (原样返回表)
	484 : 	   <dt>string key
	485 : 	   <dd>キーをそのままかえす (原样返回键)
	486 : 	   <dt>int msgid, int msgid2
	487 : 	   <dd>msgid をそのまま返す (原样返回msgid)
	488 : 	 </dl>
	489 : 	 <BR><BR>
	490 :
	491 :
	*/
	...
	509 : 	   <dt>string table
	510 : 	   <dd>  テーブル  をそのままかえす (原样返回表)
	511 : 	   <dt>string key
	512 : 	   <dd>キーをそのままかえす (原样返回键)
	513 : 	   <dt>int msgid, int msgid2
	514 : 	   <dd>msgid をそのまま返す (原样返回msgid)
	515 : 	 </dl>
	516 : 	 <BR><BR>
	517 :
	*/
	...
	536 : 	   <dd>値そのもの (值本身)
	537 : 	   <dt>string table
	538 : 	   <dd>  テーブル  をそのままかえす (原样返回表)
	539 : 	   <dt>string key
	540 : 	   <dd>キーをそのままかえす (原样返回键)
	541 : 	   <dt>int msgid, int msgid2
	542 : 	   <dd>msgid をそのまま返す (原样返回msgid)
	543 : 	 </dl>
	544 :
	545 :
	546 :
	*/
	...
	563 : 	 DBGetEntryByRankの結果［ (DBGetEntryByRank的结果)
	564 : 	 <dl>
	565 : 	   <dt>string result
	566 : 	   <dd>
	567 : 	   <dt>string list
	568 : 	   <dd>"RANK,KEY,SCORE,INFO|RANK,KEY,SCORE,INFO|...."<BR>
	569 : 		   RANKは順位である。KEYはその順位になっているスコアをもっている (RANK是排名。KEY是排名对应的分数)
	570 : 		   キー［SCOREは整数でスコアである［ (键 [SCORE是整数分数])
	571 : 		     ［<br>
	572 : 		   "1,RINGO,123465,data1|2,HOGE,4567,data2|2,FUCK,4567,data3"<BR>
	573 : 		   この場合、2位は2人いることになる (这种情况下，第2名有2人)
	574 : 	   <dt>string table
	575 : 	   <dd>  テーブル (表)
	...
	*/
	...
	594 : 	 DBGetEntryByCount の結果［ (DBGetEntryByCount的结果)
	595 : 	 <dl>
	596 : 	   <dt>string result
	597 : 	   <dd>SUCCESSFUL/FAILED
	598 : 	   <dt>string list
	599 : 	   <dd>返  リスト［1エントリあたり "INDEX,RANK,KEY,SCORE,INFO" の4個の値 (返回列表 [每个条目 "INDEX,RANK,KEY,SCORE,INFO" 共4个值)
	600 : 		   を持ちであり、それが個数分 "|" でつながる。個数が0の場合は
	601 : 		   空リストの場合［ (列表为空的情况)
	602 : 	   <dt>int num
	603 : 	   <dd>いくつとりだすか［1なら1個［2なら2個［ (取出几个 [1则1个 [2则2个])
	...
	*/
	...
	622 : 	 Broadcastのアカウントサーバーからゲームサーバーへの送信用 (Broadcast 从账户服务器到游戏服务器的发送用)
	623 : 	 <dl>
	624 : 	   <dt>string id
	625 : 	   <dd>user ID
	...
	629 : 	   <dd>メッセージ (消息)
	630 : 	   <dt>int flag
	631 : 	   <dd>1にすると、全部には送されない［ (设为1则不发送给全部)
	632 : 	 </dl>
	633 :
	*/
	...
	653 : 	 アカウントサーバーからゲームサーバーにメッセージを送信する。 (从账户服务器向游戏服务器发送消息)
	654 : 	 <dl>
	...
	664 : 	   <dd>メッセージの (消息的)
	665 : 	   <dt>int option
	666 : 	   <dd>オプション(この関数を参照) (选项(参见此函数))
	667 : 	   <dt>int mesgid
	668 : 	   <dd>メッセージID アカウントサーバーの用 (消息ID 供账户服务器使用)
	669 : 	 </dl>
	670 :
	*/
