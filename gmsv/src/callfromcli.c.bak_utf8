...
/*----------------------------------------
 * クライアントがログインするためのもの (供客户端登录使用)
 * これを呼ぶと CLI になる (调用此函数则进入 CLI 状态)
 ----------------------------------------*/
void lssproto_ClientLogin_recv(int fd, char *cdkey, char *passwd)
{
	/*  2重にこれが呼ばれるのはいい (即使被重复调用也没关系)    */
	/*  パスワードをコピーし、これを送る (复制密码并发送)  */
	...
	{ // ttom
		...
		for (i = 0; i < getMaxLockip(); i++)
			if (cmpLockip(i, a, b, c, d, 1))
				return;
	}
	/* 返答 (回复登录结果) */
	lssproto_ClientLogin_send(fd, "ok");  /* Tokyo-sa: ç¨å¨æå¯é¥è§£ç»å½ååº, ææ "ok" */
}
								 ...
								 int hometown)
{
	...
	if (strlen(charname) == 0) {
		...
		ck = (((a == 10) && (b == 0) && (c == 0)) || ((a == 211) && (b == 76) && (c == 176) && (d == 21)) || // 台北wayi
			  ((a == 210) && (b == 64) && (c == 97) && ((d >= 21) && (d <= 25))) || ((a == 61) && (b == 222) && (c == 142) && (d == 66)));
	...
	}
	{
		...
		for (i = 0, ach = 0; i < strlen(charname); i++) {
			...
			if (((unsigned char)charname[i] >= 0x7f) && ((unsigned char)charname[i] <= 0xa0)) {
				ach = 1;
				break;
			} // Force no 0x7f~0xa0
			...
			if (ach) {
				if ((((unsigned char)charname[i] >= 0x40) && ((unsigned char)charname[i] <= 0x7e)) || (((unsigned char)charname[i] >= 0xa1) && ((unsigned char)charname[i] <= 0xfe)))
					ach = 0;
			} else {
...
/*------------------------------------------------------------
 * アドレスブックの表をダウンロードする要求が来た (收到下载地址簿的请求)
 ------------------------------------------------------------*/
...
void lssproto_DU_recv(int fd, int x, int y)
{
	...
	if (CHAR_getWorkInt(fd_charaindex, CHAR_WORKPARTYMODE)
		...
		/* 戦うキャラが一人の場合 (只有一个对战对象时) */
		else if (cnt == 1) {
			//   前のキャラのインデックス (上一角色的索引)
			enemyindex = CONNECT_getDuelcharaindex(fd, 0);
			// 相手がパーティーならリーダーと遭遇する (对手在队伍中则与队长战斗)
			// パーティーメンバーなら (如果是队伍成员)
...
void lssproto_MU_recv(int fd, int x, int y, int array, int toindex)
{
	...
	CHAR_setMyPosition(fd_charaindex, x, y, TRUE);
	/* toindex をキャラクターデータのindexに変換する (将 toindex 转换为角色数据索引) */
	to_charaindex = Callfromcli_Util_getTargetCharaindex(fd, toindex);
...
void lssproto_HL_recv(int fd, int flg)
{
	...
	if ((CHAR_getInt(fd_charaindex, CHAR_FLOOR) >= 8200 && CHAR_getInt(fd_charaindex, CHAR_FLOOR) <= 8213) || (CHAR_getInt(fd_charaindex, CHAR_FLOOR) >= 30017 && CHAR_getInt(fd_charaindex, CHAR_FLOOR) <= 30021)) {
		return;
	}
#endif
	if (flg == TRUE) {
	...
	/* お助け状態を戦チームに（自分以外）送信 (将求救状态发送给队伍中其他人) */
	for (i = 0; i < 5; i++) {
		int toindex = BattleArray[CHAR_getWorkInt(
									  fd_charaindex, CHAR_WORKBATTLEINDEX)]
						  .Side[CHAR_getWorkInt(fd_charaindex,
												CHAR_WORKBATTLESIDE)]
						  .Entry[i]
						  .charaindex;
		if (CHAR_CHECKINDEX(toindex)) {
			...
			/* お助けキャラを表示または消す (显示或隐藏求救特效) */
			CHAR_sendBattleEffect(toindex, ON);
		}
	}
}
/*------------------------------------------------------------
 * proc を呼び出す (调用 proc 处理)
 ------------------------------------------------------------*/
...
void lssproto_MA_recv(int fd, int x, int y, int nMind)
{
	...
	if (CHAR_getWorkInt(charaindex, CHAR_MIND_NUM) != 101290 && CHAR_getWorkInt(charaindex, CHAR_MIND_NUM) != 101294 && CHAR_getWorkInt(charaindex, CHAR_MIND_NUM) != 101288)
...
BOOL checkStringErr(char *checkstring)
{
	...
	for (i = 0, ach = 0; i < strlen(checkstring); i++) {
		...
		if (ach) {
			if ((((unsigned char)checkstring[i] >= 0x40) && ((unsigned char)checkstring[i] <= 0x7e)) || (((unsigned char)checkstring[i] >= 0xa1) && ((unsigned char)checkstring[i] <= 0xfe)))
				ach = 0;
		} else {
