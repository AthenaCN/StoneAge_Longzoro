...
static BOOL MAP_readMapOne(char *filename) {
	FILE *f;		  /*  ファイル    (文件) */
	char buf[16];	  /*  マジックナンバー  確認用  (魔数校验用) */
	short data[1024]; /*  ショート  確認用バッファ  (short校验用缓冲) */
	int ret;		  /*  返り値受け取り用        (接收返回值用) */
	int i;			  /*  ループ数   (循环数) */
	int mapindex;

	int id = 0, xsiz = 0, ysiz = 0; /*  マップデータの場所   (地图数据位置) */

	short *tile = NULL;
	short *obj = NULL;
	MAP_Objlink **olink = NULL;
	char showstring[32];  /*  マップデータの場所   (地图数据位置) */
	struct stat filestat; /*  チェック用に使う    (用于检查) */
...
char *MAP_getdataFromRECT(int floor, RECT *seekr, RECT *realr) {
	...
	for (i = realr->y; i < realr->y + realr->height; i++) {
		for (j = realr->x; j < realr->x + realr->width; j++) {
			MAP_workdatabuffer[databufferindex++] = MAP_map[floorindex].tile[i * floorx + j];
		}
	}
...
BOOL MAP_makeVariousMap(char *atile, char *aobj, int floor, int startx,
	...
	for (i = 0; i < ysiz; i++) {
		for (j = 0; j < xsiz; j++) {
			atile[i * xsiz + j] = MAP_getImageInt(MAP_map[findex].tile[startpoint + i * xsiz + j], MAP_coKindAndInt[kind])
									  ? 1
									  : 0;

			aobj[i * xsiz + j] = MAP_getImageInt(MAP_map[findex].obj[startpoint + i * xsiz + j], MAP_coKindAndInt[kind])
									 ? 1
									 : 0;
		}
	}
...
#else
	if (MAP_removeObj(ofloor, ox, oy, objindex) && MAP_appendTailObj(nfloor, nx, ny, objindex))
		return TRUE;
#endif
	print("%s:%d:错误\n", __FILE__, __LINE__);
	return FALSE;
