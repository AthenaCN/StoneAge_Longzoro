...
/*
 * ドアを表示する門NPCの関数の説明  (显示门的门NPC函数说明)
 * タイプ  “ Door  (类型 Door)
 *
 * NPCARGUMENTの説明  (NPC参数说明)
 *
 * og|cg|idname|swcount|closetime_sec|soonflg0/1|passflg0/1/2
 *
 * passflg は1なら終了したら開くらしい  (passflg 1则结束后打开) 2なら
 * 開きっぱなしになる  (保持打开) 0なら通常ドアになる  (普通门)
 *
 * 注意：引数の時間は開始リストなら1時間  (注意：参数时间若为开始列表则为1小时)
 * 修正を加える時は遠景グラフィック修正も行うこと  (修改时需要同时修正远景图形)
 *
 * 戦闘の結果でCを送信する関数がある時は1をinfo  (战斗结果发送C的函数存在时1为info)
 * を付けてその関数をクライアントからの名前へ変更すること  (加上后将该函数改为从客户端接收)
 * 名前を変更すること  (需改名)
 */

#define NPC_DOOR_OPENMSG "门打开了!"
#define NPC_DOOR_CLOSEMSG "门关上了!"
#define NPC_DOOR_CANTOPENMSG "钥匙不匹配，无法打开门!"

#define ROPEGRAPHIC 9259 /* 縄のグラフィック  (绳子图形) */
...
/*
 * ドア初期化の修正をする時はここで  (初始化门需修改时在此)
 * maxhp, maxmp, level, str, tough を0にする  (将上述属性设为0)
 *
 */
...
void NPC_DoorPostOver(int meindex, int movedindex) {
	if (CHAR_getWorkInt(meindex, CHAR_WORKDOORSOONFLG)) {
		...
		CHAR_setFlg(meindex, CHAR_ISOVERED, 0); /* グラフィック更新  (更新图形) */
	}
}
