	...
	if (buttonproc[button].use == TRUE) {
		...
		if (buttonproc[button].checkhaveitem != -1) {
			for (i = 0; i < CHAR_MAXITEMHAVE; i++) {
				...
				if (ITEM_CHECKINDEX(itemindex)) {
					if (ITEM_getInt(itemindex, ITEM_ID) == buttonproc[button].checkhaveitem) {
						break;
					}
				}
			}
		...
		if (buttonproc[button].checkdonthaveitem != -1) {
			for (i = 0; i < CHAR_MAXITEMHAVE; i++) {
				int itemindex = CHAR_getItemIndex(talkerindex, i);
				if (ITEM_CHECKINDEX(itemindex)) {
					if (ITEM_getInt(itemindex, ITEM_ID) == buttonproc[button].checkdonthaveitem) {
						break;
					}
				}
			}
...
BOOL NPC_Windowman_readData(int meindex, int windowno, BOOL chkflg) {
	...
	while (readflg == TRUE) {
		...
		while (1) {
			...
			/* delim "=" で  初め(1)のトークンを  取る  (用delim "="取第一个(1)标记) */
			...
			/* delim "=" で2番目のトークンを  取る  (用delim "="取第二个标记) */
			...
			/* ウィンドウNo が決まっていない時の行は  無視する  (未确定窗口编号时忽略该行) */
			if (winno == -1) {
			...
			/* ウィンドウNo が一致した時は対応を見る  (窗口编号一致时查看对应)
			 * それ以外は  無視する  (其它则忽略) */
...
/*
 * buttontype= で指定した  値  を数値に  変換する  (将buttontype=指定的值转换为数值)
 *
 */
/*static*/ int NPC_Windowman_restoreButtontype(char *data) {
	int ret = 0;
