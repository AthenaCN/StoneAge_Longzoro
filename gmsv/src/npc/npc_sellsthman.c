...
void NPC_SellrePutList(int defpage, int ti) {
	int i;
	if (NSellSthList[defpage][ti].use != 0)
		return;
	for (i = (ti + 1); i < MAX_LISTNUM - 1; i++) {
		if (NSellSthList[defpage][i].use != 0)
			continue;
...
void NPC_SellsthMan_selectWindow(int meindex, int toindex, int num, int select, char *data) {
	...
	case WINDOW_CALL: {
		...
		if (NSellSthList[page][num].use == 0)
			return;
	...
	case WINDOW_FIND: {
		...
		if (NPC_getSellSth_ListString(page, num, token, sizeof(token)) == FALSE)
			return;
...
BOOL NPC_getSellSth_ListString(int page, int num, char *ListString, int slen) {
	...
	if (strlen(ListString) + strlen(buff) > slen)
		return FALSE;
