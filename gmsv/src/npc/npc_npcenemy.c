...
enum {
	CHAR_WORK_ENCOUNTTYPE = CHAR_NPCWORKINT1, /* エンカウントの種類   (遇敌的种类) */
	CHAR_WORK_DIEACT = CHAR_NPCWORKINT2,	  /* 死んだ時どうするか (死亡时怎么办) */
	CHAR_WORK_WARP_FLOOR = CHAR_NPCWORKINT3,  /* ワープするフロア (传送的楼层) */
	CHAR_WORK_WARP_X = CHAR_NPCWORKINT4,	  /* ワープするX (传送的X坐标) */
	CHAR_WORK_WARP_Y = CHAR_NPCWORKINT5,	  /* ワープするY (传送的Y坐标) */
	CHAR_WORK_ONEBATTLE = CHAR_NPCWORKINT6,	  /* 一回しか戦えない  (只能战斗一次) */
	NPC_TIME_MODE = CHAR_NPCWORKINT7,
	CHAR_WORK_BASEIMGBAK = CHAR_NPCWORKINT8,   /* もとの画像番号  (原来的图像编号) */
	CHAR_WORK_DIETIME = CHAR_NPCWORKINT9,	   /* 死んだ時間  (死亡时间) */
	CHAR_WORK_REVIVALTIME = CHAR_NPCWORKINT10, /* 蘇り返る時間  (复活时间) */
	...
	if (flg) {
		...
		if (CHAR_getWorkInt(charaindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_CLIENT) {
			...
			if (NPC_Util_GetStrFromStrWithDelim(argstr, config, buf, sizeof(buf)) != NULL) {
				...
				char buf2[256];
				/* askbattlemsg? があれば数値にするコードを取る  (若有 askbattlemsg? 则取其代码转换为数值) */
				for (i = 2; i < 7; i++) {
...
int NPC_NPCEnemy_Dying(int battleindex, int meindex) {
	...
	if (strstr(argstr, "NEWNPCENEMY")) {
		...
		if (NPC_Util_GetStrFromStrWithDelim(argstr, "endmsg", buf, sizeof(buf)) != NULL) {
			for (i = 0; i < 5; i++) {
				/* 戦いに参加している相手にメッセージ  (向正在参战的对手发送消息) */
