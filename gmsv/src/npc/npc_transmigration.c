...
static void NPC_Transmigration_selectWindow(int meindex, int toindex, int num) {
	...
	// 修正データの転生回数と照合するための文字列  (与修正数据中的转生次数比对的字符串)
...
#if 0	
	//設定ファイルを確認できない時はエラー  (无法确认配置文件时报错)  
	if(tenflg == FALSE){
...
//********************************************************
//
//	転生条件を確認しているかどうかのチェック  (检查是否确认转生条件)
//
//  フラグ  (标志)
//	0:転生回数  (0:转生次数)
//	-1:なし  (-1:无)
...
static int NPC_TransmigrationCheck(int meindex, int talker) {
	...
	if (num < 4) {
		...
		for (i = 0; i < CHAR_MAXPETHAVE; i++) {
			...
			// ペットのレベルが条件を満たしているか  (宠物等级是否满足条件)
			//			if(CHAR_getInt( petindex, CHAR_LV) < PETLEVEL) continue;
			// ここで該当ペットを確定  (在此确定目标宠物)
			break;
		}
		if (i == CHAR_MAXPETHAVE)
			return -1;
...
#endif
	{
		...
		for (i = 0; i < CHAR_MAXPETHAVE; i++) {
			...
			if (petindex == -1)
				continue;
			if (CHAR_getInt(petindex, CHAR_TRANSMIGRATION) == 2 && (CHAR_getInt(petindex, CHAR_PETID) == 2 || CHAR_getInt(petindex, CHAR_PETID) == 112 || CHAR_getInt(petindex, CHAR_PETID) == 102 || CHAR_getInt(petindex, CHAR_PETID) == 34))
				break;
...
BOOL NPC_TransmigrationMain(int meindex, int toindex, char *buf) {
	...
	// 転生ログを記録  (记录转生日志)
	LogTensei(
		CHAR_getChar(toindex, CHAR_NAME), /* キャラクター  (角色)   */
...
#define RAND(x, y) ((x - 1) + 1 + (int)((double)(y - (x - 1)) * rand() / (RAND_MAX + 1.0)))
	...
	for (i = 0; i < CHAR_MAXPETHAVE; i++) {
		...
		if (
#ifdef _PET_2TRANS
			CHAR_getInt(petindex, CHAR_PETID) == tpetidx1 || CHAR_getInt(petindex, CHAR_PETID) == tpetidx2
#else
