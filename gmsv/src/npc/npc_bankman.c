...
struct {
	...
	int battle;
	int gotowin;
} buttonproc[13]; /* ok,cancel, yes,no,prev,next の時の値  (...时值)   */
...
static BOOL NPC_Bankman_readData(int meindex, int windowno, BOOL chkflg) {
	...
	while (readflg == TRUE) {
		...
		while (1) {
			...
			/* delim "=" で  初期(1)のトークンを取る  (按=取初始(1)token)*/
			...
			/* delim "=" で2番目のトークンを取る  (按=取第2个token)*/
			...
			if (strcasecmp(firstToken, "winno") == 0) {
				...
				/* ウィンドウ番号Noを持つ  (持有窗口编号No) */
				winno = atoi(secondToken);
				continue;
			}
			/* ウィンドウ番号Noが一致していない時の値は無効にする  (窗口编号No不一致时的值设为无效) */
			if (winno == -1) {
			...
			/* ウィンドウ番号Noが一致しない時の値は を読み、移動は する  (窗口编号No不一致时读取值，移动时...) */
...
/*
 * buttontype=で指定した値を数値に置き換える  (将buttontype=指定的值转换为数值)［
 *
 */
static int NPC_Bankman_restoreButtontype(char *data) {
	int ret = 0;
