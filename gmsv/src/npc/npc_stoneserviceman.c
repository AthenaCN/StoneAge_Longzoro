...
static void NPC_StoneServiceMan_selectWindow(int meindex, int toindex, int num, int select) {
	...
	case 1:
		...
		if (CHAR_getWorkInt(meindex, NPC_WORK_FLAG2) == 1) {
			if (pUserid == NULL || SendToAP(Cli_CHECKPLAYER, meindex, 1, pUserid, NULL) == 0) {
				if (NPC_Util_GetStrFromStrWithDelim(npcarg, "SysBusyMsg", buf, sizeof(buf)) == NULL)
					return;
				sprintf(token, "\n %s", buf);
				buttontype = WINDOW_BUTTONTYPE_OK;
				windowtype = WINDOW_MESSAGETYPE_MESSAGE;
		...
		if (CHAR_getWorkInt(meindex, NPC_WORK_STATE) == SERVICE_STATE_2) {
			...
			if (nAid == AP_CHECKPLAYER) {
				switch (atoi(Data[0])) {
				case 0: // 没买
					if (NPC_Util_GetStrFromStrWithDelim(npcarg, "ErrMsg", buf, sizeof(buf)) == NULL)
						return;
					sprintf(token, "\n %s", buf);
					buttontype = WINDOW_BUTTONTYPE_OK;
					windowtype = WINDOW_MESSAGETYPE_MESSAGE;
					...
					break;
				case 1: // 有买
					if (NPC_Util_GetStrFromStrWithDelim(npcarg, "MainMsg", buf, sizeof(buf)) == NULL)
						return;
					sprintf(token, "5\n %s"
								   "\n 你要将%s"
								   "\n 的奖品给这个角色吗？\n"
	...
	case 2:
		...
		if (CHAR_getWorkInt(meindex, NPC_WORK_STATE) == SERVICE_STATE_3) {
			...
			if (nAid == AP_GET) {
						...
						for (i = 0; i < CHAR_MAXPETHAVE; i++) {
							if (CHAR_getCharPet(toindex, i) == -1) {
								...
								for (k = 0; k < enemynum; k++) {
									if (ENEMY_getInt(k, ENEMY_ID) == atoi(Data[0]))
										break;
								}
								if (k == enemynum)
									break;
				...
				if (NPC_Util_GetStrFromStrWithDelim(npcarg, "ExitMsg", buf, sizeof(buf)) == NULL)
					return;
	...
	case 3:
		pUserid = CHAR_getChar(toindex, CHAR_CDKEY);
		if (NPC_Util_GetStrFromStrWithDelim(npcarg, "CancelMsg", buf, sizeof(buf)) == NULL)
			return;
		sprintf(token, "\n %s", buf);
		buttontype = WINDOW_BUTTONTYPE_OK;
		windowtype = WINDOW_MESSAGETYPE_MESSAGE;
	...
	if (g_EnableService) {
		...
		if (CHAR_getWorkInt(meindex, NPC_WORK_ISUSE) == 1) {
			...
			if (nLeaveCount > getLoopTime()) {
				...
				if (pUserid != NULL)
					SendToAP(Cli_CANCEL, meindex, 1, pUserid, NULL);
				CHAR_setWorkInt(meindex, NPC_WORK_FLAG1, 1);
				CHAR_setWorkInt(meindex, NPC_WORK_FLAG2, 1);
				CHAR_setWorkInt(meindex, NPC_WORK_FLAG3, 1);
				CHAR_setWorkInt(meindex, NPC_WORK_STATE, SERVICE_STATE_2);
		...
		if (CHAR_getWorkInt(meindex, NPC_WORK_START) == 1) {
			...
			case 0:
				...
				return;
			case -1:
				break; // 重新connect to ap
			case -2:
				print("Msg buf is full!!\n");
				break; // 讯息buffer用光了
			case -3:
				print("Msg data is wrong!!\n");
				break; // 讯息内容错误
			case -4:
				print("calloc fail!!\n");
				break; // 记忆体配置错失败
			case -5:
				print("\nAP send get I'm life");
				break; // AP收到Cli_CONNECT的回应
			}
			CHAR_setWorkInt(meindex, NPC_WORK_START, 0);
		}
	}
...
void NPC_StoneServiceManWindowTalked(int meindex, int talkerindex, int seqno, int select, char *data) {
	...
	case CHAR_WINDOWTYPE_SERVICE_START:
		if (datanum == 1) {
			...
			NPC_StoneServiceMan_selectWindow(meindex, talkerindex, 3, -1);
		break;
...
int SendToAP(int nAid, int nIndex, int nLen, void *Data1, void *Data2) {
	...
	// Syu ADD 修正石器服务员星系确认
	char *sags = "";
	if (strcmp(getAccountservername(), "210.64.50.31") == 0)
		sags = "sags31"; // 北斗
	if (strcmp(getAccountservername(), "210.64.50.41") == 0)
		sags = "sags41"; // 天神
	if (strcmp(getAccountservername(), "210.64.50.51") == 0)
		sags = "sags51"; // 太阳
	if (strcmp(getAccountservername(), "210.64.50.61") == 0)
		sags = "sags61"; // 紫微
	if (strcmp(getAccountservername(), "210.64.50.71") == 0)
		sags = "sags71"; // 苍龙
	if (strcmp(getAccountservername(), "210.64.50.81") == 0)
		sags = "sags81"; // 银河
	if (strcmp(getAccountservername(), "210.64.50.91") == 0)
		sags = "sags91"; // 天鹰
	if (strcmp(getAccountservername(), "210.64.50.101") == 0)
		sags = "sags101"; // 网路家庭
	if (strcmp(getAccountservername(), "210.64.50.106") == 0)
		sags = "sags106"; // 星乐园
	if (strcmp(getAccountservername(), "210.64.50.111") == 0)
		sags = "sags111"; // 圣兽
	if (strcmp(getAccountservername(), "152.104.1.206") == 0)
		sags = "hkgs206"; // 香港
	if (strcmp(getAccountservername(), "202.134.122.130") == 0)
		sags = "hkgs130"; // 新界
...
int ConnectToAP(void) {
	...
	if (connect(g_nServiceSocket, &temp, sizeof(temp)) < 0) {
		...
		print("connect AP server success!\n");
