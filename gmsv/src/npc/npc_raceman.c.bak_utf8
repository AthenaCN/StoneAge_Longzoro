...
void NPC_RacemanWindowTalked(int meindex, int talkerindex, int seqno, int select, char *data) {
	...
	case CHAR_WINDOWTYPE_RACE_RANK:
		...
		case 1:
			...
			if (ranktable1[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 41, -1);
			break;
		case 2:
			...
			if (ranktable2[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 42, -1);
			break;
		case 3:
			...
			if (ranktable3[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 43, -1);
			break;
		case 4:
			...
			if (ranktable4[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 44, -1);
			break;
		case 5:
			...
			if (ranktable5[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 45, -1);
			break;
		case 11:
			if (history1[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 411, -1);
			break;
		case 12:
			if (history2[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 412, -1);
			break;
		case 13:
			if (history3[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 413, -1);
			break;
		case 14:
			if (history4[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 414, -1);
			break;
		case 15:
			if (history5[0].rank == 0)
				Raceman_MsgDisp(meindex, talkerindex, 46, -1);
			else
				Raceman_MsgDisp(meindex, talkerindex, 415, -1);
			break;
		default:
			Raceman_SelectWindow(meindex, talkerindex, 0, -1);
			break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK1:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (ranktable1[i].rank == 0 || strlen(ranktable1[i].name) == 0 || ranktable1[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK2:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (ranktable2[i].rank == 0 || strlen(ranktable2[i].name) == 0 || ranktable2[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK3:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (ranktable3[i].rank == 0 || strlen(ranktable3[i].name) == 0 || ranktable3[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK4:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (ranktable4[i].rank == 0 || strlen(ranktable4[i].name) == 0 || ranktable4[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK5:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (ranktable5[i].rank == 0 || strlen(ranktable5[i].name) == 0 || ranktable5[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK11:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (history1[i].rank == 0 || strlen(history1[i].name) == 0 || history1[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK12:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (history2[i].rank == 0 || strlen(history2[i].name) == 0 || history2[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK13:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (history3[i].rank == 0 || strlen(history3[i].name) == 0 || history3[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK14:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (history4[i].rank == 0 || strlen(history4[i].name) == 0 || history4[i].catchcnt == 0)
					break;
	...
	case CHAR_WINDOWTYPE_RACE_SHOWRANK15:
		...
		case WINDOW_BUTTONTYPE_PREV: {
			...
			for (i = Pageindex; i < Pageindex + 10; i++) {
				if (history5[i].rank == 0 || strlen(history5[i].name) == 0 || history5[i].catchcnt == 0)
					break;
...
BOOL Raceman_EventAddItem(int meindex, int talker) {
	...
	if (CHAR_getInt(talker, CHAR_CATCHCNT1 + gmode - 1) != 0) {
		...
		for (i = 0; i < 10; i++)
			CHAR_setInt(talker, CHAR_KINDCNT1 + i, 0);
		...
		switch (gmode) {
		case 1:
			NPC_Util_GetStrFromStrWithDelim(argstr, "Mode1", buf1, sizeof(buf1));
			break;
		case 2:
			NPC_Util_GetStrFromStrWithDelim(argstr, "Mode2", buf1, sizeof(buf1));
			break;
		case 3:
			NPC_Util_GetStrFromStrWithDelim(argstr, "Mode3", buf1, sizeof(buf1));
			break;
		case 4:
			NPC_Util_GetStrFromStrWithDelim(argstr, "Mode4", buf1, sizeof(buf1));
			break;
		case 5:
			NPC_Util_GetStrFromStrWithDelim(argstr, "Mode5", buf1, sizeof(buf1));
			break;
		}
	...
	if (gmode == 4 || gmode == 5) // 不限种类
	{
		...
		*/
		...
		if (NPC_Util_GetStrFromStrWithDelim(argstr, "CheckPet", buf, sizeof(buf)) != NULL) {
			checkpetID = atoi(buf);
			if (!ENEMY_CHECKINDEX(checkpetID))
				return FALSE;
...
BOOL Raceman_petcheckin(int meindex, int talker) {
	...
	switch (gmode) {
	case 1:
		NPC_Util_GetStrFromStrWithDelim(argstr, "Mode1", buf, sizeof(buf));
		break;
	case 2:
		NPC_Util_GetStrFromStrWithDelim(argstr, "Mode2", buf, sizeof(buf));
		break;
	case 3:
		NPC_Util_GetStrFromStrWithDelim(argstr, "Mode3", buf, sizeof(buf));
		break;
	case 4:
		NPC_Util_GetStrFromStrWithDelim(argstr, "Mode4", buf, sizeof(buf));
		break;
	case 5:
		NPC_Util_GetStrFromStrWithDelim(argstr, "Mode5", buf, sizeof(buf));
		break;
	}
	...
	// CHAR_talkToCli( talker, -1, buuf, CHAR_COLORYELLOW);
	if (gmode == 3 || gmode == 5) {
		saacproto_ACRaceRecordfmdo_send(acfd, talker, CHAR_getInt(talker, CHAR_FMINDEX), CHAR_getInt(petindex, CHAR_BASEBASEIMAGENUMBER), CHAR_getChar(petindex, CHAR_UNIQUECODE), CHAR_getChar(petindex, CHAR_NAME));
