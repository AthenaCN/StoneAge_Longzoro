...
static void NPC_Duelranking_selectWindow(int meindex, int toindex, int num, int select) {
	...
	case 3: // 正式赛程
	{
		...
		if (page < 0 || page >= 11)
			page = 0;
		if (page == 0)
			button = WINDOW_BUTTONTYPE_OK | WINDOW_BUTTONTYPE_NEXT;
		if (page == 10)
	...
	case 4: {
		...
		if (PKLIST_GetMyPKListTeamData(teanum, token, sizeof(token)) == FALSE)
			return;
...
*/
	...
	case 3: {
		...
		for (i = 0; i < pagecount; i++) {
			...
			if (NPC_getFamilyPK_List((page * pagecount) + i, buf, sizeof(buf)) == FALSE)
				continue;
			// andy_log
			print("FPKList[%d]:%s\n", i, buf);

			if (!getStringFromIndexWithDelim(buf, "|", 1, buf1, sizeof(buf1)))
				continue;
			famindex = atoi(buf1);
			if (!getStringFromIndexWithDelim(buf, "|", 2, familyname, sizeof(familyname)))
				continue;
			if (!getStringFromIndexWithDelim(buf, "|", 4, mname, sizeof(mname)))
				continue;
			if (!getStringFromIndexWithDelim(buf, "|", 5, buf1, sizeof(buf1)))
				continue;
			joinE = atoi(buf1);
	...
	case 3: {
		...
		if ((fp = fopen("d_fm_pk_log.txt", "r")) != NULL) {
			...
			while (fgets(buf1, sizeof(buf1) - 1, fp)) {
				// andy_log
				print("read buf1:%s !!\n", buf1);
				if (buf1[0] == 0 || buf1[0] == '#' || strlen(buf1) <= 0)
					continue;
...
void NPC_Duelrankingloop(int meindex) {
	...
	if (CHAR_getWorkInt(meindex, NPC_WORK_BATTLELOOP) == 0)
		return;
...
BOOL NPC_ReadFamilyPK_List(void) {
	...
	while (fgets(arg, sizeof(arg), fp)) {
		...
		if (!getStringFromIndexWithDelim(arg, "|", 1, buf1, sizeof(buf1)))
			continue; // fmindex
		JoinFamilyList[k].fmindex = atoi(buf1);

		if (!getStringFromIndexWithDelim(arg, "|", 2, buf1, sizeof(buf1)))
			continue;
		snprintf(JoinFamilyList[k].fmname, sizeof(JoinFamilyList[k].fmname), "%s", buf1);

		if (!getStringFromIndexWithDelim(arg, "|", 3, buf1, sizeof(buf1)))
			continue;
		snprintf(JoinFamilyList[k].uname, sizeof(JoinFamilyList[k].uname), "%s", buf1);

		if (!getStringFromIndexWithDelim(arg, "|", 4, buf1, sizeof(buf1)))
			continue;
		snprintf(JoinFamilyList[k].ucdkey, sizeof(JoinFamilyList[k].ucdkey), "%s", buf1);

		if (!getStringFromIndexWithDelim(arg, "|", 5, buf1, sizeof(buf1)))
			continue;
...
BOOL NPC_BackupFamilyPK_List(void) {
	...
	if ((fp = fopen("fmlist.txt", "w+")) == NULL)
		return FALSE;
	for (i = 0; i < MAXJOINFM; i++) {
		if (JoinFamilyList[i].use == 0)
			continue;
...
BOOL NPC_getFamilyPK_List(int ti, char *buf, int sizes) {
	// andy_log
	print("ti:%d\n", ti);
	if (ti < 0 || ti > MAXJOINFM)
		return FALSE;
	if (JoinFamilyList[ti].use == 0)
		return FALSE;
...
BOOL NPC_CheckJoin_FamilyPK_List(int meindex, int toindex) {
	...
	for (i = 0; i < MAXJOINFM; i++) {
		...
		if (JoinFamilyList[i].fmindex != CHAR_getInt(toindex, CHAR_FMINDEX))
			continue;
