...
/*設定を簡潔にするために関数定義セットを纏める  (为简化设置而整理函数定义集)    */
...
/*------------------------------------------------------------
 * Template index が正しいかどうかcheckする  (是否正确)checkする
 * 引数  (参数)
 *  index       int             index
 * 戻り値  (返回值)
 *  valid       TRUE
...
/*------------------------------------------------------------
 * Template intdata index が正しいかどうかcheckする  (是否正确)checkする
 * 引数  (参数)
 *  index     int             index
 * 戻り値  (返回值)
 *  valid       TRUE
...
/*------------------------------------------------------------
 * Template chardata index が正しいかどうかcheckする  (是否正确)checkする
 * 引数  (参数)
 *  index     int             index
 * 戻り値  (返回值)
 *  valid       TRUE
...
/*------------------------------------------------------------
 * NPC_Templateを初期化する  (初始化NPC_Template)
 * 引数  (参数)
...
/*------------------------------------------------------------
 * デフォルトのNPCテンプレートを設定する  (设置默认NPC模板)［
 * 引数  (参数)
 *  temp    NPC_Template*     テンプレートのポインタ  (模板的指针)□
 * 戻り値  (返回值)
 *  なし  (无)
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * NPC_getRandomValue NPC_getRandomValue との値数の換算のための関数  (与NPC_getRandomValue的数值换算用函数)
 * 引数  (参数)
 *  string      char*         侬
 *  unuse       int*        银歹卅中［
 * 戻り値  (返回值)
 *  ランダムの範囲の幅の設定は2つの値に保持する  (随机范围的宽度设置保存为2个值)
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * NPCTemplate NPCTemplate ファイルを読み込む  (读取NPCTemplate文件)［
 * 引数  (参数)
 *  filename   char*       設定ファイルを読み込むトップディレクトリ  (读取设置文件的顶层目录)
 * 戻り値  (返回值)
 *  成功  (成功)      TRUE(1)
 *  だけど（これ以前のファイルを読んではいけない）  (但是（不能读取此前的文件）))
 *  失敗  (失败)      FALSE(0)
 ------------------------------------------------------------*/
				...
				for (i = 0; i < arraysizeof(NPC_readtemplate); i++) {
					if (strcasecmp(NPC_readtemplate[i].keyword,
						...
						case NPC_INTFUNC: {
							...
							temp.intdata[NPC_readtemplate[i].index] = intfunction(secondToken, &randomdata[NPC_readtemplate[i].index]);
							break;
						}
						case NPC_CHARFUNC: {
...
/*------------------------------------------------------------
 *   名前に対象する template の index を返す  (返回与名称对应的模板索引)［
 * 引数  (参数)
 *  templatename        char*         テンプレートの名前  (模板的名称)
 * 戻り値  (返回值)
 *  -1  の値エラー  (值错误)□
 ------------------------------------------------------------*/
