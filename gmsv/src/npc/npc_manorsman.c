					...
					if (hadfmindex - 1 != tkfmindex) {
						...
						if (check == 0) {
							...
							if (index >= FAMILY_MAXNUM) {
								...
								if (fmdptop.fmMomentum[index] < fmpointlist.fm_momentum[manorid - 1] * 0.9)
									won1 = 0;
								//-----------------------------------------
#else
							//-----------------------------------------
	...
	case CHAR_WINDOWTYPE_CHECKMAN_MAIN:
		if (select == WINDOW_BUTTONTYPE_YES) {
			...
			if ((CHAR_getInt(talkerindex, CHAR_FMLEADERFLAG) == 1) &&
				...
				fmpks[fmpks_pos + 1].dueltime = CHAR_getWorkInt(meindex, NPC_WORK_CHALLENGEWAIT) + NowTime.tv_sec - tm1.tm_min * 60 + 1800; // Arminius 11.1 改成一律 xx:30 开打
