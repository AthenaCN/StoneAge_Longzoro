...
/*    print( "长老的引数: '%s'\n" , CHAR_getChar(
    ...
    if( MAP_IsValidCoordinate( bornfl,bornx,borny )== TRUE
        /*  フロアIDがいいかどうかを調べる  (检查楼层ID是否合适) */
        &&
        CHAR_ElderSetPosition( oldmanid ,bornfl,bornx, borny ) 
        == TRUE ){
        /* 名前の位置を初期化するなり  (初始化名字的位置) */
        return TRUE;
    }else{
        printf( "Invalid elder npcarg=%s\n", npcarg );
        
        /* あまりに重要なエラーなのでneverMakeしない  (因是极其重要的错误故不设 neverMake)
             エラーメッセージを表示させる  (显示错误消息) */
        return FALSE;
    }
#endif
 ...
 *  名前  (名字)
 *
 *  主な仕様はしかけられたときに番号の値をしたり  (主要规格是在被设置时取出其编号值)
 * することだ  (就是这样)
 * * by ringo
 */
