...
BOOL NPC_SetNewItem(int meindex, int talker, char *data) {
	...
	/*--返ってきたデータをセレクトと個数に分解--  (将返回的数据分解为选择和个数) */
	...
	/*--返ってきた個数が実際のサーバー値より多いのでおかしい--  (返回的个数多于服务器实际值所以异常) */
	/*--サーバー値の方を取る--  (采用服务器那边的数值) */
	if (kosuucnt < kosuu)
		kosuu = kosuucnt;
	...
	/*--お店のファイルが無い又はファイルが壊れていたときは終了--  (商店文件不存在或损坏时则结束) */
	if (NPC_Util_GetArgStr(meindex, argstr, sizeof(argstr)) == NULL) {
		print("shop_GetArgStr_Err");
		return FALSE;
	}

	/*---レートを取れなければ1.0)-  (取不到倍率则为1.0) */
	...
	if (NPC_Util_GetStrFromStrWithDelim(argstr, "ItemList", buf, sizeof(buf)) != NULL) {
		while (getStringFromIndexWithDelim(buf, ",", j, buff2, sizeof(buff2)) != FALSE) {
			...
			if (strstr(buff2, "-") == NULL) {
				...
				/*--アイテムが "15-25" の形で送られた場合--  (物品以15-25的形式发送时) */
				int start;
				int end;

				/* "-"で区切られた前後の数値を取る  --  (取"-"分隔的前后数值) */
				...
				/*--"-"で区切られた分のアイテムを順に取る--  (依次获取"-"分隔部分的物品) */
				for (; start < end; start++) {
...
void NPC_ItemShop_SellMain(int meindex, int talker, int before) {
	...
	if (before != -1) {
		sprintf(token, "1|0");
		lssproto_WN_send(fd, WINDOW_MESSAGETYPE_ITEMSHOPMAIN + CHAR_getWorkInt(meindex, NPC_SHOP_WORK_NO),
						 WINDOW_BUTTONTYPE_NONE,
...
/*----------------------------------------------------------
	...
	クライアントに送信するプロトコルの一覧  (发送给客户端的协议列表)
