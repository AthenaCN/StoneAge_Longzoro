...
void NPC_FMWarpManLoop(int meindex) {
	...
	if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEFREE) {
		...
		if (tm1.tm_min > CHAR_getWorkInt(meindex, NPC_WORK_TALKFLAG) && tm1.tm_min == 0) {
			// 通知成员进场
...
void NPC_ERR_FMDiSP(int meindex, int talker, int errNO) {
	...
	if (errNO == 1) {
		...
		if (NPC_Util_GetStrFromStrWithDelim(npcarg, "PartyMsg", token, sizeof(token)) == NULL) {
			/*--設定されてなければ、下のメッセージを返す--  (若未设置则返回下面的消息) */
			sprintf(token, "\n\n无法加入团队，请先解散团队！");
		}
		...
		if (CHAR_getWorkInt(talker, CHAR_WORKPARTYMODE) == CHAR_PARTY_CLIENT) {
			...
			/*--リーダーがしかけた場合のみ--  (仅限领袖触发的情况) */
			/*--プレイヤー周囲にメッセージを開く--  (向玩家周围成员发送消息) */
		...
		if (NPC_Util_GetStrFromStrWithDelim(npcarg, "MoneyMsg", token, sizeof(token)) == NULL) {
			/*--お金が足りないのに設定されてなかったら、下のメッセージを代わりに開く--  (明明金钱不足却未设置时，则改发下面的消息) */
			sprintf(token, "\n\n金钱似乎不足唷，请存好钱後再过来。");
		}
	}
...
static void NPC_FMWarpMan_selectWindow(int meindex, int toindex, int num, int select) {
	...
	if (CHAR_getWorkInt(meindex, NPC_WORK_MODEFLAG) == NPC_STATEFREE) {
		if (((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) == fmpks[fmpks_pos].host_index) && (strcmp(CHAR_getChar(toindex, CHAR_FMNAME), fmpks[fmpks_pos].host_name) == 0)) || ((CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) == fmpks[fmpks_pos].guest_index) && (strcmp(CHAR_getChar(toindex, CHAR_FMNAME), fmpks[fmpks_pos].guest_name) == 0))) {
			if ((fmpks[fmpks_pos].host_index != -1) && (CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) == fmpks[fmpks_pos].host_index)) {
			...
			} else if ((fmpks[fmpks_pos].guest_index != -1) && (CHAR_getWorkInt(toindex, CHAR_WORKFMINDEXI) == fmpks[fmpks_pos].guest_index)) {
				// WON ADD 修正家族pk场的约战问题
