...
static void NPC_Quiz_selectWindow(int meindex, int talker, int num) {
	...
	case 2:
		/*--メイン  (主流程)--*/
		{
			...
			/*-- メモリの内容を別領域にコピー  (将内存内容复制到另一区域) --*/
			memcpy(&PLAYER, pl_ptr, sizeof(struct pl));

			/*--全ての問題が終わったので結果を出す  (所有问题结束后给出结果)--*/
			if (PLAYER.quizno == CHAR_getWorkInt(meindex, CHAR_WORK_QUIZNUM)) {
				double avg;
				int okflg;
				char buf2[512];

				/*-- argファイルの読み込み  (读取arg文件) --*/
				if (NPC_Util_GetArgStr(meindex, argstr, sizeof(argstr)) == NULL) {
					print("GetArgstrErr:Quiz %s", CHAR_getChar(meindex, CHAR_NAME));
				}

				/*--正解数に応じてアイテムを追加する  (按答对数量添加物品)---*/
				...
				/*--プレイヤーのワークを初期化  (初始化玩家工作区) -*/
				CHAR_setWorkInt(meindex, CHAR_WORK_PLAYER1 + CHAR_getWorkInt(talker, CHAR_WORKSHOPRELEVANT), -1);
				...
				// 全ての終了処理が終わってからワープさせる  (全部结束处理完毕后传送)
				if (warp_flg != -1) {
			...
			/*--このファイルに設定されている問題の番号を探し出す  (查找此文件中设定的问题编号)--*/
			tbl = NPC_GetQuestion(meindex);
			if (tbl == NULL) {
				free(PLAYER.ptr);
				return;
			...
			/*--  問題数のチェック tbl[0]には問題を足した問題数がはいっている  (问题数检查 tbl[0]中存有加入问题后的数量) --*/
			if (CHAR_getWorkInt(meindex, CHAR_WORK_QUIZNUM) > (tbl[0] - 1)) {
				/*--  問題が足りない  (问题不足)--*/
				/*--メモリ解放  (释放内存)  --*/
				free(PLAYER.ptr);
			...
			/*--クイズの問題の長さによって選択肢の表示位置を変える  (根据问题长度改变选项的显示位置)--*/
			if (strlen(buf) < 40) {
				strcat(buf, "\n");
...
void NPC_QuizWindowTalked(int meindex, int talkerindex,
	...
	case CHAR_WINDOWTYPE_QUIZ_MAIN: {
		...
		if (select == WINDOW_BUTTONTYPE_OK) {
			...
			CHAR_setWorkInt(meindex, CHAR_WORK_PLAYER1 + CHAR_getWorkInt(talkerindex, CHAR_WORKSHOPRELEVANT), -1);
			free(PLAYER.ptr);
		} else if (atoi(data) == 0) {
		} else {
...
BOOL NPC_QuizAddItem(int talker, char *buf) {
	...
	/*--  景品としてアイテムが設定されている場合はランダムで選ぶ  (作为奖品设定的物品随机选择)--*/
	...
	/*アイテムの追加（後ろのアイテム欄に入れてしまう）  (添加物品（放入后面的物品栏）)  */
...
BOOL NPC_PlayerCheck(int meindex, int talker) {
	...
	/*-- ポインタの場所を覚えておく  (记住指针位置)-*/
	Player.ptr = ptr;
	Player.talkerindex = talker; // npcのインデックス  (NPC的索引)
	Player.quizno = 0;			 // 現在の問題数  (当前问题数)
	Player.answer = 0;			 // 正解数  (答对数)
	Player.ansno = 0;			 // 答えの番号を覚えておく（答えをバラバラに並した際に利用）  (记住答案编号（打乱答案顺序时使用）))
...
int *NPC_GetQuestion(int meindex) {
	...
	{
		static int tmp_tbl[65536];
		if (j + 1 > 65536) {
			print("Quiz tbl overflow j=%d\n", j);
			return NULL;
		}
		tmp_tbl[0] = j + 1;
...
BOOL NPC_EntryItemCheck(int talker, char *buf) {
	...
	while (getStringFromIndexWithDelim(buf, ",", k, buf2, sizeof(buf2)) != FALSE) {
		flg = FALSE;
		k++;
