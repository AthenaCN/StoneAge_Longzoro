...
BOOL NPC_SavePointInit(int meindex) {
	...
	if (MAP_IsValidCoordinate(bornfl, bornx, borny) == TRUE && CHAR_ElderSetPosition(oldmanid, bornfl, bornx, borny) == TRUE) {
		return TRUE;
	} else {
...
void NPC_SavePointTalked(int meindex, int talkerindex, char *msg,
	...
	if (!NPC_Util_charIsInFrontOfChar(talkerindex, meindex, RANGE) || CHAR_getFlg(talkerindex, CHAR_ISDIE)) {
		if ((CHAR_getInt(talkerindex, CHAR_X) == CHAR_getInt(meindex, CHAR_X)) && (CHAR_getInt(talkerindex, CHAR_Y) == CHAR_getInt(meindex, CHAR_Y)) && (CHAR_getInt(talkerindex, CHAR_FLOOR) == CHAR_getInt(meindex, CHAR_FLOOR))) {
		} else {
			return;
		}
	}
...
BOOL NPC_AndReduce(int meindex, int talker, char *buf, int flg) {
	...
	if (flg == 0) {
		...
		while (getStringFromIndexWithDelim(buf, "&", j, buf2, sizeof(buf2)) != FALSE) {
			j++;
			/*--  ←  があればそのアイテムは数値指定されてると見做す  (若有 ← 则视为已指定数值) */
			if (strstr(buf2, "*") != NULL) {
...
void NPC_MessageDisp(int meindex, int talker, int MesNo) {
	...
	switch (MesNo) {
	case 0: /*--  要なアイテムをもっているかのチェック  (检查是否持有必需物品) */
		NPC_Util_GetStrFromStrWithDelim(argstr, "RequestMsg", token, sizeof(token));

		break;

	case 1: /*--  要なアイテムをもっているかのチェック  (检查是否持有必需物品) */
	...
	case 2: /*--  要なアイテムをもっているかのチェック  (检查是否持有必需物品) */
