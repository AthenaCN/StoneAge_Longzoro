...
BOOL NPC_IsNPCCreateFile(char *filename) {
	...
	/*  ~で終わるファイルはなしにする  (以~结尾的文件设为无) */
	if (filename == NULL || strlen(filename) < 1 || filename[strlen(filename) - 1] == '~' || filename[0] == '#' || strcmptail(filename, ".bak") == 0)
		return FALSE;
...
static int NPC_readCreateFile(char *filename) {
	...
	POINT center[2] = {{0, 0}, {0, 0}}; /*  born が 0 で move が 1  (born为0，move为1) */
	REC wh[2] = {{0, 0}, {0, 0}};		/*  born が 0 で move が 1  (born为0，move为1) */

	POINT lu[2] = {{0, 0}, {0, 0}}; /*  born が 0 で move が 1  (born为0，move为1) */
	POINT rd[2] = {{0, 0}, {0, 0}}; /*  born が 0 で move が 1  (born为0，move为1) */

	int defborn = FALSE;			 /*  born を設定したかどうか  (是否设置了born) */
	int defmove = FALSE;			 /*  move を設定したかどうか  (是否设置了move) */
	int deflurd[2] = {FALSE, FALSE}; /*  lu,rd で設定したかどうか  (是否用lu,rd设置) */
	char *ret;
	...
	while (fgets(line, sizeof(line), f)) {
		...
		case '}':
			if (start == ON) {
				...
				if (err == FALSE) {
					...
					/* 99/01/21 対応 [0]-->[1]に修正  (99/01/21 适配[0]-->[1]修正) */
					if (defmove) {
						if (deflurd[0]) {
		...
		default: {
			...
			if (strcasecmp("floorid", firstToken) == 0) {
				...
				cr.intdata[NPC_CREATEBASEIMAGENUMBER] = CHAR_seekGraphicNumberFromString(secondToken);
				...
				/*  secondToken を  値  に分解する  (将secondToken分解为值) */
...
#ifdef DEBUG
	/* 99/4/8 By Kawata csv形式にしてやる  (99/4/8 By Kawata 做成csv格式) */
	print("Npc_Create's\n");
...
/*------------------------------------------------------------
 * createindex から boundary が設定されているかどうかを見る  (查看boundary是否已从createindex设置)
 * 番号  (编号)
 *  createindex         int         createのインデックス  (create的索引)
 * 返り値  (返回值)
 *  設定されている          TRUE(1)  (已设置)
 *  設定されていない        FALSE(0)  (未设置)
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * createindex から  見える  区間のRECT を求める  (求出从createindex可见区间的RECT)
 * 番号  (编号)
 *  createindex         int           テンプレートのインデックス  (模板索引)
 *  r                   RECT*       値を書き込むRECT のポインタ  (写入值的RECT指针)
 * 返り値  (返回值)
 *  岳      TRUE(1)
 *  撩      FALSE(0)
 ------------------------------------------------------------*/
BOOL NPC_createGetRECT(int createindex, RECT *r) {
	...
	r->width = NPC_create[createindex].intdata[NPC_CREATEMOVERIGHTDOWNX] - NPC_create[createindex].intdata[NPC_CREATEMOVELEFTUPX];
	r->height = NPC_create[createindex].intdata[NPC_CREATEMOVERIGHTDOWNY] - NPC_create[createindex].intdata[NPC_CREATEMOVELEFTUPY];
	return TRUE;
}

/*------------------------------------------------------------
 * 時間から増やしてもいいかどうかチェックする  (检查从时间开始可否增加)
 * 番号  (编号)
 *  cindex      int     インデックス  (索引)
 * 返り値  (返回值)
 *  BOOL    増やしていい場合  TRUE(1)  (可增加时为TRUE)
 *  BOOL    増やしてはいけない場合  FALSE(0)  (不可增加时为FALSE)
 ------------------------------------------------------------*/
BOOL NPC_createCheckGenerateFromTime(int cindex) {
	...
	if (NPC_create[cindex].intdata[NPC_CREATEBORNNUM] <= NPC_create[cindex].workdata[NPC_CREATEWORKENEMYNUM])
		return FALSE;
...
/*------------------------------------------------------------
 * 増える時の時間の設定を初期化する  (初始化增加时的时间设置)
 * 番号  (编号)
 *  tindex      int     インデックス  (索引)
 * 返り値  (返回值)
 *  なし  (无)
 ------------------------------------------------------------*/
void NPC_createInitTime(int index) {
	/*  増える機会になったから値を設定  (到了增加时机故设置值) */
...
/*------------------------------------------------------------
 * キャラを増やしたときにカウント数を増やす関数  (角色增加时计数增加的函数)
 * 番号  (编号)
 *  index       int         インデックス  (索引)
 * 返り値  (返回值)
 *  なし  (无)
...
/*------------------------------------------------------------
 * キャラが死んでカウント数を減らす関数  (角色死亡时减少计数的函数)
 * 番号  (编号)
 *  index       int         インデックス  (索引)
 * 返り値  (返回值)
 *  なし  (无)
...
/*------------------------------------------------------------
 * カウント数が今いくつか調べる関数  (查询当前计数数量的函数)
 * 番号  (编号)
 *  index       int         インデックス  (索引)
 * 返り値  (返回值)
 *  TRUE:MAX  FALSE:違う  (TRUE:达到上限 FALSE:未达到)
 ------------------------------------------------------------*/
BOOL NPC_createCheckMaxEnemynum(int index) {

	if (NPC_create[index].intdata[NPC_CREATEBORNNUM] <= NPC_create[index].workdata[NPC_CREATEWORKENEMYNUM])
		return TRUE;
	else
		return FALSE;
}
