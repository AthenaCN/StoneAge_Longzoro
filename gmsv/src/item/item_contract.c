...
BOOL ITEM_initContractTable() {
	...
	while (1) {
		...
		if (getStringFromIndexWithDelim(data, "|", 1, token, sizeof(token)) == TRUE) {
			index = atoi(token);
			if (index < 0 || index >= MAX_CONTRACTTABLE)
				continue;
		} else {
			continue;
		}
...
void ITEM_contractSign(int fd, int itemindex, int signindex) {
	...
	if (i >= CHAR_MAXITEMHAVE)
		return;
	if (strcmp(ITEM_getChar(itemindex, ITEM_USEFUNC), "ITEM_contract"))
		return;

	conIndex = atoi(ITEM_getChar(itemindex, ITEM_ARGUMENT));
	if (signindex >= ITEM_contractTbl[conIndex].argnum)
		return;
	...
	if (contractSign[signindex].used == TRUE)
		return;
