 ...
 * これだけの幅の素材の区を取りうる範囲になる  (能取到这么大的素材区间)
 **********************************************/

/* 素材の値のランダム幅の割合 ①②③④分  (素材值的随机宽度比例，①②③④分) */
#define ITEM_GEN_RAND_MIN 700
#define ITEM_GEN_RAND_MAX 1200

/* アイテムを検索する素材の値の幅の割合  (搜索物品的素材值宽度比例) (double) */
...
static struct _tagItemRandRangeTableForItem {
	int num;	 /* 素材の備ランクの基値  (素材的备选等级基础值) */
	int minnum;	 /* ランクを取りうる値のMin  (等级可取值的Min) */
	int maxnum;	 /* ランクを取りうる値のMAX  num +前のランクの差*ITEM_GEN_RATE)  (等级可取值的MAX，num+前一等级差*ITEM_GEN_RATE) */
	...
	for (i = 0; i < arraysizeof(ItemRandTableForItem); i++) {
		...
		if (i + 1 != arraysizeof(ItemRandTableForItem)) {
			ItemRandTableForItem[i].maxnum =
				...
				(ItemRandTableForItem[i + 1].num - ItemRandTableForItem[i].num) * ITEM_GEN_RATE;
		...
		ItemRandTableForItem[i].rate = ItemRandTableForItem[i].maxnum / (double)ItemRandTableForItem[i].num;
	}
	/*for( i = 0; i < arraysizeof( ItemRandTableForItem); i ++) {
...
#define ADD_ICACHE_INGRED(nm, vl)                                                                                                                                                        \
	if (ITEM_tbl[i].itm.string[nm].string[0]) {                                                                                                                                          \
		icache[i].ingind[k] = ITEM_getAtomIndexByName(ITEM_tbl[i].itm.string[nm].string);                                                                                                \
		if (icache[i].ingind[k] < 0) {                                                                                                                                                   \
			...
			icache[i].ingval[k] = ITEM_tbl[i].itm.data[vl];                                                                                                                              \
			k++;                                                                                                                                                                         \
		}                                                                                                                                                                                \
	}
...
int ITEM_initItemAtom(char *fn) {
	...
	while (1) {
		...
		/* そうじゃないので、前の値が入っているだけ  (不是这样的话，只是保留前一个值)
		 それだけが要素  (仅此而已) */
...
/*
  ...
  base が100で min が0.7で max が1.3 だったら  (base=100, min=0.7, max=1.3时)
  70 ~ 130 に均一に分する  (均匀分配，暂时内部均匀)
  均一じゃなくしたりするかもしれないので  (也许不弄成均匀)｝
  アイテム用ルーチンとしてここに定義する  (作为物品用例程定义在这里) by ringo 1999Oct1
  1000分のmin_rate,max_rateで扱う  (用1000分之min_rate/max_rate处理)
  double やfloat は］ややこしいので選ばない  (double/float太复杂所以不选)
 */
static int
...
/*
 * 高いのにあわせる  (配合高的)
 * 返り値はいくつになっても構わない  (返回值多少都无所谓)
 * 並べる前にソートして並べて  (排序后排列)
 * tableに沿って    分配していく  (按照table分配)
 */
...
/*
  ペットのIDから、ランクの修整値を取り出す  (从宠物ID取出等级修正值)

  int *fixuse : 成績結数の数を取り、定義数を入れて返す  (取出成绩结数个数，放入定义数返回)

  int *fixatom : どの素材に対する修整なのか  (针对哪个素材的修正)
  int *baseup : 基  値に対する加算  (对基础值的加算)
  int *minadd : かけ算の小に対する加算  (对乘算的小数的加算)
  int *maxadd : かけ算に対する加算  (对乘算的加算)
...
/*
 * その合計を元にアイテムに当たったらFALSEを返す  (根据合计如果命中物品则返回FALSE)
 */
...
#define ADD_INGRED(nm, vl)                                                                                   \
	if (items[i].string[nm].string[0]) {                                                                     \
		...
		if (index < 0) {                                                                                     \
			print("\nfucking atom:[%s] for item id %d", items[i].string[nm].string, items[i].data[ITEM_ID]); \
			continue;                                                                                        \
		}                                                                                                    \
		for (j = 0; j < ingnum; j++) {                                                                       \
					...
					if (searchtable == 0) { // 合成
						sortedingtable[i] =
							...
							ITEM_randRange(sortedingtable[i],
										   (((1 * ItemRandTableForItem[tablenum].rate) * ITEM_MERGE_RANGEWIDTH_FORMIN) * (fixedmin / (double)ITEM_RANDRANGEDOM)) * ITEM_RANDRANGEDOM,
										   ((ItemRandTableForItem[tablenum].rate * ITEM_MERGE_RANGEWIDTH_FORMAX) * (fixedmax / (double)ITEM_RANDRANGEDOM)) * ITEM_RANDRANGEDOM);
#else
			...
			if (j == pet_fixuse) { // 如该素材成分与宠物特性不符
				...
				if (searchtable == 0) {
					...
					sortedingtable[i] = ITEM_randRange(sortedingtable[i],
													   ((1 / ItemRandTableForItem[tablenum].rate) * ITEM_MERGE_RANGEWIDTH_FORMIN) * ITEM_RANDRANGEDOM * 0.8 + fm_fix,
													   ItemRandTableForItem[tablenum].rate * ITEM_MERGE_RANGEWIDTH_FORMAX * ITEM_RANDRANGEDOM * 0.8 + fm_fix);
#else
					sortedingtable[i] = ITEM_randRange(sortedingtable[i],
													   ((1 / ItemRandTableForItem[tablenum].rate) * ITEM_MERGE_RANGEWIDTH_FORMIN) * ITEM_RANDRANGEDOM,
													   ItemRandTableForItem[tablenum].rate * ITEM_MERGE_RANGEWIDTH_FORMAX * ITEM_RANDRANGEDOM);
#endif
				} else {
					sortedingtable[i] = ITEM_randRange(sortedingtable[i],
		...
		for (i = 0; i < cnt; i++) {
			...
			if (pilenum <= 0) {
				...
				LogItem(
					...
					itemindexs[i],
#else
				ITEM_getInt(itemindexs[i], ITEM_ID), /* アイテム番号  (物品编号) */
#endif
					"mergedel(合成所删除的道具)",
...
#define MAXCANINSLAY 3
	...
	if (!strcmp(code, "\0") || !strcmp(code, "NULL"))
		return FALSE;
...
#define MAXFIXNUM 2
	...
	if ((buf2 = ITEM_getChar(index, ITEM_INGNAME0)) == NULL)
		return FALSE;
