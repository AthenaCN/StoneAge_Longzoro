			...
			for (i = 0; i < arraysizeof(ITEM_itemconfentries); i++) {
				...
				if (strlen(token) != 0) {
					...
					case ITEM_INTFUNC: {
						...
						itm.data[ITEM_itemconfentries[i].index] = intfunction(line,
...
char *ITEM_makeItemStatusString(int haveitemindex, int itemindex) {
	...
	if (ITEM_getInt(itemindex, ITEM_DAMAGECRUSHE) != ITEM_getInt(itemindex, ITEM_MAXDAMAGECRUSHE)) {
		flg |= 1 << 5;
	}
#endif
...
void ITEM_equipEffect(int index) {
	...
	static struct itmeffectstruct {
		...
		{ITEM_MODIFYMP, CHAR_WORKMAXMP, 0, 0, 1000, 0}, // MP は100まで  (MP 上限100)
