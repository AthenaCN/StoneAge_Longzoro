...
void BattleCommandDispach(int fd, char *command) {
	...
	if (strncmp(command, "U", 1) == 0) {
		...
		if (sscanf(command + 2, "%d", &iNum) != 1 || (iNum < -1 || iNum >= CHAR_MAXPETHAVE) || (CHAR_getInt(charaindex, CHAR_RIDEPET) == iNum)) {
			iNum = -1;
		}
		// shan end
...
#ifdef _PETSKILLBUG // Add By Syu 2002.0805
			} else if ((CHAR_getInt(petindex, CHAR_TRANSMIGRATION) < 1) && (iNum >= CHAR_getInt(petindex, CHAR_SLOT))) {
				// sprintf( szBuffer, "Command(%s)(自动)", CHAR_getUseName( petindex ) );
				CHAR_setWorkInt(petindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK);
#endif
			} else {
...
#endif
				if (petskillindex >= 0 && PETSKILL_Use(petindex, iNum, ToNo, NULL) == TRUE) {
...
void BATTLE_ActSettingSend(int battleindex) {
	...
	for (j = 0; j < 2; j++) {
		...
		for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
			pindex = pEntry[i].charaindex;
			// コマンド決定していたら  (命令已完成则)
			if (CHAR_getWorkInt(pindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_C_OK) {
				endBit |= 1 << k; // ビットを立てる  (置位)
			} else
				// 敵だったら絶対に立てる  (是敌人则必定置位)
	...
	for (j = 0; j < 2; j++) {
		...
		for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
			...
			if (CHAR_CHECKINDEX(pindex) == FALSE)
				continue;
			// 救援中のキャラには送らない  (救援中的角色不发送)
			if (CHAR_getWorkInt(pindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_RESCUE)
				continue;
			// プレイヤーだったら  (若是玩家)
			if (CHAR_getInt(pindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) { // コマンド送る  (发送命令)
...
BOOL BATTLE_MakeCharaString(
	...
	for (j = 0; j < 2; j++) {
		if (j == 1) {
			iOffset = SIDE_OFFSET; // 相手サイドの場合はプラスする  (对方阵营则加偏移)
		} else {
			iOffset = 0;
		}
		pEntry = BattleArray[battleindex].Side[j].Entry;
		for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
			...
			if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEFLG) & CHAR_BATTLEFLG_ULTIMATE)
				continue;
			flg = 0;
			if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_INIT)
				flg |= BC_FLG_NEW;
...
void BATTLE_BpSendToWatch(
	BATTLE *pBattle,  // 観戦バトルタスクのポインタ  (观战战斗任务指针)
	char *pszBcString // BCの種類  (BC种类)
) {
	...
	for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
		...
		if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_WATCHINIT) {
			flg |= BP_FLG_JOIN; // 新規参加  (新加入)
		}
		flg |= BP_FLG_PLAYER_MENU_OFF; // プレイヤーメニューを出さない  (不显示玩家菜单)
		// プレイヤーへ送る  (发送给玩家)
...
void BATTLE_CharSendAll(int battleindex) {
	...
	for (j = 0; j < 2; j++) {
		for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
			...
			if (BATTLE_CanMoveCheck(charaindex) == FALSE || CHAR_getFlg(charaindex, CHAR_ISDIE) == TRUE) {
