...
BOOL BATTLE_IsThrowWepon(int itemindex) {
	...
	if (itemtype == ITEM_BOOMERANG || itemtype == ITEM_BOW || itemtype == ITEM_BREAKTHROW || itemtype == ITEM_BOUNDTHROW) {
		return TRUE;
	} else {
		return FALSE;
	}
	...
	for (j = 0; j < 2; j++) {
		...
		for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
			...
			if (CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY) {
				...
				if (CHAR_getFlg(charaindex, CHAR_ISDIE) == TRUE || CHAR_getInt(charaindex, CHAR_HP) == TRUE) {
...
//*********************************************************
//
// 観戦用にバトルタスクを作成する (创建观战用战斗)
//
int BATTLE_CreateForWatcher(int charaindex, int topbattleindex)
//
//    戻り値 BATTLE_ERR (返回值 BATTLE_ERR)
//
//*********************************************************
{
	...
	battleindex = BATTLE_CreateBattle();
	if (battleindex < 0)
		return BATTLE_ERR_NOTASK; // タスクが できなかったエラー (创建失败则报错)

	// こちらの陣営はプレイヤー陣営であると認識する (本方阵营视为玩家阵营)
	BattleArray[battleindex].Side[0].type = BATTLE_S_TYPE_PLAYER;
	// あちらの陣営もプレイヤー陣営であると認識する (对方阵营也视为玩家阵营)
	BattleArray[battleindex].Side[1].type = BATTLE_S_TYPE_PLAYER;
	// リーダーを保持 (保存领队)
	BattleArray[battleindex].leaderindex = charaindex;
	// 戦闘のタイプ (战斗类型)
...
int BATTLE_CountEntry(
	...
	for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
		if (pEntry[i].charaindex != -1) { // キャラがいたら数える (有角色则计数)
			cnt++;
		}
	}
	return cnt;
...
int BATTLE_GetDuelPoint(
	int battleindex, // 戦闘インターフェース (战斗接口)
	...
	if (dpadd == 0) {
		dpadd = 0; // この場合は0 (此情况为0)
	} else if (dpadd < 0) {
		dpadd = min(-1, dpadd); // 小さくとも -1 はもらう (再小也取-1)
	} else {
		dpadd = max(1, dpadd); // 小さくとも 1 はもらう (再小也至少1)
	}
...
static int BATTLE_Finish(int battleindex) {
	...
	if (BATTLE_CHECKINDEX(battleindex) == FALSE)
		return BATTLE_ERR_BATTLEINDEX;
	if (BattleArray[battleindex].winside == 0 && BattleArray[battleindex].type == BATTLE_TYPE_P_vs_E && BattleArray[battleindex].WinFunc != NULL) {
...
//*********************************************************
{
	...
	for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
		charaindex = pEntry[i].charaindex;
		// キャラクタが いなかったら次へ (角色不存在则跳过)
		if( CHAR_CHECKINDEX( charaindex ) == FALSE )continue;
		// 敵キャラじゃなかったら次へ (非敌角色则跳过)
		if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) != CHAR_TYPEENEMY ){
			continue;
		}
		//     ここで とりあえずコマンドを 与える (此处暂时给予指令)
		// 今は攻撃とする (当前设为攻击)
...
static BOOL BATTLE_TimeOutCheck(int battleindex) {
	...
	for (j = 0; j < 2; j++) {
		...
		for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
			...
			if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_C_WAIT) {
				BATTLE_Exit(charaindex, battleindex);
				CHAR_DischargePartyNoMsg(charaindex);
				CHAR_talkToCli(charaindex, -1,
...
//*************************************************************
{
	...
	int i,
		...
		enemy,
		side,
		oldside = -3,  // デフにありえない値 (不可能的初始值)
		oldenemy = -3, // デフにありえない値 (不可能的初始值)
		armtype,
		move,
		per,
		ComboId = 1, // コンボID (连击编号)
		start = -1;
	...
	for (i = 0; i < entrynum; i++) {
					...
					if (strstr(arg, "合击")) {
						...
						for (k = 0; k < entrynum; k++) {																																	  // 战场上所有人回圈
							if (BATTLE_TargetCheck(battleindex, pEntryList[k].charaindex)) {																								  // 活着
								if ((lr < 10 && BATTLE_Index2No(battleindex, pEntryList[k].charaindex) < 10) || (lr >= 10 && BATTLE_Index2No(battleindex, pEntryList[k].charaindex) >= 10)) { // 同一队
									int l;
		...
		pEntryList[i].combo = 0; // 初期化 (初始化)

		if (start != -1) {				 // 連続技の 途中 (连击进行中)
			if (com != BATTLE_COM_ATTACK //   技 コマンドでない (非攻击指令)
				|| enemy != oldenemy	 // 攻撃してる敵が 同じでない (攻击目标不同)
				|| side != oldside		 // サイドが 違う (阵营改变)
				|| armtype == 1			 // 武器である (是武器)
				|| move == 0			 //   動けない (不能移动)
			) {
				start = -1;		// リセット (重置)
				oldside = side; // サイド記える (记录阵营)
			} else {
				// コンボID保存 (保存连击ID)
		...
		if (start == -1) {								 // 連続技を していない場合 (未在连击时)
			if (com == BATTLE_COM_ATTACK && armtype != 1 // 武器でない (非武器)
				&& move == 1							 //   動ける (能移动)
				&& RAND(1, 100) <= per) {				 // 連続技 である (可连击)
				start = i;
				oldenemy = enemy; // 攻撃してる敵を記える (记录攻击目标)
				oldside = side;	  // サイド記える (记录阵营)
				ComboId++;
			}
		}
...
//*************************************************************
//
//  コンボが 成立できるかチェック (检查连击是否成立)
// 後の 値がコンボID で ない なら FALSE (后续值非同一连击则FALSE)
//
static BOOL ComboCheck2(
	BATTLE_CHARLIST *pEntryList, // エントリスト (入场列表)
	int nownum,					 // 現在の位置 (当前位置)
	int entrynum				 // エントリ 数 (入场数)
	)
//
// コンボIDで きるなら TRUE (连击成立则TRUE)    TRUE
//       できない  FALSE (否则FALSE)
//
//*************************************************************
{
	...
	for (i = nownum + 1; i < entrynum; i++) {
		...
		// コンボID が 違うと 終了 (连击ID不同则结束)
		if (ComboId != pEntryList[i].combo)
			break;

		// 戦闘に 参加していないなら次へ (未参战则跳过)
		if (CHAR_getWorkInt(charaindex, CHAR_WORKBATTLEMODE) == 0)
			break;
...
void BATTLE_UltimateExtra(
	...
	if (CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
		...
		if (CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPET) {
			...
			/*
					// ペットがいなかったら戦闘から 除外させる (宠物不在则从战斗移除)
					BATTLE_PetDefaultExit( enemyindex, battleindex );
			*/
...
void BATTLE_NormalDeadExtra(
	int battleindex, // 戦闘インターフェース (战斗接口)
	int charaindex,	 // 倒したキャラのインデックス (被击倒角色的索引)
	int enemyindex	 // 倒されたキャラのインデックス (被击倒角色的索引)
	)
//
//
//*************************************************************
		...
		for (i = 0; i < BATTLE_ENTRY_MAX; i++) {
			...
			if (pEntryEnemy[i].flg & BENT_FLG_ULTIMATE) {
				if (CHAR_getInt(enemyindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER) {
					int dpadd, dpnow;
					dpnow = CHAR_getInt(enemyindex, CHAR_DUELPOINT);
					dpadd = dpnow * DUELPOINT_RATE; // その分の もらえる (取其相应数值)
					dpnow -= dpadd;
					CHAR_setWorkInt(enemyindex, CHAR_WORKGETEXP,
									CHAR_getWorkInt(enemyindex, CHAR_WORKGETEXP) - dpadd);
...
static int BATTLE_Battling(int battleindex) {
	...
	for (i = 0; i < entrynum; i++) {
		...
		case BATTLE_COM_S_MIGHTY:
			...
			gBattleDuckModyfy = CHAR_GETWORKINT_HIGH(charaindex, CHAR_WORKBATTLECOM3); //   逃げられない共通へ  (无法闪避，统一)
			break;
		}
		...
		case BATTLE_COM_S_RETRACE:
			...
			if ((COM == BATTLE_COM_S_BECOMEFOX) && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_MISS)																								   // 没有miss
				...
				&& (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_ALLGUARD) && (Battle_Attack_ReturnData_x.Battle_Attack_ReturnData != BATTLE_RET_ARRANGE) && (BATTLE_TargetCheck(battleindex, defNo)) // 还活着
...
// Terry add 2001/11/05
		...
		case BATTLE_COM_ITEM:
...
#if 1 // Robin fix 战斗不可换防具饰品
			{
				...
				if (itemtype == ITEM_HELM || itemtype == ITEM_ARMOUR || itemtype == ITEM_BRACELET
					//|| itemtype == ITEM_MUSIC
					|| itemtype == ITEM_NECKLACE || itemtype == ITEM_RING || itemtype == ITEM_BELT || itemtype == ITEM_EARRING || itemtype == ITEM_NOSERING || itemtype == ITEM_AMULET
					//|| itemtype == CHAR_HEAD
				) {
		...
		case BATTLE_COM_COMBO:
			...
			aAttackList[0] = EntryList[i].num; // リストに加える (加入列表)
			i++;							   // 後の値から (从下一个开始)
			k = 1;
