...
void ITEM_useStatusRecovery_Battle(
	int charaindex,	  // 対象のインターフェース  (对象接口)
	int toNo,		  // 対象の番号  (目标编号)
	int haveitemindex // アイテムの番号  (道具编号)
) {
	int i;
...
void ITEM_useRefresh_Effect(int charaindex, int toindex, int haveitemindex) {
	...
	CHAR_setItemIndex(charaindex, haveitemindex, -1);
	CHAR_sendItemDataOne(charaindex, haveitemindex); /* アイテム更新  (道具更新) */
...
void ITEM_useAddexp_Effect(int charaindex, int toindex, int haveitemindex) {
	...
	{
		int point;
		point = CHAR_getWorkInt(charaindex, CHAR_WORKITEM_ADDEXP) * (int)(CHAR_getWorkInt(charaindex, CHAR_WORKITEM_ADDEXPTIME) / 60);
		if (point >= 72000) {
...
void ITEM_useFirecracker_Battle(int charaindex, int toindex, int haveitemindex) {
	...
	if ((buff1 = ITEM_getChar(itemindex, ITEM_ARGUMENT)) == NULL)
		return;
