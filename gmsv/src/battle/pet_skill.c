...
/*========================================================================
 * ペットの  種類についてのソース  宠物的各式技能  (宠物的各式技能)
 *========================================================================*/
...
typedef struct tagPetskill_PetskillFunctionTable {
	char *functionname;		/* 関数名  (函数名) */
	PETSKILL_CALLFUNC func; /* 呼び出される関数  (调用函数) */
	int hash;				/*  hash */
	int status;				/* ペットの状態  (宠物状态) */
} PETSKILL_PetskillFunctionTable;

/* ペットスキル関数テーブル  (宠物技能函数表) */
...
BOOL PETSKILL_initPetskill(char *filename) {
	...
	/*  内部の効な値が何値あるかどうか調べる  (检查内部有效值有几个)    */ // 读曲宠技总数
...
//	print("\n 最大宠技ID = %d\n", max_skillid);
	...
	PETSKILL_petskill = allocateMemory(sizeof(struct tagPetskill) * PETSKILL_petskillnum);
...
/*------------------------------------------------------------------------
 * Petskillの修正ファイル  読み  込み  (读取Petskill修正文件)
 *-----------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------
 * PETSKILL_IDから関数を知る関数  (从PETSKILL_ID得知函数)
 * 返り値  (返回值)
 * 戻り  : 関数  (返回: 函数)
 * 無し  : -1  (无: -1)
 *-----------------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * ペットの関数からポインタを返す  (从宠物函数返回指针)
 * 引数  (参数)
 *  name        char*       関数の名前  (name char* 函数名)
 * 返り値  (返回值)
 *  関数へのポインタ、ない場合はNULL  (函数指针，无则NULL)
 ------------------------------------------------------------*/
...
int PETSKILL_Guardian(
	...
	if ((pszP = strstr(pszOption, "COM:")) != NULL && strstr(pszP + 4, "防御") != NULL) {
