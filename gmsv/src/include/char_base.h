...
typedef enum {
	...
	CHAR_VARIABLEAI = CHAR_LUCK,				  /*  ペットの復活変化をしない場合に使用する  (宠物复活无变化时使用)   */
	...
	CHAR_ALLOCPOINT = CHAR_LEVELUPPOINT,		  /* レベルアップ時の割り振りポイント(ペット用)  (升级时分配点数(宠物用)) */
	...
	CHAR_PETMAILEFFECT = CHAR_DUELSTWINCOUNT,	  /* ペットメールの表示のエフェクト番号  (宠物邮件显示的特效编号) */
	CHAR_PETSENDMAILCOUNT = CHAR_TALKCOUNT,		  /* ペットがメールを送った回数  (宠物发送邮件的次数) */
	CHAR_PETRANK = CHAR_LASTTIMESETLUCK,		  /* ペットのランク(ペット用のインデックス用  (宠物的等级(宠物用索引)) */
