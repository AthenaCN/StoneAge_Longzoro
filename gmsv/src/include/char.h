...
typedef enum {
	CHAR_WALKSUCCESSED,	  /* 成功  (成功)      */
	CHAR_WALKSYSTEMERROR, /* システムエラーのindexチェックに  (系统错误的索引检查中)
						   * ひかかかったとか  (被卡住等) */
	CHAR_WALKEXTEND,	  /* 区域オーバーして歩こうとした［NPCにのみ  (试图越区行走［仅NPC) */
	CHAR_WALKHITOBJECT,	  /*  物にぶつかって｝歩けなかった  (撞到障碍物而无法行走)    */
	CHAR_WALKPREWALK,	  /*  prewalkイベントで歩けなかった( 石化 )  (prewalk事件中无法行走(石化)) */
	CHAR_WALKDIE,		  /*  死んでいて歩けない  (死亡无法行走)  */
	CHAR_WALK1357,		  /*  移動方向に歩こうとして歩けなかった  (试图朝移动方向行走但失败)  */
} CHAR_WALKRET;

/*====================キャラの移動に関する関数  (角色移动相关函数)====================*/
...
typedef enum {
	CHAR_WINDOWTYPE_RETURNTOELDER = -1, /*  城へ戻るウィンドウ  (返回城中窗口)    */
	CHAR_WINDOWTYPE_RESURRECTION = -2,	/*  復活するウィンドウ  (复活窗口)    */

	CHAR_WINDOWTYPE_SELECTBATTLE = 1,	   /* する戦いを選択するウィンドウ  (选择战斗的窗口) */
	CHAR_WINDOWTYPE_SELECTDUEL = 2,		   /* するDUELを選択するウィンドウ  (选择决斗的窗口) */
	CHAR_WINDOWTYPE_SELECTTRADECARD = 3,   /* カードを選択するウィンドウ  (选择卡片的窗口) */
	CHAR_WINDOWTYPE_SELECTPARTY = 4,	   /* パーティを選択するウィンドウ  (选择队伍的窗口) */
	CHAR_WINDOWTYPE_SELECTBATTLEWATCH = 5, /* 観戦を選択するウィンドウ  (选择观战的窗口) */
	CHAR_WINDOWTYPE_MICMESSAGE = 6,		   /* MICNPCを呼び出して貰うウィンドウ  (呼出MICNPC的窗口) */

	// CoolFish: Trade 2001/4/18
	CHAR_WINDOWTYPE_SELECTTRADE = 7, /* 交易 Window */

	CHAR_WINDOWTYPE_SELECTRENAMEITEM_PAGE1 = 10,			/* 名前を変更するアイテムを選択するウィンドウ  (选择改名物品的窗口) */
	CHAR_WINDOWTYPE_SELECTRENAMEITEM_PAGE2 = 11,			/* 名前を変更するアイテムを選択するウィンドウ  (选择改名物品的窗口) */
	CHAR_WINDOWTYPE_SELECTRENAMEITEM_PAGE3 = 12,			/* 名前を変更するアイテムを選択するウィンドウ  (选择改名物品的窗口) */
	CHAR_WINDOWTYPE_SELECTRENAMEITEM_PAGE4 = 13,			/* 名前を変更するアイテムを選択するウィンドウ  (选择改名物品的窗口) */
	CHAR_WINDOWTYPE_SELECTRENAMEITEM_RENAME = 14,			/* 名前を付けるウィンドウ  (命名窗口) */
	CHAR_WINDOWTYPE_SELECTRENAMEITEM_RENAME_ATTENTION = 15, /* 名前を付けるウィンドウ  (命名窗口) */
...
EXTERN int EnemyMoveNum; /*  前に戦った相手の数  (此前交战的对手数量) */
extern char *DebugFunctionName;
extern int DebugPoint;

#define DB_DUELPOINT "db_duel"			// デュエルポイントデータベース  (决斗点数数据库)
#define DB_ADDRESSBOOK "db_addressbook" // アドレスブックデータベース  (通讯录数据库)
...
typedef struct tagCHAR_effectsetting {
	int floor;								//	フロア  (楼层)
	int effect;								//	効果番号  (效果编号)
	int level;								//	効果のレベル［戦の強さ［  (效果等级［战斗强度［)
	int sendflg;							//	送るエフェクトを送ったかどうか［  (是否发送了效果显示［)
	char month[CHAR_EFFECT_SETTINGBUFFER];	//	発動する月  (生效月份)
	char day[CHAR_EFFECT_SETTINGBUFFER];	//	発動する日  (生效日期)
	char hour[CHAR_EFFECT_SETTINGBUFFER];	//	発動する時間  (生效时间)
	char min[CHAR_EFFECT_SETTINGBUFFER];	//	発動する分  (生效分钟)
	char expire[CHAR_EFFECT_SETTINGBUFFER]; //	発動している強さ［(  (正在生效的强度［)
