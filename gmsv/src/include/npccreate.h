...
typedef enum {
	NPC_CREATEFLOORID,		  /*  フリーID  (空闲ID)    */
	NPC_CREATEBORNLEFTUPX,	  /*  左上のX座標  (左上X坐标)   */
	NPC_CREATEBORNLEFTUPY,	  /*  左上のY座標  (左上Y坐标)   */
	NPC_CREATEBORNRIGHTDOWNX, /*  右下のX座標  (右下X坐标)   */
	NPC_CREATEBORNRIGHTDOWNY, /*  右下のY座標  (右下Y坐标)   */

	NPC_CREATEMOVELEFTUPX,	  /*  左上のX座標  (左上X坐标)   */
	NPC_CREATEMOVELEFTUPY,	  /*  左上のY座標  (左上Y坐标)   */
	NPC_CREATEMOVERIGHTDOWNX, /*  右下のX座標  (右下X坐标)   */
	NPC_CREATEMOVERIGHTDOWNY, /*  右下のY座標  (右下Y坐标)   */

	NPC_CREATEDIR,			   /*  纏める時の方向  (汇总时的方向)    */
	NPC_CREATEBASEIMAGENUMBER, /*  template の修正を書き直すID  (重写template修正的ID)
									 グラフィックNo    */
	NPC_CREATETIME,			   /* 時間  (时间) */
	NPC_CREATEBORNNUM,		   /*    その場所に出現させる数  (使其出现的数量)    */

	NPC_CREATEENEMYNUM, /*  敵の数  (敌人数量)  */
	NPC_CREATEBOUNDARY,
	NPC_CREATEDATE,
...
typedef enum {
	NPC_CREATENAME, /* templateを書き直すかもしれない前  (可能要重写template的前面) 蟆*/
	NPC_CREATECHARNUM,
} NPC_CREATECHAR;
