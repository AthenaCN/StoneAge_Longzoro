...
typedef enum {
	...
	PETSKILL_FIELD_MAP,	   /* 通常マップのみ  (仅普通地图) */

} PETSKILL_FIELDTYPE;

typedef enum {
	PETSKILL_TARGET_MYSELF,				 /* 自分のみ  (仅自己) */
	PETSKILL_TARGET_OTHER,				 /* 敵の魔法  自分以外)  (敌方魔法，除自己以外) */
	PETSKILL_TARGET_ALLMYSIDE,			 /*     全   (全部)   */
	PETSKILL_TARGET_ALLOTHERSIDE,		 /* 相手側全  (对方全部)   */
	PETSKILL_TARGET_ALL,				 /* 全員  (全员) */
	PETSKILL_TARGET_NONE,				 /* 誰も選べない［使った時の時  (无法选择对象［使用时的场合) */
	PETSKILL_TARGET_OTHERWITHOUTMYSELF,	 /* 敵の魔法  自分以外以外)  (敌方魔法，自己以外) */
	PETSKILL_TARGET_WITHOUTMYSELFANDPET, /* 自分とペット対象  (自己和宠物为对象) */
} PETSKILL_TARGETTYPE;
...
typedef enum {
	PETSKILL_NAME,	   /*      */
	PETSKILL_COMMENT,  /* コメント  (注释) */
	PETSKILL_FUNCNAME, /* 関数  (函数)   */
	PETSKILL_OPTION,   /* オプション  (选项) */
...
#define PETSKILL_ID_GBREAK 0		 // ガードブレイク  (破防)
#define PETSKILL_ID_RENZOKU 1		 //   連続攻撃  (连续攻击)
#define PETSKILL_ID_GUARDIAN 2		 // ガーディアン  (守护)
#define PETSKILL_ID_CHARGE 3		 // チャージ攻撃  (蓄力攻击)
#define PETSKILL_ID_ICHIGEKI 100	 // 一撃の殺  (一击必杀)
#define PETSKILL_ID_POWERBALANCE 110 //     の先頭  (的开头)

//**********************************************************************
//
//  檗  -- 敵を倒すのみの攻撃  (石化 -- 仅对敌攻击)
//
...
//*******************************************************
// 大地属性(土属性と相手の    からの  攻撃)  (大地属性、土属性与对方的攻击)
//
