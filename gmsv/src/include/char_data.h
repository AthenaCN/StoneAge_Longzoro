...
typedef struct tagLevelUpPattern {
	...
	int hpupminpoint; /*  hp の上がる   限界値  (hp可提升的上限值)*/
	int hpupmaxpoint; /*  hp の上がる   限界値  (hp可提升的上限值)*/

	int needexp; /*    必要な経験値    (所需经验值) */
#else
	int uppoint[5];
#endif
} LevelUpPattern;

/*====================初期値・レベルアップ関係==================== (初始值/升级相关) */
BOOL CHAR_getDefaultChar(Char *nc, int imagenumber);

/*====================グラフィック番号から関係==================== (从图像编号关系) */
int CHAR_getNewImagenumberFromEquip(int basebaseimagenumber,
									ITEM_CATEGORY category);
/*====================  番号と名前   ->   名前関係==================== (编号与名字→名字关系) */
