...
typedef enum {
	...
	E_T_FIREAT,
	E_T_WINDAT,
	E_T_POISON,	   /*   歩くたびにダメージ  (每走一步受伤害)          */
	E_T_PARALYSIS, /* しびれ｝1の移動ができない  (麻痹，1格内无法移动)［ */
	E_T_SLEEP,	   /* 眠り［移動できない  (睡眠，无法移动)            */
	E_T_STONE,	   /* 石［移動できない  (石化，无法移动)              */
	E_T_DRUNK,	   /* 酔う［命中が下がる  (醉酒，命中下降)     */
	E_T_CONFUSION, /* 混乱［攻撃を繰り返す  (混乱，反复攻击)     */
	E_T_PETSKILL1,
	E_T_PETSKILL2,
	E_T_PETSKILL3,
	E_T_PETSKILL4,
...
typedef enum {
	...
	ENEMY_DUELPOINT,
	ENEMY_STYLE,
	ENEMY_PETFLG, /* ペットになるかどうか  (是否成为宠物) */
...
typedef enum {
	GROUP_ID,
	GROUP_APPEARBYITEMID,	 /* このアイテムを持っていたら出現する -1 は無視  (持有此物品时出现，-1无视)*/
	GROUP_NOTAPPEARBYITEMID, /* このアイテムを持っていたら出現しない -1 は無視  (持有此物品时不出现，-1无视)*/
	ENEMY_ID1,
	ENEMY_ID2,
	ENEMY_ID3,
	ENEMY_ID4,
