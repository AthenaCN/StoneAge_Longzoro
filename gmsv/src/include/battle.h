...
typedef struct _Battle {
	...
	int battleindex; /* バトル番号  (战斗编号) */
	int mode;		 /* バトルモード  (战斗模式) */
	int type;		 /* 戦タイプ (0:通常)(1:DUEL)(2:ボス戦)  (战斗类型) */
	...
	int battlemap;
#endif
	struct _Battle *pNext;	 // 次の観戦モード  (下一个观战模式)
	struct _Battle *pBefore; // 前の観戦モード  (上一个观战模式)
} BATTLE;

#define BATTLE_FLG_FREEDP (1 << 0)	  // 減っても減らない  (即使减少也不减)
#define BATTLE_FLG_CHARALOST (1 << 1) // キャラが死んだり逃げたりしている  (角色死亡或逃跑中)

enum {					  // 次のfield_attに使える値  (用于下一个field_att的值)
	BATTLE_ATTR_NONE = 0, // 属性なし  (无属性)
	BATTLE_ATTR_EARTH,	  // 哗
	BATTLE_ATTR_WATER,	  //
	BATTLE_ATTR_FIRE,	  // 绍
	BATTLE_ATTR_WIND,	  // 氘
...
#define TARGET_SIDE_0 20 // 相手側サイドㄟ属性  (对手方阵营)
#define TARGET_SIDE_1 21 // 自分側サイドㄠ属性  (己方阵营)
#define TARGET_ALL 22	 // 全属性  (全部)
...
extern char szBadStatusString[];				  // ステータス異常用データ  (状态异常用数据)
extern int gWeponType;							  // バトルの武器パターン  (战斗武器类型)
...
int BATTLE_PetDefaultEntry(
	int charaindex,	 // ペットを持っているプレイヤーの  (持有宠物的玩家的)
