...
typedef struct tagConfig
{
	...
	char configfilename[32];  /* config ファイル (配置文件)   */
	unsigned char debuglevel; /* デバッグレベル (调试级别) */

	unsigned int usememoryunit;	   /* メモリの使用単位 (内存使用单位) */
	unsigned int usememoryunitnum; /* メモリの使用数 (内存使用数量) */

	char asname[32];		   /* アカウントサーバの名前 (账号服务器名称) */
	unsigned short acservport; /* アカウントサーバのポート (账号服务器端口)  */
	char acpasswd[32];		   /* アカウントサーバへのパスワード (账号服务器密码) */
	char gsnamefromas[32];	   /*
								* アカウントサーバから受け取る (从账号服务器接收)
								* ゲームサーバとしての名前 (作为游戏服务器的名称)
								*/
	...
	unsigned short port; /* サーバの接続ポート (服务器连接端口) */

	int servernumber;		/* ゲームサーバの番号 (游戏服务器编号) */
	int reuseaddr;			/* Address already used... が出ている場合に (出现地址已被占用时) */
	int do_nodelay;			/* TCP_NODELAY にするかどうか (是否使用 TCP_NODELAY) */
	int log_write_time;		/* 書き込みログを取るかどうか (是否记录写入日志) */
	int log_io_time;		/* I/O系の時間を計るかどうか (是否统计 I/O 时间) */
	int log_game_time;		/* ゲーム処理系の時間を計る (统计游戏处理时间) */
	int log_netloop_faster; /* netloop_faster のログ (netloop_faster 日志) */
	int saacwritenum;		/* アカウントサーバに write するか (是否写入账号服务器) */
	int saacreadnum;		/* アカウントサーバから dispatch を読むか (是否读取账号服务器的 dispatch) */

	unsigned short fdnum;	   /* どれだけコネクションを使うか (select 使用的连接数) */
	unsigned int othercharnum; /* その他のキャラの数 (其他角色数量)  */

	unsigned int objnum;	 /* オブジェクトの数 (对象数量) */
	unsigned int petcharnum; /* ペットの数 (宠物数量)    */
	unsigned int itemnum;	 /* アイテムの数 (物品数量) */
	unsigned int battlenum;	 /* バトルの数 (战斗数量)    */
	unsigned int battleexp;	 /* バトル経験 (战斗经验)    */

	char topdir[64]; /* トップディレクトリ (顶级目录)  */

	char mapdir[64];		  /* マップディレクトリ (地图目录)  */
	char maptilefile[64];	  /* マップ読み込みファイル (地图配置文件)  */
	char battlemapfile[64];	  /* バトルマップ読み込みファイル (战斗地图配置文件)  */
	char itemfile[64];		  /* アイテム読み込みファイル (物品配置文件)  */
	char invfile[64];		  /* インベントリ読み込みファイル (背包配置文件)  */
	char appearfile[64];	  /* 出現読み込みファイル (出现配置文件)  */
	char titlenamefile[64];	  /* タイトルファイル (标题文件)   */
	char titleconfigfile[64]; /* タイトル読み込みファイル (标题配置文件)   */
	char encountfile[64];	  /* エンカウント読み込みファイル (遇敌配置文件)   */
	char enemybasefile[64];	  /* 敵ベース読み込みファイル (敌人基础配置文件)   */
	char enemyfile[64];		  /* 敵読み込みファイル (敌人配置文件)   */
	char groupfile[64];		  /* グループ読み込みファイル (队伍配置文件)   */
	char magicfile[64];		  /* マジック読み込みファイル (魔法配置文件)   */
	...
	char petskillfile[64]; /* ペットスキル読み込みファイル (宠物技能配置文件)   */
	char itematomfile[64]; /* アイテムのアトムファイル (物品原子配置文件) */
	char effectfile[64];   /* 効果読み込みファイル (效果配置文件)  */
	char quizfile[64];	   /* クイズ読み込みファイル (问答配置文件)  */

	char lsgenlog[64]; /* サーバの lsgen アウトプットファイル (服务器 lsgen 输出文件)  */

	char storedir[64]; /* ストアドディレクトリ (存档目录)    */
	char npcdir[64];   /* NPCの読み込みファイルを置くディレクトリ (存放 NPC 配置文件的目录)   */
	...
	char logconfname[64];	  /*
							   * ログ読み込みファイル (日志配置文件)
							   */
	char chatmagicpasswd[64]; /* チャット用パスワード (聊天用密码) */

#ifdef _STORECHAR
	char storechar[64];
#endif

	unsigned int chatmagiccdkeycheck; /* チャット用に CDKEY をチェックするか (聊天时是否校验 CDKEY) */

	unsigned int filesearchnum;		/* ファイルを検索するファイルの数 (文件搜索数量) */
	unsigned int npctemplatenum;	/* NPCのテンプレートファイルの数 (NPC 模板文件数量) */
	unsigned int npccreatenum;		/* NPCの作成ファイルの数 (NPC 创建文件数量) */
	unsigned int walksendinterval;	/* 移動の送信間隔 (移动发送间隔) */
	unsigned int CAsendinterval_ms; /* CA 送信間隔 (ms) (CA 发送间隔) */
	unsigned int CDsendinterval_ms; /* CD 送信間隔 (ms) (CD 发送间隔) */
	unsigned int Onelooptime_ms;	/* 1ループにかかる時間 (单次循环耗时) */
	unsigned int Petdeletetime;		/* ペットを消す時間 (宠物删除时间) */
	unsigned int Itemdeletetime;	/* アイテムを消す時間 (物品删除时间) */
	/* ログインのキャラのセーブをする間隔 (登录角色存档间隔) */
	unsigned int CharSavesendinterval;
	unsigned int addressbookoffmsgnum; /*
										* アドレスブックにオフライン (地址簿中的离线)
										* メッセージを (消息)
										* 残せる数 (可留存的数量)
										*/
	...
	unsigned int loghour;				/*
										 * ログを書き込む時間（時間） (日志写入时间（小时）)
										 */
	unsigned int battledebugmsg;		/*
										 * バトルのデバッグメッセージを出すかどうか (是否输出战斗调试消息)
										 */
...
#endif
/*
 *  コンフィグファイルを開くための構造体 (用于打开配置文件的结构体)
 *  xxxx=yyyy の形式のものを開く (打开 xxxx=yyyy 形式的文件)
 */

typedef struct tagReadConf
{
	char name[32]; /* xxxx に入る値 (存放入 xxxx 的值) */

	/* 第2トークンが NULL の場合は代入しない (第二段为 NULL 时不赋值) */
	char *charvalue; /* yyyy を代入する領域へのポインタ (存储 yyyy 的缓冲区指针) */
	size_t charsize; /* charvalue のサイズ (charvalue 的大小) */

	/*
	 *     設定値=の際 "ON" だったら intvalue に 1 を代入する (当值为 ON 时向 intvalue 代入 1)
	 * それ以外は atoi の結果 (否则使用 atoi 的结果)
	 */
	void *value; /* yyyy を変換して代入する領域へのポインタ (转换 yyyy 后存入的缓冲区指针) */
	CTYPE valuetype;
} ReadConf;
...
/*------------------------------------------------------------
 * memoryunitnum を取る (获取 memoryunitnum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * アカウントサーバから受け取ったゲームサーバの名前を取る (获取从账号服务器接收的游戏服务器名称)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned short
...
/*------------------------------------------------------------
 * reuseaddr の値を取る (获取 reuseaddr 的值)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned short
...
/*------------------------------------------------------------
 * saacwritenum を取る (获取 saacwritenum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *   int
...
/*------------------------------------------------------------
 * saacwritenum をセットする (设置 saacwritenum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *   int
...
/*------------------------------------------------------------
 * saacreadnum を取る (获取 saacreadnum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *   int
...
/*------------------------------------------------------------
 * saacreadnum をセットする (设置 saacreadnum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *   int
...
/*------------------------------------------------------------
 * petcharanum を取る (获取 petcharanum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * othercharnum を取る (获取 othercharnum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * maptilefile を取る (获取 maptilefile)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * battlemapfile を取る (获取 battlemapfile)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * titlenamefile を取る (获取 titlenamefile)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * titleconfigfile を取る (获取 titleconfigfile)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * encountfile を取る (获取 encountfile)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * enemybasefile を取る (获取 enemybasefile)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * lsgenlogfile を取る (获取 lsgenlogfile)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * NPC 用のディレクトリを返す関数 (返回 NPC 目录的函数)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * デバッグチャット用での CDKEY チェックをするかどうかを取る (获取是否在调试聊天中校验 CDKEY)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  char*
...
/*------------------------------------------------------------
 * filesearchnum を取る (获取 filesearchnum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * npctemplatenum を取る (获取 npctemplatenum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * npccreatenum を取る (获取 npccreatenum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * walksendinterval を取る (获取 walksendinterval)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * walksendinterval をセットする (设置 walksendinterval)
 * 引数 (参数)
 *  unsigned int 	interval	時間（ミリ秒） (时间（毫秒）)
 * 戻り値 (返回值)
 *  void
 ------------------------------------------------------------*/
...
/*------------------------------------------------------------
 * CAsendinterval を取る (获取 CAsendinterval)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * CAsendinterval をセットする (设置 CAsendinterval)
 * 引数 (参数)
...
/*------------------------------------------------------------
 * CDsendinterval を取る (获取 CDsendinterval)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * CDsendinterval をセットする (设置 CDsendinterval)
 * 引数 (参数)
...
/*------------------------------------------------------------
 * Onelooptime を取る (获取 Onelooptime)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * Onelooptime をセットする (设置 Onelooptime)
 * 引数 (参数)
 *  interval		unsigned int
 * 戻り値 (返回值)
 * void
...
/*------------------------------------------------------------
 * Petdeletetime を取る (获取 Petdeletetime)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * Petdeletetime をセットする (设置 Petdeletetime)
 * 引数 (参数)
 *  interval		unsigned int
 * 戻り値 (返回值)
 * void
...
/*------------------------------------------------------------
 * Itemdeletetime を取る (获取 Itemdeletetime)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * Itemdeletetime をセットする (设置 Itemdeletetime)
 * 引数 (参数)
 *  interval		unsigned int
 * 戻り値 (返回值)
 * void
...
/*------------------------------------------------------------
 * CharSavesendinterval を取る (获取 CharSavesendinterval)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * CharSavesendinterval をセットする (设置 CharSavesendinterval)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * Addressbookoffmsgnum を取る (获取 Addressbookoffmsgnum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * Protocolreadfrequency を取る (获取 Protocolreadfrequency)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * Allowerrornum を取る (获取 Allowerrornum)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int
...
/*------------------------------------------------------------
 * バトルのデバッグメッセージを出すか (是否输出战斗调试消息)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int 出すかどうか (是否输出)
 ------------------------------------------------------------*/
...
/*
 * Config のデフォルト値を設定する関数 (设置 Config 默认值的函数)
 * 引数 (参数)
 *  argv0   char*   コマンドライン引数 (命令行参数)
 */
void defaultConfig(char *argv0)
{
	char *program; /* program を取得するため (用于获取程序名) */
	...
	if (program == NULL)
		...
		program++; /* "/" の後ろに進める為に ++ する (跳过 "/" 前进到下一字符) */
...
/*
 * ポインタに型に応じた値を代入する関数 (按类型向指针赋值)
 * 引数 (参数)
 *  to      void*   値を代入するポインタ (要赋值的指针)
 *  type    CTYPE   to の型を設定する (设置 to 的类型)
 *  value   double  to に代入する値 (代入 to 的值)
 * 戻り値 (返回值)
 *  なし (无)
 */
 ...
 * 戻り値 (返回值)
 *      TRUE(1)     成功 (成功)
 *      FALSE(0)    失敗      -> ファイルのオープンに失敗した (失败：文件打开失败)
 ------------------------------------------------------------*/
BOOL readconfigfile(char *filename)
{
	FILE *f = NULL;
	char linebuf[256];			/* 行を読むためのバッファ (读取行的缓冲区) */
	int linenum = 0;			/* 行数を数える (统计行数) */
	char realopenfilename[256]; /* ホスト別に open するファイル (按主机打开的配置文件)  */
	...
	if (f == NULL) {
		f = fopen(filename, "r"); /* ファイルのオープン (打开文件) */
		if (f == NULL) {
			print("Can't open %s\n", filename);
			return FALSE;
		}
	...
	while (fgets(linebuf, sizeof(linebuf), f)) {
		char firstToken[256]; /* 第1トークン (第一段)  */
		int i;				  /* ループ数 (循环计数) */
		int ret;			  /* リターンコード (返回值) */
...
/*------------------------------------------------------------
 * エンコードキーをセットする (设置编码密钥)
 * 引数 (参数)
 *  なし (无)
 * 戻り値 (返回值)
 *  unsigned int キーを返す (返回密钥)
------------------------------------------------------------*/
...
/*------------------------------------------------------------
* アカウントサーバに書き込む設定の単位をセットする (设置写入账号服务器的数据单位)
* 引数 (参数)
*  なし (无)
* 戻り値 (返回值)
*  unsigned int キーを返す (返回密钥)
...
BOOL LoadRacepetfile(void)
{
	...
	while (1) {
		...
		getStringFromIndexWithDelim(line, "|", 5, token, sizeof(token));
		if (atoi(token) < 0 || atoi(token) > 140)
			return FALSE;
