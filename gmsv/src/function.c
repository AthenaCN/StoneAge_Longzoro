...
static CorrespondStringAndFunctionTable
	correspondStringAndFunctionTable[] =
		{
			/* 以下はサーバーコードで定義されている関数だ。NPCもこれをつかうときがある (以下为服务器代码定义的函数，NPC 有时也会用到) */
			...
			/* 以下はアイテムを使用するとき用の関数だ (以下是使用物品时的函数) */
