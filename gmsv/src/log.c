...
/*
 *
 * ログファイルの種類はappend するものとしないものがある。 (日志文件的种类分为需要追加的和不需要的)
 * appendするものは最初にファイルを開いておく。 (追加类型在启动时先打开文件)
 * そうでないものは書きこみのたびにfopen(..,"w")する (非追加类型每次写入时用fopen(..,"w")打开)
 * by ringo
 */
...
struct tagLogconf
{
	...
	char filename[256];
	FILE *f;
	BOOL append; /* append するか、書きこみのたびにSEEK_SETするか (是追加写入还是每次写入时SEEK_SET) */
...
/*------------------------------------------------------------
 * ログ設定ファイルを読んで file を開く (读取日志配置文件，打开file)
 * 引数 (参数)
 *  filename        char*       ログ設定ファイル (日志配置文件)
 * 返り値 (返回值)
 *  FALSE   は重要な失敗である。 (FALSE 为重大失败)
 ------------------------------------------------------------*/
...
void LogItem(
	char *CharName, /* キャラクタ名 (角色名)*/
	char *CharID,	/* キャラクタID (角色ID) */
	int ItemNo,		/* アイテム番号 (物品编号) */
	char *Key,		/* キーワード (关键词) */
	int floor,		/* 座標 (坐标)*/
	int x,
	int y,
...
void LogTensei(
	...
	char *Key,	  /* キーワード (关键词) */
	int level,	  // レベル (等级)
	int transNum, // 転生回数 (转生次数)
	int quest,	  // クエスト数 (任务数)
	int home,	  // 名声 (名声)
	int item,	  // 加えたアイテムの個数 (增加的物品个数)
	int pet,	  // 加えたペットの個数 (增加的宠物个数)
	int vital,	  // 化前Vital (变化前Vital)
	int b_vital,  // 化後 vital (变化后vital)
	int str,	  // 化前str (变化前str)
	int b_str,	  // 化後 str (变化后str)
	int tgh,	  // 化前      (变化前tgh)
	int b_tgh,	  // 化後        (变化后tgh)
	int dex,	  // 化前      (变化前dex)
	int b_dex	  // 化後        (变化后dex)
...
#endif
/*------------------------------------------------------------
 * 設定にしたがってすべてのファイルをバックアップ (根据设定备份所有文件)
 * ファイルはクローズされていなければクローズする (若文件尚未关闭则关闭之)
 * 引数  struct tm (参数 struct tm)
 *  なし (无)
 * 返り値 (返回值)
 *  オープンしたファイルの数 (已打开文件的数量)
 ------------------------------------------------------------*/
void backupAllLogFile(struct tm *ptm)
{
	...
	for (i = 0; i < arraysizeof(LogConf); i++) {
		/* append でないものはしない (非append的不处理) */
		if (!LogConf[i].append)
			continue;

		/* バックアップファイルを作る (生成备份文件) */
		sprintf(szBuffer, "%s.%4d%02d%02d", LogConf[i].filename,
				ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday);

		if (LogConf[i].f != NULL) {
			/* オープンされていたらクローズ (若已打开则关闭) */
			fclose(LogConf[i].f);
			/* リネーム (重命名) */
			rename(LogConf[i].filename, szBuffer);
			/* 再びオープン (重新打开) */
